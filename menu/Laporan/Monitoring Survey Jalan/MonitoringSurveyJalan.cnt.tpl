...
//			   	while(aNode.tabViewMenus && tabView.getTab(0))tabView.removeTab(tabView.get('activeTab')); 
		   ...
		   var node = new YAHOO.widget.TextNode("Transaksi", root, false);
{/literal}
...
<tr>
		...
		<ul class="yui-nav">
			<li><a href="#treeDiv1"><em>Navigasi</em></a></li>
		</ul>            
...
/*		YAHOO.util.Event.onDOMReady(function(){
			   var dashboardViewMenus = {/literal}{$dashboardViewMenus}{literal};
			   var tabView = new YAHOO.widget.TabView('dashboardDiv');
			   for(var i=0; i<dashboardViewMenus.length; i++){
					tabView.addTab( new YAHOO.widget.Tab({
						label: dashboardViewMenus[i].Name,
						content: '<iframe marginheight="0" marginwidth="0" class="iframe-dashboard" height="{/literal}{$tabHeight}{literal}px" width="100%" src="?Menu=' + dashboardViewMenus[i].Ref + '" frameborder="0"></iframe>',
						active: true
					}), 0);
			   }
		}); */

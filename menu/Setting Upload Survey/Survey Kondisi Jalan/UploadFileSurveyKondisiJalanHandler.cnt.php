...
class UploadFileSurveyKondisiJalanHandlerContent extends ContentInterface
{
  public function UploadFileSurveyKondisiJalanHandlerContent(){
...
#	$db->Execute("DELETE FROM svy_kondisi_ruas_jalan");
	...
	for ($i=2; $i<=$baris; $i++){ 	
		if($data->val($i, 1) != null){
			  ...
			  #insertdata
			  $rsl = $db->Execute("
				SELECT * FROM svy_kondisi_ruas_jalan 
				WHERE 
						id__mst_ruas_jalan = '". $id_ruas ."' AND
			...
			if($db->RowCount($rsl)== 0){		  
				  $sql = "INSERT INTO 
							svy_kondisi_ruas_jalan(id__mst_ruas_jalan,timestamp,post,offset,id__mst_lajur,id__mst_kondisi_jalan)
						  VALUES (
								'$id_ruas', '$timestamp', '$post','$offset','".$DataLajur['id']."','".$DataKondisi['id']."');";
				  $db->Execute($sql);
				}

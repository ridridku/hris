...
if ($pil !="") {
		...
		if ($pil==2) {
			$sql .= "AND r_pnpt__finger_print = '".$cari."' ";
		}
...
<form action="list_surat_peringatan2.php" method="POST" name="frm">
...
<br>
      ...
      <td  align="left" valign="top" class="ewTableHeader">ID FINGER</td>
			...
			<td  align="left" valign="top" class="ewTableHeader">NO KTP</td><tr>
		      ...
		      <?PHP if ($jum <= 0) { ?>
				...
				<tr align="center"
                                    onclick="GetPengaduan('<?=$list_arr_satuan[$i]['r_pegawai__nama'];?>','<?=$jabatan;?>');"
                                    onMouseOver="oldColor=this.style.backgroundColor;this.style.backgroundColor='#CCFFCC';"
                                    onMouseOut="this.style.backgroundColor=oldColor;",<?=$initSet[$i];?>,'over','<?=$row_class[$i];?>','#CCFFCC','#FFCC99');"
           ...
           <td align="left" class="tdatacontent"> <?=strtoupper($finger);?> </td>
           ...
           <td align="left" class="tdatacontent"><?=strtoupper($list_arr_satuan[$i]['r_dept__ket']);?></td>
					 <td align="left" class="tdatacontent"> <?=strtoupper($list_arr_satuan[$i]['r_jabatan__ket']);?> </td</tr>
				<? }?>
...
<script>
function GetPengaduan(nama_atasan,jabatan_atasan) {
         window.opener.document.getElementById('nama_atasan').value=nama_atasan;
         window.opener.document.getElementById('jabatan_atasan').value=jabatan_atasan;
         window.close();
...
function doPaging(hal)
{
    ...
    frm.action='list_surat_peringatan2.php?q='+kode_cabang_encoded;
    frm.submit();
}

...
$sql="select  tbl_kasus_pengaduan.*, tbl_kasus_pengaduan.kode_wni as kode_link ,  tbl_wni.nama as nama_terlapor , nama_majikan , alamat_majikan, tbl_wni.nama ,no_paspor, nama_kasus, nama_pengaduan,
 nm_perwakilan,
...
where tbl_kasus_pengaduan.kode_perwakilan='$kode_perwakilan' and ( kode_kat_kasus=1 or  kode_kat_kasus=2 or kode_kat_kasus=3    ) ";
...
$sql.="UNION select  tbl_kasus_pengaduan.*, tbl_kasus_pengaduan.kode_bhi as kode_link ,  bhi  as nama_terlapor , '' as nama_majikan , '' as alamat_majikan, bhi as nama ,'' as no_paspor, nama_kasus, nama_pengaduan,
 nm_perwakilan,
 SUBSTRING(tgl_pengaduan,6,2)  as bulan_a ,
 LEFT(tgl_pengaduan,4) as tahun_a , 
 RIGHT(tgl_pengaduan,2) as tanggal_a , '' as nama_kabupaten
 from  tbl_kasus_pengaduan 
 inner join  tbl_bhi on tbl_bhi.kode_bhi  = tbl_kasus_pengaduan.kode_bhi
 left  join tbl_mast_pengaduan on tbl_mast_pengaduan.kode_pengaduan = tbl_kasus_pengaduan.kode_pengaduan
 left join tbl_mast_kasus on tbl_mast_kasus.kode_kasus = tbl_kasus_pengaduan.kode_kasus
  inner join tbl_mast_perwakilan on tbl_mast_perwakilan.kode_perwakilan = tbl_kasus_pengaduan.kode_perwakilan
 where   kode_kat_kasus=5  ";

if ($pil !="") {
	
		if ($pil==1) {
			$sql.=" and bhi LIKE '%".$cari."%' ";
		} 
		if ($pil==2) {
			$sql .= "AND kode_form_pengaduan = '".$cari."' ";
		}
...
<form action="list_pengaduan2.php" method="POST" name="frm">
...
<tr><td colspan="5" align="center" class="judul"><strong>DAFTAR KASUS/PENGADUAN WNI/TKI/ABK  UNTUK PERWAKILAN <?=$nm_perwakilan?> </strong></td></tr>
...
<br>
		...
		<td align="center">
			...
			<td  align="left" valign="top" class="ewTableHeader">NAMA WNI/TKI/ABK/BHI</td>
			...
			<td  align="left" valign="top" class="ewTableHeader">JENIS KASUS</td>
		      ...
		      <? if ($jum <= 0) { ?>
				...
				?>
					...
					<? if ($list_arr_satuan[$i]['kode_kat_kasus']==5){ echo "BHI"; } ?>
					
					</td>
					<td align="left" class="tdatacontent"> <?=strtoupper($list_arr_satuan[$i]['nama']);?> </td>
					<td align="left" class="tdatacontent"> <?=strtoupper($list_arr_satuan[$i]['no_paspor']);?> </td>

					<td align="left" class="tdatacontent"><?=$list_arr_satuan[$i]['kode_wni'];?></td>
					<td align="left" class="tdatacontent"><?=$list_arr_satuan[$i]['nama_kabupaten'];?></td>
					 ...
					 <? 
						 ...
						 $sql_detail2="select * from tbl_kasus_pengaduan_detail  inner join tbl_mast_kasus on tbl_mast_kasus.kode_kasus =tbl_kasus_pengaduan_detail.kode_kasus  where kode_form_pengaduan='$kode_form_pengaduan'";
...
<script>
function GetPengaduan(kode_form_pengaduan,kode_wni,nama,kode_kat_kasus) {
    ...
    window.opener.document.getElementById('kode_wni').value=kode_wni;
	 ...
	 window.opener.document.getElementById('kode_kat_kasus').value=kode_kat_kasus;

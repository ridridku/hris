...
$sql="select * , nm_perwakilan , nama_jenis_wni as nama_jenis ,tbl_wni.nama as nama  , tbl_wni.alamat as alamat_ind  , tbl_wni.tlp as tlp from
						tbl_wni
						...
						left  join tbl_mast_wil_kabupaten on tbl_mast_wil_kabupaten.id_kabupaten = tbl_wni.id_kabupaten where tbl_wni.kode_perwakilan='$kode_perwakilan' and  kode_sumber=6  ";
...
<form action="list_pemberi_kerja_abk_baru.php" method="POST" name="frm" >
...
<tr><td colspan="5" align="center" class="judul"><strong>DAFTAR ABK UNTUK PERWAKILAN <?=$nm_perwakilan?> </strong></td></tr>
...
<td width="30">
		...
		<option value='1' <? if ($pil==1) { echo "selected"; } ?>>Nama ABK</option>
...
<a href="#" OnClick="JavaScript:Ajax('ajax_input','pemberi_kerja_abk_baru/form.php?kode_perwakilan=<?=$kode_perwakilan?>')">
...
<br>
		...
		<td align="center">
			...
			<tr> 
			<td  align="left" valign="top" class="ewTableHeader">NAMA ABK</td>
				...
				?>
					...
					<td align="left" class="tdatacontent"><?=$list_arr_satuan[$i]['alamat_ind'];?></td>
					 ...
					 <td align="left" class="tdatacontent"> 
								...
								<? if ($list_arr_satuan[$i]['kode_sumber']==3 or $list_arr_satuan[$i]['kode_sumber']==4 or $list_arr_satuan[$i]['kode_sumber']==5 or $list_arr_satuan[$i]['kode_sumber']==6){ echo "TKI ABK"; } ?>
...
<script>
function GetPengaduan(kode_wni,nama,alamat,tlp,alamat_ln,tlp_ln) {
...
function doPaging(hal) {
  frm.action="list_pemberi_kerja_abk_baru.php";
  frm.submit();
}
...
//alert(prop_id);
if (prop_id != '') {
	http.open('get', 'list_pemberi_kerja_abk_baru.php?get_prop=1&no_prop='+prop_id);
...
if (prop_id != '') {
	http.open('get', 'list_pemberi_kerja_abk_baru.php?get_kec=1&no_prop='+prop_id+'&no_kab='+kab_id);
...
//alert(prop_id+'  -- '+kab_id);
if (prop_id != '') {
	http.open('get', 'list_pemberi_kerja_abk_baru.php?get_kec=1&no_prop='+prop_id+'&no_kab='+kab_id+'&no_kec='+kec_id);
...
//alert(prop_id);
if (prop_id != '') {
	http.open('get', 'list_pemberi_kerja_abk_baru.php?get_jenis=1&no_prop='+prop_id);
...
//alert(prop_id);
if (prop_id != '') {
	http.open('get', 'list_pemberi_kerja_abk_baru.php?get_jenis_sektor=1&no_prop='+prop_id);

...
FROM
  r_penempatan, r_pegawai, r_cabang, r_subcabang
WHERE
  r_penempatan.r_pnpt__id_pegawai = r_pegawai__id
  AND
  r_penempatan.r_pnpt__subcab = r_subcabang.r_subcab__id
  AND
  r_subcabang.r_subcab__cabang = r_cabang.r_cabang__id AND
  r_penempatan.r_pnpt__aktif = 1  AND r_cabang__id ='$kode_cabang' ";


if ($pil !="") {
	
		if ($pil==1) {
			$sql.=" and r_pegawai.r_pegawai__nama LIKE '%".addslashes($cari)."%' ";
		} 
		if ($pil==2) {
			$sql .= "AND r_penempatan.r_pnpt__nip = '".$cari."' ";
		}
...
<form action="list_daftar_user.php" method="POST" name="frm">
...
<br>
		...
		<td align="center">
			...
			<td  align="left" valign="top" class="ewTableHeader">TELEPON</td>
                        <td  align="left" valign="top" class="ewTableHeader">EMAIL</td>
		      ...
		      <?PHP if ($jum <= 0) { ?>
				...
				<? for ($i=0;$i<count($list_arr_satuan);$i++) { 
				   $j = $i+1;	
				   	$nama=str_replace("'","",$list_arr_satuan[$i]['r_pegawai__nama']);
                                        ...
                                        $jabatan=str_replace("'","",$list_arr_satuan[$i]['r_pegawai__nama_jalan']);
                                        $level=str_replace("'","",$list_arr_satuan[$i]['r_pegawai__tlp_kantor']);    
                                        $shift=str_replace("'","",$list_arr_satuan[$i]['r_pegawai__tlp_rumah']);  
                                        $kelamin=str_replace("'","",$list_arr_satuan[$i]['r_pegawai__jk']); 
				...
				?>
					 ...
					 <td align="left" class="tdatacontent"><?=strtoupper($list_arr_satuan[$i]['r_cabang__nama']);?> </td>
                                         <td align="left" class="tdatacontent"><?=strtoupper($list_arr_satuan[$i]['r_pegawai__nama_jalan']);?> </td>
                                         <td align="left" class="tdatacontent"><?=strtoupper($list_arr_satuan[$i]['r_pegawai__tlp_kantor']);?></td>
                                         <td align="left" class="tdatacontent"><?=strtoupper($list_arr_satuan[$i]['r_pegawai__tlp_rumah']);?></td>
					 <td align="left" class="tdatacontent"> <?=strtoupper($list_arr_satuan[$i]['r_pegawai__jk']);?> </td>
...
<script>
function GetPengaduan(r_pegawai__nama,r_pnpt__nip,r_pegawai__tlp_kantor,r_pegawai__jk,r_pegawai__tlp_rumah,r_pegawai__nama_jalan) {
        ...
        window.opener.document.getElementById('r_pegawai__tlp_kantor').value=r_pegawai__tlp_kantor;
        window.opener.document.getElementById('r_pegawai__jk').value=r_pegawai__jk;
        window.opener.document.getElementById('r_pegawai__tlp_rumah').value=r_pegawai__tlp_rumah;
        window.opener.document.getElementById('r_pegawai__nama_jalan').value=r_pegawai__nama_jalan;
...
function doPaging(hal) {
  frm.action="list_daftar_user.php";
  frm.submit();
}
...
//alert(prop_id);
if (prop_id != '') {
	http.open('get', 'list_daftar_user.php?get_prop=1&no_prop='+prop_id);
...
if (prop_id != '') {
	http.open('get', 'list_daftar_user.php?get_kec=1&no_prop='+prop_id+'&no_kab='+kab_id);
...
//alert(prop_id+'  -- '+kab_id);
if (prop_id != '') {
	http.open('get', 'list_daftar_user.php?get_kec=1&no_prop='+prop_id+'&no_kab='+kab_id+'&no_kec='+kec_id);
...
//alert(prop_id);
if (prop_id != '') {
	http.open('get', 'list_daftar_user.php?get_jenis=1&no_prop='+prop_id);
...
//alert(prop_id);
if (prop_id != '') {
	http.open('get', 'list_daftar_user.php?get_jenis_sektor=1&no_prop='+prop_id);

...
if ($pil !="") {
		...
		if ($pil==2) {
			$sql .= "AND kode_form_pengaduan = '".addslashes($cari)."' ";
		}
		if ($pil==3) { //ktp
		...
		if ($pil==4) { //paspor
			$sql .= "AND tbl_wni.kode_wni like '%".addslashes($cari)."%' ";
		}

		if ($pil==5) { // tahun
			$sql .= "AND LEFT(tgl_pengaduan,4) = '".addslashes($cari)."' ";
		}
...
<option value='1' <? if ($pil==1) { echo "selected"; } ?>>Nama TKI/WNI/ABK </option>
			...
			<td  align="left" valign="top" class="ewTableHeader">JENIS KASUS</td>
		      ...
		      <? if ($jum <= 0) { ?>
				...
				<? for ($i=0;$i<count($list_arr_satuan);$i++) { 
				   ...
				   $kode_form_pengaduan=str_replace("'","",$list_arr_satuan[$i]['kode_form_pengaduan']);
				    $kode_wni=str_replace("'","",$list_arr_satuan[$i]['kode_wni']);
					 $nama=str_replace("'","",$list_arr_satuan[$i]['nama']);

				?>
                
				<tr align="center" onclick="GetPengaduan('<?=$kode_form_pengaduan;?>', '<?=$kode_wni;?>', '<?=$nama;?>');"  onMouseOver="setPointer(this, <?=$initSet[$i];?>, 'over', '<?=$row_class[$i];?>', '#CCFFCC', '#FFCC99');" onMouseOut="setPointer(this, <?=$initSet[$i];?>, 'out', '<?=$row_class[$i];?>', '#CCFFCC', '#FFCC99');" onMouseDown="setPointer(this, <?=$initSet[$i];?>, 'click', '<?=$row_class[$i];?>', '#CCFFCC', '#FFCC99');"> 

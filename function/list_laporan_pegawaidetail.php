...
{ $kode_cabang = $_POST['kode_cabang_cari'];
}else{ $kode_cabang = $_GET['kode_cabang_cari'];}
...
$sql="select * from r_pegawai A where A.r_pegawai__id not in (select distinct B.r_pnpt__id_pegawai from r_penempatan B)";


if ($pil !="") {
	
		if ($pil==1) {
			$sql.=" and A.r_pegawai__nama LIKE '%".addslashes($cari)."%' ";
		} 
		if ($pil==2) {
			$sql .= "AND A.r_pegawai__ktp = '".$cari."' ";
		}
//		if ($pil==3) { //ktp
//
...
$sql.=" ORDER BY A.r_pegawai__user_created DESC "; 
//var_dump($sql)or die();
...
<tr><td colspan="5" align="center" class="judul"><strong>DAFTAR  KARYAWAN  BARU</strong></td></tr>
...
<option value='1' <?PHP if ($pil==1) { echo "selected"; } ?>>Nama  </option>
   <option value='2' <?PHP if ($pil==2) { echo "selected"; } ?>>No KTP</option>
...
<img src="icon_find.gif" border="0">&nbsp;cari
 ...
 <a href="#" OnClick="JavaScript:Ajax('ajax_input','kasus_tki/form.php?kode_cabang=<?=$kode_cabang?>')">
			...
			<td  align="left" valign="top" class="ewTableHeader">NO KTP</td>
		      ...
		      <?PHP if ($jum <= 0) { ?>
				...
				<? for ($i=0;$i<count($list_arr_satuan);$i++) { 
				   $j = $i+1;	
				   	$nama=str_replace("'","",$list_arr_satuan[$i]['r_pegawai__nama']);
                                        $finger=str_replace("'","",$list_arr_satuan[$i]['r_pegawai__nama_jalan']);
				...
				?>
					 ...
					 <td align="left" class="tdatacontent"><?=strtoupper($list_arr_satuan[$i]['r_pegawai__tmpt_lahir']);?> </td>
                                         <td align="left" class="tdatacontent"><?=strtoupper($list_arr_satuan[$i]['r_pegawai__tgl_lahir']);?></td>
					 <td align="left" class="tdatacontent"> <?=strtoupper($list_arr_satuan[$i]['r_pegawai__ktp']);?> </td>
...
<script>
function GetPengaduan(r_pegawai__nama,r_pegawai__id,r_pegawai__tgl_masuk) {

...
where tbl_kasus_pengaduan.kode_perwakilan='$kode_perwakilan'";
...
if ($pil !="") {
		...
		if ($pil==4) { //paspor
			$sql .= "AND tbl_wni.alamat = '".$cari."' ";
		}

		if ($pil==5) { // tahun
			$sql .= "AND nama_kasus = '".$cari."' ";
		}

		if ($pil==6) { // tahun
			$sql.=" and nama_pengaduan '%".$cari."%' ";
		}

        if ($pil==7) { // tahun
			$sql.=" and alamat_ln '%".$cari."%' ";
		}
...
<form action="list_jenazah.php" method="POST" name="frm">
...
<tr><td colspan="5" align="center" class="judul"><strong>DAFTAR JENAZAH WNI/TKI UNTUK PERWAKILAN <?=$nm_perwakilan?> </strong></td></tr>
...
<option value='5' <? if ($pil==5) { echo "selected"; } ?>>Jenis Kasus</option>
<option value='6' <? if ($pil==6) { echo "selected"; } ?>>Jenis Pengaduan</option>
<option value='7' <? if ($pil==7) { echo "selected"; } ?>>Alamat Luar Negeri</option>
<option value='8' <? if ($pil==8) { echo "selected"; } ?>>Telepon Luar Negeri</option>
...
<br>
		...
		<td align="center">
			...
			<td  align="left" valign="top" class="ewTableHeader">JENIS PENGADUAN</td>
        	<td  align="left" valign="top" class="ewTableHeader">ALAMAT LUAR NEGERI</td>
             	<td  align="left" valign="top" class="ewTableHeader">TELEPON LUAR NEGERI</td>
		      ...
		      <? if ($jum <= 0) { ?>
				...
				?>
					...
					<td align="left" class="tdatacontent"><?=$list_arr_satuan[$i]['nama_kasus'];?></td>
					 <td align="left" class="tdatacontent"><?=$list_arr_satuan[$i]['nama_pengaduan'];?></td>
...
<script>
function GetPemulangan(kode_form_pengaduan,kode_wni,nama) {
...
function doPaging(hal) {
  frm.action="list_pemulangan.php";
  frm.submit();
}

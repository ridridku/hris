...
if ($_GET['kode_kat_kasus']) $kode_kat_kasus = $_GET['kode_kat_kasus'];
	...
	if ($jenis_dokumen !='') { 
			if ($jenis_dokumen=='1') { $nama_dokumen='Akte Cerai';  } 
			if ($jenis_dokumen=='2') { $nama_dokumen='Akte Lahir';  }
			if ($jenis_dokumen=='3') { $nama_dokumen='Akte kematian';  }
			if ($jenis_dokumen=='5') { $nama_dokumen='Akte lain-Lain';  }
				 $nama_dokumen = $rs_dokument->fields['jenis_dokumen']; 		 
...
<table width="100%"    border=0 width="100%">	
				...
				<? if ($kode_jenis_dok !='') {  ?>
				 <tr><td class="ewTableHeader"  width="100" ><b>JENIS DOKUMEN</b></td><td width="5"> <b>:</b>  </td><td colspan="2"><b><?=$asal?></b>&nbsp;</td></tr>
				<? } ?>
...
FROM tbl_mast_kawasan a  ";
 ...
 $rs = $db->Execute($sql_kawasan);				
	...
	$chart->setTitle("GRAFIK REKAP LEGALISASI DOKUMEN");
...
<head>
	<title>GRAFIK REKAP LEGALISASI</title>

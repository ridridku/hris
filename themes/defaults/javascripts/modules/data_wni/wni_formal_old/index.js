...
with (theForm){
		...
		{ 
			alert ("Silahkan isi Tempat Lahir WNI !"); 
			tempat_lahir.focus();
			return false; 
		}
	else if (jk.value == "") 
		...
		{ 
			alert ("Silahkan Pilih Provinsi asal WNI !"); 
			no_propinsi.focus();
			return false; 
		}		
	else if (id_kab.value == "") 
		{ 
			alert ("Silahkan Pilih Kabupaten/Kota asal WNI !"); 
			id_kab.focus();
			return false; 
		}
		...
		{ 
			alert ("Silahkan isi Alamat di Indonesia !"); 
			alamat.focus();
			return false; 
		}
			 else if (tlp.value == "") 
		{ 
			alert ("Silahkan isi No.Telp Di Indonesia !"); 
			tlp.focus();
			return false; 
		}
		else if (alamat_ln.value == "") 
		{ 
			alert ("Silahkan isi Alamat di Luar Negeri !"); 
			alamat_ln.focus();
			return false; 
		}
			 else if (tlp_ln.value == "") 
		{ 
			alert ("Silahkan isi No.Telp di Luar Negeri !"); 
			tlp_ln.focus();
			return false; 
		}
			else if (lama_tinggal.value == "") 
		{ 
			alert ("Silahkan isi Lama Tinggal WNI di Luar Negeri !"); 
			lama_tinggal.focus();
			return false; 
		}
		else if (kode_jenis_wni.value == "") 
	 		{ 
			alert ("Silahkan Pilih Jenis Formal !"); 
			kode_jenis_wni.focus();
			return false; 
		}
...
if(http.readyState == 4)
	{ 
		var response = http.responseText;
		 var a = response.split('^/&');
		document.getElementById('ajax_jenis').innerHTML = a[0];
		//frmCreate.nama_lengkap.focus();
    }
}

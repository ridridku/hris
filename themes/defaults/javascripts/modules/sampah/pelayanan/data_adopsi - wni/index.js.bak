...
with (theForm){
		...
		{ 
        	...
        	else if (tgl_adopsi.value == "")
		{
			alert ("Silahkan pilih tanggal adopsi !");
			tgl_adopsi.focus();
			return false;
		}
		else if (kode_wni.value == "")
		{
			alert ("Silahkan Pilih WNI !");
			kode_wni.focus();
			return false;
		}

	 else if (nama_calon_ota.value == "")
		{
			alert ("Silahkan isi Nama Calon Orang Tua !");
			nama_calon_ota.focus();
			return false;
		}

	else if (alamat_cota.value == "")
		{
...
function goLepas() {
			...
			window.open('../../../function/list_adopsi.php?kode_perwakilan='+kode_perwakilan+'',null,'height=500,width=550,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,left=0,top=0,screenX=0,screenY=0')
 			}

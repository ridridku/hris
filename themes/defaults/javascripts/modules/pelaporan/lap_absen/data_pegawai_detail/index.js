...
with (frmList1){
	if (kode_cabang_cari.value == "") 
		{ 
			alert ("Silahkan isi Data cabang aaaa !"); 
...
//------------------JS PILIH cari_karyawan-----------//

function combo_karyawan(subcab_id)
{

if (subcab_id != '') {
	http.open('get','index.php?get_combo_karyawan=1&no_subcab='+subcab_id);
	http.onreadystatechange = handlechoice_combo_karyawan; 
	http.send(null);
	} 
}

function handlechoice_combo_karyawan(){
if(http.readyState == 4)
	{ 
		var response = http.responseText;
		var a = response.split('^/&');
		document.getElementById('ajax_combo_karyawan').innerHTML = a[0];
		
    }
}
//------------------CLOSE JS PILIH SUBCAB-----------//

function goCaripegawai() {
 alert(aaa)
			//document.frm.KodeEselon1.value='';
			//document.frm.NamaEselon1.value='';
			kode_cabang_cari= document.frmCreate.kode_cabang_cari.value;
			window.open('../../../../function/list_laporan_pegawaidetail.php?kode_cabang_cari='+kode_cabang_cari+'',null,'height=500,width=550,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,left=0,top=0,screenX=0,screenY=0')
 			}
</SCRIPT>
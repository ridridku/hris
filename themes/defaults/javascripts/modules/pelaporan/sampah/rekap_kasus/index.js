...
function goDetailBaik(kode_perwakilan,tahun,bulan,kode_kasus,kode_negara,kode_klasifikasi_wni,kode_sektor,kode_jenis,jk,usia) {
		 
			 
			window.open('../../../function/list_rekap_kasus.php?kode_perwakilan='+kode_perwakilan+'&tahun='+tahun+'&bulan='+bulan+'&kode_kasus='+kode_kasus+'&kode_negara='+kode_negara+'&kode_klasifikasi_wni='+kode_klasifikasi_wni+'&kode_sektor='+kode_sektor+'&kode_jenis='+kode_jenis+'&jk='+jk+'&usia='+usia+'',null,'height=500,width=550,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,left=0,top=0,screenX=0,screenY=0')
}


function goDetailKawasan(kode_kawasan,tahun,bulan,kode_kasus,kode_negara,kode_klasifikasi_wni,kode_sektor,kode_jenis,jk,usia) {
 
			window.open('../../../function/list_rekap_kasus_kawasan.php?kode_kawasan='+kode_kawasan+'&tahun='+tahun+'&bulan='+bulan+'&kode_kasus='+kode_kasus+'&kode_negara='+kode_negara+'&kode_klasifikasi_wni='+kode_klasifikasi_wni+'&kode_sektor='+kode_sektor+'&kode_jenis='+kode_jenis+'&jk='+jk+'&usia='+usia+'',null,'height=500,width=550,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,left=0,top=0,screenX=0,screenY=0')
}


function goDetailNegara(kode_kawasan,tahun,bulan,kode_kasus,kode_negara,kode_klasifikasi_wni,kode_sektor,kode_jenis,jk,usia) { 
	//alert (kode_kawasan);
			window.open('../../../function/list_rekap_kasus_negara.php?kode_kawasan='+kode_kawasan+'&tahun='+tahun+'&bulan='+bulan+'&kode_kasus='+kode_kasus+'&kode_negara='+kode_negara+'&kode_klasifikasi_wni='+kode_klasifikasi_wni+'&kode_sektor='+kode_sektor+'&kode_jenis='+kode_jenis+'&jk='+jk+'&usia='+usia+'',null,'height=500,width=550,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,left=0,top=0,screenX=0,screenY=0')
}
...
if(http.readyState == 4)
	{ 
		var response = http.responseText;
		 var a = response.split('^/&');
		document.getElementById('ajax_kawasan').innerHTML = a[0];
		document.getElementById('ajax_negara').innerHTML = a[1];
		//frmCreate.nama_lengkap.focus();
    }
}
...
function cari_negara(prop_id)
{
//alert(prop_id);
if (prop_id != '') {
	http.open('get', 'index.php?get_negara=1&no_prop='+prop_id);
	http.onreadystatechange = handlechoice_negara; 
	http.send(null);
	} 
}

function handlechoice_negara(){
if(http.readyState == 4)
	{ 
		var response = http.responseText;
		 var a = response.split('^/&');
		document.getElementById('ajax_negara').innerHTML = a[0];

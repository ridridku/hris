...
<a class="button" href="#" onclick="this.blur();showLevel('_menuEdit_',1,1);"><span><img src="<!--{$HREF_IMG_PATH}-->/icon/search.png" align="absmiddle"> Pencarian Data</span></a>
		...
		<tr><td class="tcat"> Data Persidangan WNI/TKI</td></tr> 
		</table>
		...
		<TABLE id="table-add-box">
				...
				<TR>
					<TD>Kode Sidang</TD>
					<TD><!--{if $EDIT_VAL==0}-->
					<INPUT TYPE="text" NAME="kode_sidang" value="(AUTONUMBER BY SYSTEM)" size="35" readOnly class="text_disabled">
				 
					<!--{else}-->
					<INPUT TYPE="text" NAME="kode_sidang" value="<!--{$KODE_SIDANG}-->" size="35" readOnly class="text_disabled">
					<!--{/if}-->
					</TD>			
				</TR> 
				...
				<TR>
					...
					<!--{else}-->
								 <input type="text" name="tanggal" value="<!--{$TGL_SIDANG}-->" >
							 <img src="<!--{$HREF_IMG_PATH}-->/icon/calendar.png"  onclick="displayCalendar(document.frmCreate.tanggal,'yyyy-mm-dd',this)"  class="imgLink" align="absmiddle" title="Show Calendar List">  	
					<!--{/if}-->
					</TD>	
				</TR>
		...
		<TR>
					...
					<TD>
						 <textarea cols="50" rows="3" name="pengacara_pendamping"><!--{$PENGACARA_PENDAMPING}--></textarea> 
					</TD>	
				</TR>
				...
				<TR>
					<TD>Materi Sidang </TD>
					<TD>
						 <textarea cols="50" rows="3" name="materi"><!--{$MATERI}--></textarea> 
					</TD>	
				</TR>
	...
	<TABLE WIDTH="100%" ALIGN="CENTER" CELLPADDING="1" CELLSPACING="2">
		...
		<table class="tborder" cellpadding="6" cellspacing="1" border="0" width="100%" align="center">
		<tr><td class="thead"><img src="<!--{$HREF_IMG_PATH}-->/layout/columns.gif" align="absmiddle" border="0"> Daftar Persidangan WNI/TKI</td></tr>
		<tr><td class="alt2" style="padding:0px;">
		<table width="100%">
		<tr>
											...
											<th class="tdatahead">Tanggal Sidang</TH>										 
											<th class="tdatahead">Materi</TH>									 
			...
			<tr class='<!--{cycle values="alt,alt3"}-->'>
											...
											<!--{if ($DATA_TB[x].kode_kat_kasus==1)}-->WNI<!--{/if}-->
											<!--{if ($DATA_TB[x].kode_kat_kasus==2)}-->TKI<!--{/if}-->
											<!--{if ($DATA_TB[x].kode_kat_kasus==3)}-->ABK<!--{/if}-->
											<!--{if ($DATA_TB[x].kode_kat_kasus==4)}-->Staff<!--{/if}-->
											...
											<TD class="tdatacontent" nowrap><!--{$DATA_TB[x].materi}--></TD>
										 
											<TD width="20" class="tdatacontent" ALIGN="CENTER"><IMG SRC="<!--{$HREF_IMG_PATH}-->/icon/edit.gif" WIDTH="12" HEIGHT="13" BORDER=0 ALT="<!--{$EDIT}-->" onclick="return checkEdit('<!--{$SELF}-->?opt=1&kode_sidang=<!--{$DATA_TB[x].kode_sidang}-->&kode_kat_kasus=<!--{$DATA_TB[x].kode_kat_kasus}-->&mod_id=<!--{$MOD_ID}-->&<!--{$STR_COMPLETER_}-->');" class="imgLink"></TD>
											<TD width="20" class="tdatacontent" ALIGN="CENTER"><IMG SRC="<!--{$HREF_IMG_PATH}-->/icon/delete.gif" WIDTH="12" HEIGHT="13" BORDER=0 ALT="<!--{$DELETE}-->" onclick="return checkDelete('engine.php?op=2&kode_sidang=<!--{$DATA_TB[x].kode_sidang}--> &mod_id=<!--{$MOD_ID}-->&<!--{$STR_COMPLETER_}-->');" class="imgLink"></TD>
										</TR>
										<!--{sectionelse}-->
										<TR>

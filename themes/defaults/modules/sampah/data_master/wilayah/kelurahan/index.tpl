...
<a class="button" href="#" onclick="this.blur();showLevel('_menuEdit_',1,1);"><span><img src="<!--{$HREF_IMG_PATH}-->/icon/search.png" align="absmiddle"> Search By</span></a>
		...
		<TABLE id="table-add-box">
				<TR>
					<TD><!--{$TB_CODE}--></TD>
					<TD><!--{if $EDIT_VAL==0}-->
					<INPUT TYPE="text" NAME="kel_id_foo" value="(AUTONUMBER BY SYSTEM)" size="35" readOnly class="text_disabled">
					<INPUT TYPE="hidden" NAME="id_kelurahan" value="<!--{$EDIT_ID_KEL}-->">
					<!--{else}-->
					<INPUT TYPE="text" NAME="id_kelurahan" value="<!--{$EDIT_ID_KEL}-->" size="35" readOnly class="text_disabled">
					<!--{/if}-->
					</TD>			
				</TR>
				<TR>
					<TD><!--{$TB_NO_KELURAHAN}--></TD>
					<TD><INPUT TYPE="text" NAME="no_kelurahan" value="<!--{$EDIT_NO_KEL}-->" size="35"></TD>
				</TR>

				<TR>
					<TD><!--{$TB_NAMA_PROPINSI}--></TD>
					<TD>
				...
				<TR>
					<TD><!--{$TB_NAMA_KABUPATEN}--></TD>
					<TD>
					<div id="ajax_kabupaten">
					<select name="no_kab" onchange="cari_kec(this.value);"> 
						<option value="">[Pilih Kabupaten]</option>
						<!--{section name=x loop=$DATA_KABUPATEN}-->
						<!--{if trim($DATA_KABUPATEN[x].no_kabupaten) == $EDIT_NO_KAB}-->
				...
				<TR>
					<TD><!--{$TB_NAMA_KECAMATAN}--></TD>
					<TD>
					<div id="ajax_kecamatan">
					<select name="no_kec"> 
						<option value="">[Pilih Kecamatan]</option>
						<!--{section name=x loop=$DATA_KECAMATAN}-->
						<!--{if trim($DATA_KECAMATAN[x].no_kecamatan) == trim ($EDIT_NO_KEC)}-->
						<option value="<!--{$DATA_KECAMATAN[x].no_kecamatan}-->" selected > <!--{$DATA_KECAMATAN[x].nama_kecamatan}--> </option>
						<!--{else}-->
						<option value="<!--{$DATA_KECAMATAN[x].no_kecamatan}-->"  > <!--{$DATA_KECAMATAN[x].nama_kecamatan}--> </option>
						<!--{/if}-->
						<!--{/section}-->
						</select>										
					</div></TD>
				...
				<TR>
					<TD><!--{$TB_NAMA_KELURAHAN}--></TD>
					<TD><INPUT TYPE="text" NAME="nama_kelurahan" value="<!--{$EDIT_KELURAHAN_NAMA}-->" size="35"></TD>
				</TR>				
				<TR><td height="40"></td>
					<TD>
					<INPUT TYPE="hidden" name="id_kelurahan" value="<!--{$EDIT_ID_KEL}-->">
					<INPUT TYPE="hidden" name="no_prop" value="<!--{$EDIT_PROV_ID}-->">
					<INPUT TYPE="hidden" name="no_kab" value="<!--{$EDIT_NO_KAB}-->">
					<INPUT TYPE="hidden" name="no_kec" value="<!--{$EDIT_NO_KEC}-->">
	...
	<TABLE WIDTH="100%" ALIGN="CENTER" CELLPADDING="1" CELLSPACING="2">
		...
		<TABLE id="table-search-box">							
			<TR>
								<TD><!--{$TB_NAMA_PROPINSI}--></TD>
								<TD><INPUT TYPE="text" NAME="nama_propinsi" size="30"></TD>
							</TR>
		...
		<table width="100%">
		<tr>
											<th class="tdatahead"><!--{$TB_NO_KELURAHAN}--></TH>
											<th class="tdatahead"><!--{$TB_NAMA_PROPINSI}--></TH>
											<th class="tdatahead"><!--{$TB_NAMA_KABUPATEN}--></TH>
											<th class="tdatahead"><!--{$TB_NAMA_KECAMATAN}--></TH>
											<th class="tdatahead"><!--{$TB_NAMA_KELURAHAN}--></TH>
			...
			<tr class='<!--{cycle values="alt,alt3"}-->'>
											...
											<TD class="tdatacontent" nowrap> <!--{$DATA_TB[x].nama_kelurahan}--> </TD>
											<TD width="20" class="tdatacontent" ALIGN="CENTER"><IMG SRC="<!--{$HREF_IMG_PATH}-->/icon/edit.gif" WIDTH="12" HEIGHT="13" BORDER=0 ALT="<!--{$EDIT}-->" onclick="return checkEdit('<!--{$SELF}-->?opt=1&no_propinsi=<!--{$DATA_TB[x].no_propinsi}-->&no_kabupaten=<!--{$DATA_TB[x].no_kabupaten}-->&no_kecamatan=<!--{$DATA_TB[x].no_kecamatan}-->&id_kelurahan=<!--{$DATA_TB[x].id_kelurahan}-->&mod_id=<!--{$MOD_ID}-->&<!--{$STR_COMPLETER_}-->');" class="imgLink"></TD>
											<TD width="20" class="tdatacontent" ALIGN="CENTER"><IMG SRC="<!--{$HREF_IMG_PATH}-->/icon/delete.gif" WIDTH="12" HEIGHT="13" BORDER=0 ALT="<!--{$DELETE}-->" onclick="return checkDelete('engine.php?op=2&no_propinsi=<!--{$DATA_TB[x].no_propinsi}-->&no_kabupaten=<!--{$DATA_TB[x].no_kabupaten}-->&no_kecamatan=<!--{$DATA_TB[x].no_kecamatan}-->&id_kelurahan=<!--{$DATA_TB[x].id_kelurahan}-->&mod_id=<!--{$MOD_ID}-->&<!--{$STR_COMPLETER_}-->');" class="imgLink"></TD>
										</TR>
										<!--{sectionelse}-->
										<TR>

...
<a class="button" href="#" onclick="this.blur();showLevel('_menuEdit_',1,1);"><span><img src="<!--{$HREF_IMG_PATH}-->/icon/search.png" align="absmiddle"> Search By</span></a>
		...
		<TABLE id="table-add-box">
				...
				<TR>
					<TD>No.KTP</TD>
					<TD><INPUT TYPE="text" NAME="kode_wni" value="<!--{$EDIT_KODE_WNI}-->" size="35">
					<input name="ButtonDepartemen" type="button" class="button" style="cursor: hand;" onclick="goDep()" value=" ... " />
					</TD>
				</TR>
...
<TR>
					...
					<select name="kode_jenis_wni" > 
						...
						<!--{if trim($DATA_JNS_WNI[x].kode_jenis_wni) == $EDIT_KODE_JENIS}-->
						<option value="<!--{$DATA_JNS_WNI[x].kode_jenis_wni}-->" selected > <!--{$DATA_JNS_WNI[x].nama_jenis_wni}--> </option>
						<!--{else}-->
						<option value="<!--{$DATA_JNS_WNI[x].kode_jenis_wni}-->"  > <!--{$DATA_JNS_WNI[x].nama_jenis_wni}--> </option>
						<!--{/if}-->
						<!--{/section}-->
						</select>										
				 </TD>

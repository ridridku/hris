...
<a class="button" href="#" onclick="this.blur();showLevel('_menuEdit_',1,1);"><span><img src="<!--{$HREF_IMG_PATH}-->/icon/search.png" align="absmiddle"> Pencarian Data</span></a>
		...
		<tr><td class="tcat"> MASTER PPTKIS</td></tr>
		</table>
		...
		<TABLE id="table-add-box">
				<TR>
					<TD width="100">KODE JENIS PPTKIS</TD>	
					<TD><!--{if $EDIT_VAL==0}-->					
					<INPUT TYPE="text" NAME="kode_pjtki" value="(OTOMATIS OLEH SISTEM)" size="35" readOnly class="text_disabled">
					<!--{else}-->
					<INPUT TYPE="text" NAME="kode_pjtki" value="<!--{$EDIT_KODE_PJTKI}-->" size="35" readOnly class="text_disabled">
					<!--{/if}-->
					</TD>			
				</TR>
				<TR>
					<TD>NAMA PPTKIS </TD>
					<TD><!--{if $EDIT_VAL==0}-->
					<INPUT TYPE="text" NAME="nama_pjtki" size="35">
					<!--{else}-->
					<INPUT TYPE="text" NAME="nama_pjtki" value="<!--{$EDIT_NAMA_PJTKI}-->" size="35">
					<!--{/if}-->
					</TD>
				</TR>
				<TR>
...
<TR>
					...
					<!--{if ($JENIS_USER_SES==1)}-->
								...
								<!--{else}-->
									...
									<!--{if  ($DATA_PROPINSI[x].no_propinsi) == $KODEP}-->
									<option value="<!--{$DATA_PROPINSI[x].no_propinsi}-->" selected > <!--{$DATA_PROPINSI[x].nama_propinsi}--> </option>
									<!--{else}-->
									<option value="<!--{$DATA_PROPINSI[x].no_propinsi}-->"  > <!--{$DATA_PROPINSI[x].nama_propinsi}--> </option>
									<!--{/if}-->
						...
						<option value=""> Pilih Propinsi </option>
								...
								<!--{else}-->
									...
									<!--{if  trim($DATA_PROPINSI[x].no_propinsi) == trim($KODEP)}-->
									<option value="<!--{$DATA_PROPINSI[x].no_propinsi}-->" selected > <!--{$DATA_PROPINSI[x].nama_propinsi}--> </option>
									<!--{else}-->
									<option value="<!--{$DATA_PROPINSI[x].no_propinsi}-->"  > <!--{$DATA_PROPINSI[x].nama_propinsi}--> </option>
									<!--{/if}-->
			...
			<TR>
				<TD>NAMA PPTKIS</TD>
				<TD><INPUT TYPE="text" NAME="nama_pjtki" id="nama_pjtki" size="25">				
				</TD>
			</TR>
			<TR>
				<TD>ALAMAT</TD>
			...
			<TR><TD></TD>
				...
				<a class="button" href="#" onclick="this.blur(); document.frmList1.reset(); frmList1.nama_pjtki.focus();"><span><img src="<!--{$HREF_IMG_PATH}-->/icon/blank.gif" align="absmiddle"><!--{$RESET}--></span></a>					
				</CENTER>
				</TD>
			</TR>
			</TABLE>
		...
		<th class="tdatahead" align="left">Nama PPTKIS</th>
...
<!--{section name=x loop=$DATA_TB}-->										
			<tr class='<!--{cycle values="alt,alt3"}-->'>
				...
				<td width="20" class="tdatacontent" ALIGN="CENTER"><IMG SRC="<!--{$HREF_IMG_PATH}-->/icon/edit.gif" WIDTH="12" HEIGHT="13" BORDER="0" ALT="<!--{$EDIT}-->" onclick="return checkEdit('<!--{$SELF}-->?opt=1&kode_pjtki=<!--{$DATA_TB[x].kode_pjtki}-->&mod_id=<!--{$MOD_ID}-->&<!--{$STR_COMPLETER_}-->');" class="imgLink"></td>
				<td width="20" class="tdatacontent" ALIGN="CENTER"><IMG SRC="<!--{$HREF_IMG_PATH}-->/icon/delete.gif" WIDTH="12" HEIGHT="13" BORDER="0" ALT="<!--{$DELETE}-->" onclick="return checkDelete('engine.php?op=2&kode_pjtki=<!--{$DATA_TB[x].kode_pjtki}-->&mod_id=<!--{$MOD_ID}-->&<!--{$STR_COMPLETER_}-->');" class="imgLink"></td>
			</tr>
			<!--{sectionelse}-->
			<tr>
...
<INPUT TYPE="hidden" name="nama_pjtki" value="<!--{$nama_pjtki}-->">

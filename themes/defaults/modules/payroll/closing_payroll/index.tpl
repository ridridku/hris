...
<a class="button" href="#" onclick="this.blur();showAll('_menuEntry1_',1);hideAll('_menuEntry2_',1);this.disabled=true;" <!--{if $OPT==1}--> DISABLED <!--{/if}-->><span><img src="<!--{$HREF_IMG_PATH}-->/icon/details.gif" align="absmiddle">Closing Rekap</span></a>
...
<DIV ID="_menuEntry1_1" style="top:10px;width:100%;display:none;position:absolute;">
		...
		<tr><td class="thead"><img src="<!--{$HREF_IMG_PATH}-->/layout/form.gif" align="absmiddle" border="0"> Anda akan closing periode <!--{$PERIODE_AWAL|date_format:"%d-%m-%Y"}--> s/d <!--{$PERIODE_AKHIR|date_format:"%d-%m-%Y"}--> </td></tr>
		...
		<TABLE id="table-add-box">
				...
				<TD>Periode Aktif Berikutnya<font color="#ff0000">*</font></TD>
                                <TD>:
                                <!--{if $EDIT_VAL==0}-->
                                    <input type="text" readonly="" NAME="periode_awal" value="<!--{$START_NEXT|date_format:"%Y-%m-%d"}-->">
							 <img src="<!--{$HREF_IMG_PATH}-->/icon/calendar.png"  onclick="displayCalendar(document.frmCreate.periode_awal,'yyyy-mm-dd',this)"  class="imgLink" align="absmiddle" title="Show Calendar List">
					<!--{else}-->
                                        <input readonly="" type="text" name="periode_awal" value="<!--{$START_NEXT|date_format:"%Y-%m-%d"}-->" >
							 <img src="<!--{$HREF_IMG_PATH}-->/icon/calendar.png"  onclick="displayCalendar(document.frmCreate.periode_awal,'yyyy-mm-dd',this)"  class="imgLink" align="absmiddle" title="Show Calendar List">
					<!--{/if}-->
                                ...
                                <TD>: 
                                <!--{if $EDIT_VAL==0}-->
                                    <input type="text" readonly="" NAME="periode_akhir" value="<!--{$END_NEXT|date_format:"%Y-%m-%d"}-->">
							 <img src="<!--{$HREF_IMG_PATH}-->/icon/calendar.png"  onclick="displayCalendar(document.frmCreate.periode_akhir,'yyyy-mm-dd',this)"  class="imgLink" align="absmiddle" title="Show Calendar List">
					<!--{else}-->
                                        <input readonly="" type="text" name="periode_akhir" value="<!--{$END_NEXT|date_format:"%Y-%m-%d"}-->" >
							 <img src="<!--{$HREF_IMG_PATH}-->/icon/calendar.png"  onclick="displayCalendar(document.frmCreate.periode_akhir,'yyyy-mm-dd',this)"  class="imgLink" align="absmiddle" title="Show Calendar List">
					<!--{/if}-->
...
<!--form_cari--> 			
		...
		<table class="tborder" cellpadding="6" cellspacing="1" border="0" width="100%" align="center" >
		<tr><td class="thead"><img src="<!--{$HREF_IMG_PATH}-->/layout/columns.gif" align="absmiddle" border="0"> Periode aktif  Mulai : <!--{$PERIODE_AWAL|date_format:"%d-%m-%Y"}--> s/d <!--{$PERIODE_AKHIR|date_format:"%d-%m-%Y"}--> </td></tr>
		<tr><td class="alt2" style="padding:0px;">
		...
		<tr>
			...
			<tr>
			  <th width="13%" align="left" class="alt2" >
                              <!--{if ($AWAL_CARI)!=''}--> 
                              <!--{$AWAL_CARI|date_format:"%d-%m-%Y"}-->
                            <!--{else}--> 
                            <!--{$PERIODE_AWAL_AKTIF|date_format:"%d-%m-%Y"}-->
                                 <!--{/if}--></TH>
                            <th width="13%" align="left" class="alt2" > 
                                <!--{if ($AWAL_CARI)!=''}--> 
                              <!--{$AKHIR_CARI|date_format:"%d-%m-%Y"}-->
                            <!--{else}--> 
                            <!--{$PERIODE_AKHIR_AKTIF|date_format:"%d-%m-%Y"}-->
                                 <!--{/if}--></TH>
			...
			<tr>
                        ...
                        <TD align="center"> <!--{if ($DATA_TB[x].rkp_closing)>=4}--> <font color="#1117bf"> Closing</font> 
                        <!--{else}--> <font color="#ff0000"><!--{$DATA_TB[x].t_rkp__awal}--></font> 
                        <!--{/if}--> </TD>
                        <TD align="center"><!--{if ($DATA_TB[x].rkp_closing)>=4}-->  <font color="#1117bf"> Closing</font> 
                        <!--{else}--> <font color="#ff0000"><!--{$DATA_TB[x].t_rkp__akhir}--></font> 
                        <!--{/if}--> </TD>
			...
			<TD> <font color="#0c49f2"><!--{$DATA_TB[x].rkp_peg_aktif}--> </font> </TD>
                        ...
                        <TD align="center"><font color="#f7ba13"><!--{$DATA_TB[x].rkp_hrd}--></font> </TD>

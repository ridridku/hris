...
<a class="button" href="#" onclick="this.blur();showLevel('_menuEdit_',1,1);"><span><img src="<!--{$HREF_IMG_PATH}-->/icon/search.png" align="absmiddle"> Pencarian Data</span></a>
		...
		<tr><td class="tcat"> Data Legalitas Dokumen</td></tr> 
		</table>
		<table class="tborder" cellpadding="6" cellspacing="1" border="0" width="100%" align="center">
		<tr><td class="thead"><img src="<!--{$HREF_IMG_PATH}-->/layout/form.gif" align="absmiddle" border="0"> Form Tambah/Edit Data</td></tr>
		...
		<TABLE id="table-add-box">
				...
				<TR>
					...
					<!--{if $EDIT_VAL==0}-->
					<INPUT TYPE="text" NAME="kode_agenda_masuk"   size="25"   class="text_disabled">
				 
					<!--{else}-->
					<INPUT TYPE="text" NAME="kode_agenda_masuk" value="<!--{$EDIT_KODE_AGENDA_MASUK}-->" size="25" readOnly class="text_disabled">
					<!--{/if}-->
					</TD>			
				</TR>
			...
			<TR>
					<TD>No.Kemlu</TD>
					<TD><INPUT TYPE="text" NAME="no_kemlu" value="<!--{$EDIT_NO_KEMLU}-->" size="25"></TD>
				</TR>

				<TR>
					<TD>No.AHU</TD>
					<TD><INPUT TYPE="text" NAME="no_ahu" value="<!--{$EDIT_NO_AHU}-->" size="25"></TD>
				</TR> 		 
					<TR>
					<TD>Perwakilan</TD>
					<TD> 
				...
				<TR>
					...
					<!--{else}-->
								 <input type="text" name="tanggal" value="<!--{$EDIT_TANGGAL_STEMPEL}-->" >
							 <img src="<!--{$HREF_IMG_PATH}-->/icon/calendar.png"  onclick="displayCalendar(document.frmCreate.tanggal,'yyyy-mm-dd',this)"  class="imgLink" align="absmiddle" title="Show Calendar List">  	
					<!--{/if}-->
					</TD>	
				</TR>
				...
				<TR>
					<TD>Jenis Dokumen</TD>
					<TD>
						<select name="kode_jenis_dok" > 
						<option value="">[ Pilih Jenis Dokumen ]</option>
						<option value="1" <!--{if ($EDIT_KODE_JENIS_DOK==1) }-->  selected <!--{/if}--> >Akte Cerai</option>
						<option value="2" <!--{if ($EDIT_KODE_JENIS_DOK==2) }-->  selected <!--{/if}-->>Akte Kelahiran</option>
						<option value="3" <!--{if ($EDIT_KODE_JENIS_DOK==3) }-->  selected <!--{/if}-->>Akte Kematian</option>
						<option value="4" <!--{if ($EDIT_KODE_JENIS_DOK==4) }-->  selected <!--{/if}--> >Akte Lain</option>	 
						</select>				
					</TD>	
				</TR>
	...
	<TABLE WIDTH="100%" ALIGN="CENTER" CELLPADDING="1" CELLSPACING="2">
		...
		<TABLE id="table-search-box">	
					...
					<!--{/if}-->		
							...
							<TR>
								<TD>Jenis Dokumen</TD>
								<TD><select name="kode_jenis_dok_cari" >
									<option value="">[ Pilih Jenis Dokumen ]</option>
										 <option value="1" <!--{if ($EDIT_KODE_JENIS_DOK==1) }-->  selected <!--{/if}--> >Akte Cerai</option>
										<option value="2" <!--{if ($EDIT_KODE_JENIS_DOK==2) }-->  selected <!--{/if}-->>Akte Kelahiran</option>
										<option value="3" <!--{if ($EDIT_KODE_JENIS_DOK==3) }-->  selected <!--{/if}-->>Akte Kematian</option>
							...
							<TR>
								<TD>No. Agenda</TD>
								<TD><INPUT TYPE="text" NAME="kode_agenda_masuk_cari" size="30"></TD>
							</TR>

							<TR>
								<TD>No. AHU</TD>
								<TD><INPUT TYPE="text" NAME="no_ahu_cari" size="30"></TD>
							</TR>
		...
		<table class="tborder" cellpadding="6" cellspacing="1" border="0" width="100%" align="center">
		<tr><td class="thead"><img src="<!--{$HREF_IMG_PATH}-->/layout/columns.gif" align="absmiddle" border="0"> Daftar Legalitas Dokumen</td></tr>
		<tr><td class="alt2" style="padding:0px;">
		<table width="100%">
		<tr>
											<th class="tdatahead" align="left">No</TH>
											<th class="tdatahead" align="left">No. Agenda</TH>
											<th class="tdatahead" align="left">No. AHU</TH>
											<th class="tdatahead" align="left">No. Kemlu</TH>
											<th class="tdatahead" align="left">Perwakilan</TH>
											<th class="tdatahead" align="left">Nama</TH>
											<th class="tdatahead" align="left">Tgl Stempel</TH>
											<th class="tdatahead" align="left">Jenis Dokumen</TH>
											<th class="tdatahead" align="left">Jumlah Hal</TH>
			...
			<!--{section name=x loop=$DATA_TB}-->										
										<tr class='<!--{cycle values="alt,alt3"}-->'>
											...
											<TD class="tdatacontent" nowrap> <!--{$DATA_TB[x].kode_agenda_masuk}--> </TD>
											<TD class="tdatacontent" nowrap> <!--{$DATA_TB[x].no_ahu}--> </TD>
											<TD class="tdatacontent" nowrap> <!--{$DATA_TB[x].no_kemlu}--> </TD>
											<TD class="tdatacontent" nowrap> <!--{$DATA_TB[x].nm_perwakilan}--> </TD>
											<TD class="tdatacontent" nowrap> <!--{$DATA_TB[x].nama}--> </TD>
											<TD class="tdatacontent" nowrap> <!--{$DATA_TB[x].tgl_input}--> </TD>
											<TD class="tdatacontent" nowrap> 
											<!--{if ($DATA_TB[x].kode_jenis_dok==1)}--> Akte Cerai <!--{/if}--> 
											<!--{if ($DATA_TB[x].kode_jenis_dok==2)}--> Akte Kelahiran <!--{/if}--> 
											<!--{if ($DATA_TB[x].kode_jenis_dok==3)}--> Akte Kematian <!--{/if}--> 
											<!--{if ($DATA_TB[x].kode_jenis_dok==4)}--> Akte Lain <!--{/if}--> 
											</TD>
											<TD class="tdatacontent" nowrap> <!--{$DATA_TB[x].jml_hal}--> </TD>
											<TD width="20" class="tdatacontent" ALIGN="CENTER"><IMG SRC="<!--{$HREF_IMG_PATH}-->/icon/edit.gif" WIDTH="12" HEIGHT="13" BORDER=0 ALT="<!--{$EDIT}-->" onclick="return checkEdit('<!--{$SELF}-->?opt=1&kode_agenda_masuk=<!--{$DATA_TB[x].kode_agenda_masuk}-->&mod_id=<!--{$MOD_ID}-->&<!--{$STR_COMPLETER_}-->');" class="imgLink"></TD>

											<TD width="20" class="tdatacontent" ALIGN="CENTER"><IMG SRC="<!--{$HREF_IMG_PATH}-->/icon/delete.gif" WIDTH="12" HEIGHT="13" BORDER=0 ALT="<!--{$DELETE}-->" onclick="return checkDelete('engine.php?op=2&kode_agenda_masuk=<!--{$DATA_TB[x].kode_agenda_masuk}--> &mod_id=<!--{$MOD_ID}-->&<!--{$STR_COMPLETER_}-->');" class="imgLink">
											</TD>
...
<INPUT TYPE="hidden" name="kode_jenis_dok_cari" value="<!--{$KODE_JENIS_DOK_CARI}-->">
<INPUT TYPE="hidden" name="kode_agenda_masuk_cari" value="<!--{$KODE_AGENDA_MASUK_CARI}-->">
<INPUT TYPE="hidden" name="no_ahu_cari" value="<!--{$NO_AHU_CARI}-->">

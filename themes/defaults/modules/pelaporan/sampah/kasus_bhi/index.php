...
require_once($DIR_INC.'/class.pager.php');
 require_once($DIR_HOME.'/laporan/inc.kasus_bhi.php');   
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/kasus_bhi';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/kasus_bhi';
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$sql = " select  sumber_informasi,   kode_kat_kasus,  'BHI' as jns ,  kronologis_permasalahan, bantuan  , kode_form_pengaduan,   tbl_bhi.bhi   ,   tbl_bhi.bhi   , nama_kasus, nama_pengaduan,
							 nm_perwakilan,
							 SUBSTRING(tgl_pengaduan,6,2)  as bulan_tl ,
							 LEFT(tgl_pengaduan,4) as tahun_tl , 
							 RIGHT(tgl_pengaduan,2) as tanggal_tl  from  tbl_kasus_pengaduan
				...
				$labeltgl=$arr[tanggal_tl]." ".$nama_bulan." ".$arr[tahun_tl];
					...
					$content_data .= print_content($arr[bhi],$arr[sumber_informasi],$arr[nm_perwakilan],$labeltgl,$arr[kronologis_permasalahan],"","","");
 ...
 $sql_detail8="select * from
			...
			$file= $DIR_HOME."/files/laporan_kasus_bhi_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_kasus_bhi_".$nm_perwakilan."_".$tahun.".xls";	
}

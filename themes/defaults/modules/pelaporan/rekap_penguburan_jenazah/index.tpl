...
<TR>
 ...
 <TR>
							...
							<TD>
								...
								<SELECT name="kode_sebab"  style="width:200px" >
								<OPTION VALUE="" selected>[Pilih Sebab Kematian]</OPTION>
								<!--{section name=x loop=$DATA_SEBAB}-->
									<!--{if ($DATA_SEBAB[x].kode_sebab) == $SES_PERWAKILAN}-->
									<option value="<!--{$DATA_SEBAB[x].kode_sebab}-->" selected  > <!--{$DATA_SEBAB[x].penyebab_kematian}--> </option>
									<!--{else}-->
									<option value="<!--{$DATA_SEBAB[x].kode_sebab}-->"  > <!--{$DATA_SEBAB[x].penyebab_kematian}--> </option>
									<!--{/if}-->
									<!--{/section}-->
								</SELECT>
		...
		<tr><td class="tcat">REKAP PENGUBURAN JENAZAH DI LUAR NEGERI</td></tr>
		</table>
...
<!--{if ( $BULAN!='' ) }-->
				...
				<!--{/if}-->
				 ...
				 <!--{if ($NM_SEBAB !='') }-->
											<tr><td class="tdatacontent"  width="100" >SEBAB KEMATIAN</td><td width="5"> : </td><td colspan="2"><!--{$NM_SEBAB}--> &nbsp;</td></tr>
				<!--{/if}-->
											</table>
										   </TD>
											</TR>
...
<div style="position:relative;float:right;margin-top:20;"><INPUT TYPE="text" NAME="print_desc" class="text_transparent" style="background:transparant;border:0;">
 <IMG SRC="<!--{$HREF_IMG_PATH}-->/grafik.jpg"  width="55"  height="55" style="cursor:pointer" align="absmiddle"  onMouseOver="document.frmList.print_desc.value='Grafik';"   onclick="window.open('../../../libchart/grafik/rekap_penguburan_jenazah.php?<!--{$VAR_CARI}-->',null,'height=600,width=700,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,left=0,top=0,screenX=0,screenY=0')" >
 &nbsp;&nbsp;

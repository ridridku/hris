...
$smarty->assign ("HREF_HOME_PATH_AJAX", $HREF_HOME.'/modules/manfaat/master_pinjaman');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/manfaat/master_pinjaman';
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/manfaat');
$FILE_JS  = $JS_MODUL.'/master_pinjaman';
...
if ($_GET['plafon_cari']) $plafon_cari = $_GET['plafon_cari'];
else if ($_POST['plafon_cari']) $plafon_cari = $_POST['plafon_cari'];
else $plafon_cari="";




$smarty->assign ("PLAFON_CARI", $plafon_cari);
...
if ($_GET['search'] == '1')
	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
		  ...
		  if($jenis_user=='2'){
				...
				if($jenis_cari !=''){
					$sql .= " AND r_ang__id = '".$jenis_cari."' ";
				}
                               
				if($plafon_cari !=''){
					$sql .= " AND r_ang__platfond LIKE  '%".$plafon_cari."%' "; 
				}	
			 	$sql .= " ORDER BY  trim(r_ang__id) asc ";
    ...
    
{
			...
			if($jenis_user=='2'){
                            ...
                            $sql = "SELECT
                                                ...
                                                r_ang__date_updated,
                                                r_ang__user_created,
                                                r_ang__user_updated FROM r_angsuran  where 1=1  ";

...
if ($_GET['kode_pn']) $CODE = $_GET['kode_pn'];
else if ($_POST['kode_pn']) $CODE = $_POST['kode_pn'];
else $CODE="";

if ($_GET['nama_pn']) $NAME = $_GET['nama_pn'];
else if ($_POST['nama_pn']) $NAME = $_POST['nama_pn'];
else $NAME="";
...
$smarty->assign ("kode_asal", $KODE_ASAL);
$smarty->assign ("alamat", $ALAMAT);
$smarty->assign ("nama_pn", $NAME);
$smarty->assign ("kode_pn", $CODE);


$str_completer = "mod_id=".$mod_id."&limit=".$LIMIT."&SORT=".$SORT."&kode_pn=".$CODE."&nama_pn=".$NAME."&alamat=".$ALAMAT."&kode_asal=".$KODE_ASAL;
...
if($opt=="1"){
	...
	$sql_	.= "WHERE kode_pn =".$_GET['kode_pn'];
	$resultSet = $db->Execute($sql_);
	$edit_kode_pn = $resultSet->fields[kode_pn];
	$edit_kode_asal = $resultSet->fields[kode_asal];
	$edit_nama_pn = $resultSet->fields[nama_pn];
...
$smarty->assign ("EDIT_KODE_PN", $edit_kode_pn);
$smarty->assign ("EDIT_KODE_ASAL", $edit_kode_asal);
$smarty->assign ("EDIT_NAMA_PN", $edit_nama_pn);
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$sql  = "SELECT * ";
				$sql .= "FROM ".$tbl_name." left join tbl_mast_wil_propinsi on tbl_mast_wil_propinsi.no_propinsi = tbl_mast_pengadilan.no_propinsi ";
				$sql .= "WHERE 1=1 ";

				if($CODE!=''){
					$sql .= "AND kode_pn LIKE '%".$CODE."%' ";
				}
				
				if($KODE_ASAL!=''){
					$sql .= "AND kode_asal LIKE '%".$KODE_ASAL."%' ";
				}
				
				if($NAME!=''){
					$sql .= "AND nama_pn LIKE '%".$NAME."%' ";
				}

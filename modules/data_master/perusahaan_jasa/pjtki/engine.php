...
$user_id = base64_decode($_SESSION['UID']);
 ...
 $sql2="INSERT INTO tbl_log (url, waktu, module, user_id ) VALUES ('$ip_now',now(),'Hapus data >> master PPTKIS','$user_id') ";
 $db->Execute($sql2);
}
...
$ip_now = $_SERVER['REMOTE_ADDR'];
$user_id = base64_decode($_SESSION['UID']);
 $sql2="INSERT INTO tbl_log (url, waktu, module, user_id )  VALUES ('$ip_now',now(),'Ubah data >> master PPTKIS','$user_id') ";
 $db->Execute($sql2);
}
...
global $tbl_name;
global $field_name;
$sql	 = "INSERT INTO tbl_mast_pjtki (`nama_pjtki`,no_propinsi,`alamat`,`tlp`) ";
$sql	.= " VALUES ('".strip_tags($name)."', '$_POST[no_propinsi]','".strip_tags($alamat)."','".strip_tags($tlp)."')";

$sqlresult = $db->Execute($sql);

$ip_now = $_SERVER['REMOTE_ADDR'];
$user_id = base64_decode($_SESSION['UID']);
 $sql2="INSERT INTO tbl_log (url, waktu, module, user_id )  VALUES ('$ip_now',now(),'Tambah Data >> master PPTKIS','$user_id') ";
 $db->Execute($sql2);
}

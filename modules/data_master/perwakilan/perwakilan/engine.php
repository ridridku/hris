...
$user_id = base64_decode($_SESSION['UID']);
 ...
 $sql2="INSERT INTO tbl_log (url, waktu, module, user_id ) VALUES ('$ip_now',now(),'Hapus data >> master Kedutaan Besar RI','$user_id') ";
 $db->Execute($sql2);
}
...
global $db;
			...
			$ip_now = $_SERVER['REMOTE_ADDR'];
$user_id = base64_decode($_SESSION['UID']);
 $sql2="INSERT INTO tbl_log (url, waktu, module, user_id )  VALUES ('$ip_now',now(),'Ubah data >> master Kedutaan Besar RI','$user_id') ";
 $db->Execute($sql2);
...
$sql_cek="select * from tbl_mast_perwakilan where kode_perwakilan='$_POST[kode_perwakilan]'";
 $rs_val = $db->Execute($sql_cek);
 $kode_perwakilan = $rs_val->fields['kode_perwakilan'];
			...
			if ($kode_perwakilan!='') {		 
				...
				$sql	.= " VALUES ('".$_POST['kode_perwakilan']."','".strip_tags($nm_perwakilan)."','".strip_tags($alamat)."','".strip_tags($kode_negara)."','$_POST[tlp]','1')";

				$sqlresult = $db->Execute($sql);
				
$ip_now = $_SERVER['REMOTE_ADDR'];
$user_id = base64_decode($_SESSION['UID']);
 $sql2="INSERT INTO tbl_log (url, waktu, module, user_id )  VALUES ('$ip_now',now(),'Tambah Data >> master Kedutaan Besar RI','$user_id') ";
 $db->Execute($sql2);
				Header("Location:index.php?mod_id=$mod_id&limit=".$_POST[limit]."&SORT=".$_POST['SORT']."&page=".$_POST[page]);	

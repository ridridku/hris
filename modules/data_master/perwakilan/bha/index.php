...
$smarty->assign ("HREF_HOME_PATH_AJAX", $HREF_HOME.'/modules/data_master/perwakilan/bha');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/data_master/perwakilan/bha';

#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/data_master');
$FILE_JS  = $JS_MODUL.'/perwakilan/bha';
...
if ($_GET['kode_bha_cari']) $kode_bha_cari = addslashes($_GET['kode_bha_cari']);
else if ($_POST['kode_bha_cari']) $kode_bha_cari = addslashes($_POST['kode_bha_cari']);
else $kode_bha_cari="";

if ($_GET['nama_bha_cari']) $nama_bha_cari = addslashes($_GET['nama_bha_cari']);
else if ($_POST['nama_bha_cari']) $nama_bha_cari = addslashes($_POST['nama_bha_cari']);
else $nama_bha_cari="";

//if ($_GET['alamat_cari']) $alamat_cari = $_GET['alamat_cari'];
//else if ($_POST['alamat_cari']) $alamat_cari = $_POST['alamat_cari'];
//else $alamat_cari="";
$smarty->assign ("KODE_BHA_CARI", $kode_bha_cari);
$smarty->assign ("NAMA_BHA_CARI", $nama_bha_cari);
 
$str_completer = "mod_id=".$mod_id."&limit=".$LIMIT."&SORT=".$SORT."&kode_bha_cari=".$kode_bha_cari."&nama_bha_cari=".$nama_bha_cari.
//"&alamat=".$alamat_cari;
...
//echo "<br><br><br><br><br><br><br><br><br><br>kode_perwakilan_cari".$kode_bha_cari;
//SHOW DATA PROVINSI
//-------------------------------------
...
if($opt=="1"){
	...
	$sql_ = "select  *  from tbl_mast_badan_hukum_asing
	where  kode_bha ='".$_GET['kode_bha']."'  ";
 
	//print $sql_;
	$resultSet = $db->Execute($sql_);
	$edit_kode_bha = $resultSet->fields[kode_bha];
	$edit_nama_bha= $resultSet->fields[nama_bha];
...
$smarty->assign ("EDIT_KODE_PERWAKILAN_ASING", $edit_kode_bha);
$smarty->assign ("EDIT_NAMA_PERWAKILAN", $edit_nama_bha);
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
			    ...
			    if($kode_bha_cari!=''){
					$sql .= "AND kode_bha  like '%".$kode_bha_cari."%' ";
				}

				 if($nama_bha_cari!=''){
					$sql .= "AND nama_bha like '%".$nama_bha_cari."%' ";
				}

				 
 
				 $sql .= " ORDER BY  nama_bha ";

...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/data_master/perwakilan/perwakilan_asing';

#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/data_master');
$FILE_JS  = $JS_MODUL.'/perwakilan/perwakilan_asing';
...
if ($_GET['kode_perwakilan_asing_cari']) $kode_perwakilan_asing_cari = $_GET['kode_perwakilan_asing_cari'];
else if ($_POST['kode_perwakilan_asing_cari']) $kode_perwakilan_asing_cari = $_POST['kode_perwakilan_asing_cari'];
else $kode_perwakilan_asing_cari="";

if ($_GET['nama_perwakilan_cari']) $nama_perwakilan_cari = $_GET['nama_perwakilan_cari'];
else if ($_POST['nama_perwakilan_cari']) $nama_perwakilan_cari = $_POST['nama_perwakilan_cari'];
else $nama_perwakilan_cari="";
 
$smarty->assign ("KODE_PERWAKILAN_ASING_CARI", $kode_perwakilan_asing_cari);
$smarty->assign ("NAMA_PERWAKILAN_CARI", $nama_perwakilan_cari);

$str_completer = "mod_id=".$mod_id."&limit=".$LIMIT."&SORT=".$SORT."&kode_perwakilan_asing=".$kode_perwakilan_asing_cari."&nm_perwakilan=".$nama_perwakilan_cari;
$str_completer_ = "limit=".$LIMIT."&SORT=".$SORT."&page=".$page;



// SHOW DATA NEGARA ========================
$sql_negara_show = "SELECT  *  FROM tbl_mast_negara   order by nama_negara  ";
...
//echo "<br><br><br><br><br><br><br><br><br><br>kode_perwakilan_cari".$kode_perwakilan_asing_cari;
//SHOW DATA PROVINSI
//-------------------------------------
...
if($opt=="1"){
	...
	$sql_ = "select  *  from tbl_mast_perwakilan_asing
	where  kode_perwakilan_asing ='".$_GET['kode_perwakilan_asing']."'  ";
 
	//print $sql_;
	$resultSet = $db->Execute($sql_);
	$edit_kode_perwakilan_asing = $resultSet->fields[kode_perwakilan_asing];
	$edit_nama_perwakilan= $resultSet->fields[nama_perwakilan];
	$edit_alamat= $resultSet->fields[alamat];
	$edit_tlp= $resultSet->fields[tlp];

	$edit_fax= $resultSet->fields[fax];

	$edit_email= $resultSet->fields[email];
...
$smarty->assign ("EDIT_NAMA_PERWAKILAN", $edit_nama_perwakilan);
$smarty->assign ("EDIT_ALAMAT", $edit_alamat);
$smarty->assign ("EDIT_TLP", $edit_tlp);
 
 $smarty->assign ("EDIT_FAX", $edit_fax);
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$sql  = "select * from tbl_mast_perwakilan_asing left join tbl_mast_negara on tbl_mast_negara.kode_negara = tbl_mast_perwakilan_asing.kode_negara  where 1=1  and kode_jenis_pw=1 ";
  
			    if($kode_perwakilan_asing_cari!=''){
					$sql .= "AND kode_perwakilan_asing  = '".$kode_perwakilan_asing_cari."' ";
				}

				 if($nama_perwakilan_cari!=''){
					$sql .= "AND nama_perwakilan like '%".$nama_perwakilan_cari."%' ";
				}

				//if($alamat_cari!=''){
					//$sql .= "AND alamat_cari LIKE '%".$alamat_cari."%' ";
				//} 
 
				  $sql .= " ORDER BY  nama_perwakilan ";

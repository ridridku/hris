...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/data_master/perwakilan/konsulat_jenderal';

#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/data_master');
$FILE_JS  = $JS_MODUL.'/perwakilan/konsulat_jenderal';
...
if ($_GET['nm_perwakilan']) $nm_perwakilan = $_GET['nm_perwakilan'];
else if ($_POST['nm_perwakilan']) $nm_perwakilan = $_POST['nm_perwakilan'];
else $nm_perwakilan="";
...
$smarty->assign ("KODE_PERWAKILAN", $kode_perwakilan);
$smarty->assign ("NM_PERWAKILAN", $nm_perwakilan);

$str_completer = "mod_id=".$mod_id."&limit=".$LIMIT."&SORT=".$SORT."&kode_perwakilan=".$kode_perwakilan."&nm_perwakilan=".$nm_perwakilan."&alamat=".$alamat_cari;
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
			...
			$sql  = "select * from tbl_mast_perwakilan left join tbl_mast_negara on tbl_mast_negara.kode_negara =  tbl_mast_perwakilan.kode_negara where 1=1  and kode_jenis_pw=2 ";
  
			    if($kode_perwakilan!=''){
					$sql .= "AND kode_perwakilan  = '".$kode_perwakilan."' ";
				}

				 if($nm_perwakilan!=''){
					$sql .= "AND nm_perwakilan like '%".$nm_perwakilan."%' ";
				}

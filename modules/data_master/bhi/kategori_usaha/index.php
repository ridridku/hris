...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/data_master/bhi/kategori_usaha';
...
$FILE_JS  = $JS_MODUL.'/bhi/kategori_usaha';
...
else $ORDER="kode_kat_usaha";

if ($_GET['page']) $page = $_GET['page'];
else if ($_POST['page']) $page = $_POST['page'];
else $page="1";

if ($_GET['kode_kat_usaha']) $CODE = $_GET['kode_kat_usaha'];
else if ($_POST['kode_kat_usaha']) $CODE = $_POST['kode_kat_usaha'];
else $CODE="";

if ($_GET['nama_usaha']) $NAME = $_GET['nama_usaha'];
else if ($_POST['nama_usaha']) $NAME = $_POST['nama_usaha'];
else $NAME="";



$smarty->assign ("kode_kat_usaha", $CODE);
$smarty->assign ("nama_usaha", $NAME);

$str_completer = "mod_id=".$mod_id."&limit=".$LIMIT."&SORT=".$SORT."&kode_kat_usaha=".$CODE."&nama_usaha=".$NAME;
...
if($opt=="1"){
	...
	$sql_	.= "WHERE kode_kat_usaha =".$_GET['kode_kat_usaha'];
	$resultSet = $db->Execute($sql_);
	$edit_kode_kat_usaha = $resultSet->fields[kode_kat_usaha];
	$edit_nama_usaha = $resultSet->fields[nama_usaha];
...
$smarty->assign ("EDIT_NAMA_USAHA", $edit_nama_usaha);
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				if($CODE!=''){
					$sql .= "AND kode_kat_usaha LIKE '%".$CODE."%' ";
				}
				
				if($NAME!=''){
					$sql .= "AND nama_usaha LIKE '%".$NAME."%' ";
				}

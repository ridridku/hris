...
else $ORDER="a.no_propinsi ASC,a.id_kabupaten";

if ($_GET['page']) $page = $_GET['page'];
else if ($_POST['page']) $page = $_POST['page'];
else $page="1";

if ($_GET['id_kabupaten']) $CODE = $_GET['id_kabupaten'];
else if ($_POST['id_kabupaten']) $CODE = $_POST['id_kabupaten'];
else $CODE="";

if ($_GET['nama_kabupaten']) $NAME = $_GET['nama_kabupaten'];
else if ($_POST['nama_kabupaten']) $NAME = $_POST['nama_kabupaten'];
else $NAME="";
...
$smarty->assign ("NAMA_PROPINSI", $NAMA_PROP);
$smarty->assign ("NAMA_KABUPATEN", $NAME);

$str_completer = "mod_id=".$mod_id."&limit=".$LIMIT."&SORT=".$SORT."&id_kabupaten=".$CODE."&nama_kabupaten=".$NAME."&nama_propinsi=".$NAMA_PROP;
...
if ($_GET[get_prop] == 1)
{
	$no_propinsi = $_GET[no_propinsi];
			if($no_propinsi!=''){	
					$sql_kabupaten = "SELECT * FROM tbl_mast_wil_propinsi WHERE no_propinsi='".$no_propinsi."' ORDER BY id_propinsi ASC";
					$recordSet_kabupaten = $db->Execute($sql_kabupaten);
					$input_kab="<select name=no_prop >";
...
if ($opt=="1") {
	...
	$sql_	 = "SELECT * FROM tbl_mast_wil_kabupaten ";
	$sql_	.= "WHERE no_propinsi ='".$_GET['no_propinsi']."' AND id_kabupaten='".$_GET['id_kabupaten']."'";

	$resultSet = $db->Execute($sql_);
	$edit_prov_id = $resultSet->fields[no_propinsi];
	$edit_no_kab= $resultSet->fields[no_kabupaten];
	$edit_kab_id = $resultSet->fields[id_kabupaten];
	$edit_nama_kabupaten = $resultSet->fields[nama_kabupaten];
	$edit = 1;
	//$opt = 1;
...
$smarty->assign("EDIT_KABUPATEN_NAMA",$edit_nama_kabupaten);
$smarty->assign("EDIT_KAB_ID",$edit_kab_id);
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$sql  = "SELECT a.no_propinsi,a.id_kabupaten,b.nama_propinsi,a.nama_kabupaten 
				FROM tbl_mast_wil_kabupaten a LEFT JOIN tbl_mast_wil_propinsi b ON a.no_propinsi=b.no_propinsi WHERE 1=1 ";

				if($CODE!=''){
					$sql .= "AND a.id_kabupaten LIKE '%".$CODE."%' ";
				}

				if($NAME!=''){
					$sql .= "AND a.nama_kabupaten LIKE '%".$NAME."%' ";
				}

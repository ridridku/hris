...
$user_id = base64_decode($_SESSION['UID']);
 ...
 $sql2="INSERT INTO tbl_log (url, waktu, module, user_id ) VALUES ('$ip_now',now(),'Hapus data >> master WNA','$user_id') ";
 $db->Execute($sql2);
}
...
$ip_now = $_SERVER['REMOTE_ADDR'];
$user_id = base64_decode($_SESSION['UID']);
 $sql2="INSERT INTO tbl_log (url, waktu, module, user_id )  VALUES ('$ip_now',now(),'Ubah data >> master WNA','$user_id') ";
 $db->Execute($sql2);
}
...
global $field_name;
		...
		$sql	.= " VALUES ( '".strip_tags($kode_negara)."','".strip_tags($nama_wna)."','".strip_tags($alamat_ind)."','".strip_tags($tlp_ind)."','".strip_tags($alamat_ln)."','".strip_tags($tlp_ln)."','".strip_tags($keterangan)."')";

		$sqlresult = $db->Execute($sql);
$ip_now = $_SERVER['REMOTE_ADDR'];
$user_id = base64_decode($_SESSION['UID']);
 $sql2="INSERT INTO tbl_log (url, waktu, module, user_id )  VALUES ('$ip_now',now(),'Tambah Data >> master WNA','$user_id') ";
 $db->Execute($sql2);

...
$user_id = base64_decode($_SESSION['UID']);
 ...
 $sql2="INSERT INTO tbl_log (url, waktu, module, user_id ) VALUES ('$ip_now',now(),'Hapus data >> master Lokal Staf','$user_id') ";
 $db->Execute($sql2);
}
...
$ip_now = $_SERVER['REMOTE_ADDR'];
$user_id = base64_decode($_SESSION['UID']);
 $sql2="INSERT INTO tbl_log (url, waktu, module, user_id )  VALUES ('$ip_now',now(),'Ubah data >> master Lokal Staf','$user_id') ";
 $db->Execute($sql2);
}
...
$sql_cek="select * from tbl_mast_staf where kode_staf='$_POST[kode_staf]' ";
 $rs_val = $db->Execute($sql_cek);
 $kode_staf = $rs_val->fields['kode_staf'];
		...
		if ($kode_staf!='') {
				...
				$sql	.= " VALUES ('".$_POST['kode_staf']."','".strip_tags($kode_p)."','".strip_tags($nama)."','".strip_tags($alamat)."','".strip_tags($tlp)."','".strip_tags($jk)."','".strip_tags($tgl_lahir)."','$_POST[tempat_lahir]')";

				$sqlresult = $db->Execute($sql);
$ip_now = $_SERVER['REMOTE_ADDR'];
$user_id = base64_decode($_SESSION['UID']);
 $sql2="INSERT INTO tbl_log (url, waktu, module, user_id )  VALUES ('$ip_now',now(),'Tambah Data >> master Lokal Staf','$user_id') ";
 $db->Execute($sql2);

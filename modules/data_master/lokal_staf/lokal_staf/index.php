...
$smarty->assign ("HREF_HOME_PATH_AJAX", $HREF_HOME.'/modules/data_master/lokal_staf/lokal_staf');
...
if ($_GET['kode_staf']) $CODE = addslashes($_GET['kode_staf']);
else if ($_POST['kode_staf']) $CODE = addslashes($_POST['kode_staf']);
else $CODE="";
...
if ($_GET['nama']) $NAMA = addslashes($_GET['nama']);
else if ($_POST['nama']) $NAMA = addslashes($_POST['nama']);
else $NAMA="";
...
if ($_GET['kode_jenis_pw_cari']) $kode_jenis_pw_cari = $_GET['kode_jenis_pw_cari'];
 ...
 $smarty->assign ("KODE_JENIS_PW_CARI", $kode_jenis_pw_cari);
$smarty->assign ("KODE_STAF", $kode_staf);
...
if ($_GET[get_prop] == 1)
{
	$no_propinsi = $_GET[no_prop];
			if($no_propinsi!=''){	
					$sql_kabupaten = "SELECT * from tbl_mast_perwakilan_asing  WHERE kode_jenis_pw='".$no_propinsi."'  order by nama_perwakilan asc ";
...
if($opt=="1"){
	...
	$sql_	 = "SELECT * , tbl_mast_staf.alamat as alamat FROM ".$tbl_name."  inner join tbl_mast_perwakilan_asing on tbl_mast_perwakilan_asing.kode_perwakilan_asing = tbl_mast_staf.kode_perwakilan_asing ";
...
$sql_perwakilan_asing_show = "SELECT  *  FROM tbl_mast_perwakilan_asing where kode_jenis_pw='$kode_jenis_pw' order by nama_perwakilan asc ";
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$sql  = "SELECT * ,   tbl_mast_staf.alamat as alamat   ";

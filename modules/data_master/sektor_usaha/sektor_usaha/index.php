...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/data_master/sektor_usaha/sektor_usaha';

#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/data_master');
$FILE_JS  = $JS_MODUL.'/sektor_usaha/sektor_usaha';
...
if ($_GET['kode_sektor_cari']) $kode_sektor_cari = $_GET['kode_sektor_cari'];
else if ($_POST['kode_sektor_cari']) $kode_sektor_cari = $_POST['kode_sektor_cari'];
else $kode_sektor_cari="";

if ($_GET['nama_sektor_cari']) $nama_sektor_cari = $_GET['nama_sektor_cari'];
else if ($_POST['nama_sektor_cari']) $nama_sektor_cari = $_POST['nama_sektor_cari'];
else $nama_sektor_cari="";

//if ($_GET['alamat_cari']) $alamat_cari = $_GET['alamat_cari'];
//else if ($_POST['alamat_cari']) $alamat_cari = $_POST['alamat_cari'];
//else $alamat_cari="";

 $smarty->assign ("KODE_SEKTOR_CARI", $kode_sektor_cari);
$smarty->assign ("NAMA_SEKTOR_CARI", $nama_sektor_cari);
$str_completer = "mod_id=".$mod_id."&limit=".$LIMIT."&SORT=".$SORT."&kode_sektor=".$kode_sektor_cari."&nama_sektor =".$nama_sektor_cari.
//"&alamat=".$alamat_cari;
...
if($opt=="1"){
	...
	$sql_ = "select  *  from tbl_mast_sektor
	where  kode_sektor ='".$_GET['kode_sektor']."'  ";
 
	//print $sql_;
	$resultSet = $db->Execute($sql_);
	$edit_kode_sektor = $resultSet->fields[kode_sektor];
	$edit_nama_sektor= $resultSet->fields[nama_sektor];
...
$smarty->assign ("EDIT_NAMA_SEKTOR", $edit_nama_sektor);
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$sql  = "select * from tbl_mast_sektor where 1=1  ";
  
			    if($kode_sektor_cari!=''){
					$sql .= "AND kode_sektor  like '%".$kode_sektor_cari."%' ";
				}

				 if($nama_sektor_cari!=''){
					$sql .= "AND nama_sektor = '".$nama_sektor_cari."' ";
				}

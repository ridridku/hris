...
$opt = $_GET[opt];
$id_form_jl_03_main = $_GET[id_fjl_03_main];
$id_form_jl_03_detail = $_GET[id_fjl_03_detail];
$ed = 0;

if ($opt=="1" AND $id_form_jl_03_main<>'' AND $id_form_jl_03_detail<>'') {
...
$sql = "SELECT * 
		FROM ".$tbl_name_detail." a LEFT JOIN ".$tbl_name_main." b ON a.id_fjl_03_main=b.id_fjl_03_main 
		WHERE 1=1 AND b.id_fjl_03_main='".$id_form_jl_03_main."' AND a.id_fjl_03_detail='".$id_form_jl_03_detail."' AND id_jns_jln='".$id_jns_jln."' ";
...
$smarty->assign ("ID_JL_03_MAIN", $recordSet_Edit->fields[id_fjl_03_main]);
$smarty->assign ("ID_FORM_JL_03_DETAIL", $recordSet_Edit->fields[id_fjl_03_detail]);
...
$smarty->assign ("RD_DAK", $recordSet_Edit->fields[rd_dak]);
$smarty->assign ("RD", $recordSet_Edit->fields[rd]);
$smarty->assign ("ALASAN", $recordSet_Edit->fields[alasan]);
$smarty->assign ("KELENGKAPAN_GAMBAR", $recordSet_Edit->fields[kelengkapan_gambar]);
$smarty->assign ("KELENGKAPAN_SPESIFIK", $recordSet_Edit->fields[kelengkapan_spesifik]);
$smarty->assign ("KELENGKAPAN_RAB", $recordSet_Edit->fields[kelengkapan_rab]);
...
$smarty->assign("JENIS_JLN",$recordSet_Edit->fields[id_jns_jln]);
//$jenis_jalan	= $recordSet_Edit->fields[id_jns_jln]==1? "Provinsi":"Kabupaten/ Kota";
$smarty->assign("ID_HIDDEN_JSN_JLN",$recordSet_Edit->fields[id_jns_jln]);

$jenis_jalan	= $recordSet_Edit->fields[id_jns_jln]==1? "Provinsi":"Kabupaten/ Kota";

$smarty->assign ("PID_JENIS_JLN", $jenis_jalan);
//$smarty->assign ("PID_JENIS_JLN", $jenis_jalan);
/*** End 0f 21-06-2010 ***/
}
...
if ($_GET[get_kabupaten] == 1)
{
			...
			if($no_propinsi!='' AND $no_kabupaten!=''){	
					...
					while(!$result_data_ruas_jalan ->EOF):
						$data_ruas_jalan .= "<option value=\"".$result_data_ruas_jalan->fields['no_ruas']."\">".$result_data_ruas_jalan->fields['no_ruas']."-> ".$result_data_ruas_jalan->fields['nama_pangkal_ruas']." - ".$result_data_ruas_jalan->fields['nama_ujung_ruas']."</option>";
...
<td class=\"text-regular\">".$result_kecamatan_combo  ->fields['nama_kecamatan']."</td>";
					...
					$table_kecamatan.="<input type=\"hidden\" name=\"jml_kecamatan\" value=\"".$z++."\"><br>";				
					$delimeter   = "-";
					$option_choice = $table_kecamatan."^/&".$data_ruas_jalan."^/&".$delimeter;	
					echo $option_choice;
			}
}
...
if ($_GET[get_data_ruas]==1) {
	...
	*****/
	...
	$tmp_nama_ruas = "<input name=\"nama_ruas\" type=\"text\" value=\"".$result_ruas->fields[nama_pangkal_ruas]." - ".$result_ruas->fields[nama_ujung_ruas]."\" size=\"75\" maxlength=\"200\" readonly>";
	...
	/***
		...
		document.getElementById('ajax_kecamatan').innerHTML = a[2];
		document.getElementById('ajax_hidden_jns_jln').innerHTML = a[3];	
	***/	
	
	$delimeter   = "-";
	$option_choice = $tmp_nama_ruas."^/&".$tmp_jsn_jln."^/&".$table_kecamatan."^/&".$tmp_hidden_jsn_jln."^/&".$delimeter;	
	echo $option_choice;
	}
}
//---------------------------------------------------------------
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				***/
				...
				$sql = "SELECT b.id_fjl_03_main,a.id_fjl_03_detail,a.nama_ruas,a.kecamatan_yg_dilalui,if(a.rd_dak='1','Ya','Tidak') as rd_dak,if(a.rd='1','Ya','Tidak') as rd  
				FROM ".$tbl_name_detail." a LEFT JOIN ".$tbl_name_main." b ON a.id_fjl_03_main=b.id_fjl_03_main 
				WHERE 1=1 ";
...
$smarty->assign ("TABLE_CAPTION", _CAPTION_TABLE_FORM_JL_03);
$smarty->assign ("TABLE_NAME", _NAMA_TABLE_FORM_JL_03);
...
$smarty->assign ("TITLE", _TITLE_FORM_JL_03_MAIN);
$smarty->assign ("HEAD_TITLE", _HEAD_TITLE_FORM_JL_03_MAIN);
...
$smarty->assign ("TB_KESESUAIAN_PROGRAM", _KESESUAIAN_PROGRAM);
$smarty->assign ("TB_KESESUAIAN_RD_DAK", _KESESUAIAN_RD_DAK);
$smarty->assign ("TB_KESESUAIAN_RD", _KESESUAIAN_RD);
$smarty->assign ("SUBMIT", _SUBMIT);
$smarty->assign ("RESET", _RESET);
$smarty->assign ("LIST_ME", _LIST_FORM_JL_03_MAIN);
...
$smarty->assign ("ID_JNS_JLN", $jns_jln);
/*** End 0f 06-21-2010 ***/

...
function import_() {
	...
	$get_sql		= " SELECT * FROM `".$tmp_tbl_name_main."` ORDER BY id_fjl_09_main ASC ";
	//print $get_sql;
		...
		if($get_recordSet2->RecordCount()>0) {
			while ($arr2=$get_recordSet2->FetchRow()) {
				$sql_insert2 = " INSERT INTO `".$tbl_name_detail."` (`id_fjl_09_main`,`nama_paket`,`tujuan`,`manfaat`,`keterangan`,`id_jns_jln`) VALUES (
					...
					" .sqlvalue(@$get_recordSet2->fields["tujuan"], true) .", 
					" .sqlvalue(@$get_recordSet2->fields["manfaat"], false) .", 
					" .sqlvalue(@$get_recordSet2->fields["keterangan"], false) .", 
					" .sqlvalue(@$get_recordSet2->fields["id_jns_jln"], true) .") ";
					
				$sql_exec2 = $tb_import->Execute($sql_insert2);
				$sql_del = "DELETE FROM `".$tmp_tbl_name_detail."` WHERE id_fjl_09_detail='{$get_recordSet2->fields[id_fjl_09_detail]}' ";

...
$sql = " INSERT INTO `".$tbl_name_main."` (`tanggal`, `no_propinsi`, `no_kabupaten`, `tahun1`, `tahun2`, `id_jns_jln`) 
        VALUES (
...
function import_() {
  ...
  if(!$conn)
	...
	$SES_NO_PROP	= $_SESSION['SESSION_NO_PROPINSI'];
			...
			$get_sql		= " SELECT * FROM tbl_form_jl_10_main  where no_propinsi=".$SES_NO_PROP; //localhost atw daerah
	...
	if (@$count!='') {
				...
				while ($arr=$get_recordSet->FetchRow()) { // loop data di daerah
					...
					//cek apakah data sudah ada di server atw belum melalui id_auto_daerah
					 $sql_cek="select * from tbl_form_jl_10_main where id_auto_daerah=".sqlvalue(@$arr["id_fjl_10_main"], true)." AND no_propinsi=".sqlvalue(@$arr["no_propinsi"], true)." and no_kabupaten=".sqlvalue(@$arr["no_kabupaten"], true);
					 $rs_cari =   $tb_import->Execute($sql_cek); 
					 $id_auto_daerah = $rs_cari->fields['id_auto_daerah'];
					 $id_fjl_10_main_cek = $rs_cari->fields['id_fjl_10_main'];

					if ($id_auto_daerah !='') { //data sudah ada di pusat
						 
						// script update DI PUSAT
 
						$tmp_sql = " UPDATE tbl_form_jl_10_main  SET " ;
								$tmp_sql.= "  tanggal='".$arr[tanggal]."'," ;
								$tmp_sql.= "  no_propinsi='".$arr[no_propinsi]."'," ;
								$tmp_sql.= "  no_kabupaten='".$arr[no_kabupaten]."'," ;
								$tmp_sql.= "  tahun1='".$arr[tahun1]."'," ;
								$tmp_sql.= "  tahun2='".$arr[tahun2]."'," ;
								$tmp_sql.= "  id_jns_jln='".$arr[id_jns_jln]."' "  ;
								$tmp_sql.= "  where id_fjl_10_main ='".$id_fjl_10_main_cek."' " ;
								$tmp_sqlresult = $tb_import->Execute($tmp_sql);	

								 $sql_del = "DELETE FROM tbl_form_jl_10_detail WHERE id_fjl_10_main='$id_fjl_10_main_cek' ";
							...
							$sql_insert = " INSERT INTO `".$tbl_name_main."` (`tanggal`, `no_propinsi`, `no_kabupaten`, `tahun1`, `tahun2`, `id_jns_jln` ,id_auto_daerah) 
							...
							" .sqlvalue(@$arr["id_fjl_10_main"], false) .")";
					 ...
					 $sql_detail	=  " SELECT * FROM `".$tbl_name_detail."` WHERE id_fjl_10_main='$arr[id_fjl_10_main]' ";

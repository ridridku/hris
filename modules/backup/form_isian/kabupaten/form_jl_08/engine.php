...
//=================================================================
	...
	$get_sql		= " SELECT * FROM tbl_form_jl_08_main where no_propinsi=".$SES_NO_PROP ;
	//print $get_sql;
	...
	//cek data di pusat
	$sql_cek="select * from tbl_form_jl_08_main where id_auto_daerah=".sqlvalue(@$arr["id_fjl_08_main"], true)." AND no_propinsi=".sqlvalue(@$arr["no_propinsi"], true)." and no_kabupaten=".sqlvalue(@$arr["no_kabupaten"], true);
	$rs_cari =   	$tb_import->Execute($sql_cek); 
                    $id_auto_daerah = $rs_cari->fields['id_auto_daerah'];
					$id_k_01_main_cek = $rs_cari->fields['id_fjl_08_main'];
					
					if ($id_auto_daerah !='') { //data sudah ada di pusat
	// script update DI PUSAT
					$tmp_sql = " UPDATE tbl_form_jl_08_main SET tanggal='".$arr[tanggal]."', ";
					$tmp_aql.= " no_propinsi ='".$arr[no_propinsi]."',"; 
					$tmp_sql.= " no_kabupaten ='".$arr[no_kabupaten]."',";
					$tmp_sql.= " triwulan ='".$arr[triwulan]."',";
					$tmp_sql.= " status_progress ='".$arr[status_progress]."',";
					$tmp_sql.= " catatan ='".$arr[catatan]."',";
					$tmp_sql.= " id_jns_jln ='".$arr[id_jns_jln]."'";
					$tmp_sql.= " WHERE id_auto_daerah='".$arr[id_fjl_08_main]."'";
					...
					//simpan di tabel asli server
							$sql_insert = " INSERT INTO `".$tbl_name_main."` (`tanggal`, `no_propinsi`, `no_kabupaten`, `triwulan`, `status_progress`, `catatan`, `id_jns_jln`, id_auto_daerah) 
							VALUES (".sqlvalue(@$arr["tanggal"], true).", 
									...
									".sqlvalue(@$arr["id_fjl_08_main"], false) .")";
									$sql_exec = $tb_import->Execute($sql_insert);

					}
					//cari detail di local atw daerah
					  $sql_detail	    =  " SELECT * FROM tbl_form_jl_08_detail WHERE id_fjl_08_main='".$arr[id_fjl_08_main]."' "; //ambil di localhost
		...
		if($get_recordSet2->RecordCount()>0){//3
			...
			//SIMPAN DI TABEL ASLI SERVER
									$sql_insert2  = " update tbl_form_jl_08_detail set ";
									$sql_insert2 .= "  nama_paket='".sqlvalue(@$arr2[nama_paket], true)."',"; 
									$sql_insert2 .= "  lhr='".sqlvalue(@$arr2[lhr], true)."',"; 
									$sql_insert2 .= "  jalan_perkerasan='".sqlvalue(@$arr2[jalan_perkerasan], true)."',"; 
									$sql_insert2 .= "  jalan_bahu='".sqlvalue(@$arr2[jalan_bahu], true)."',"; 
									$sql_insert2 .= "  jalan_drainase='".sqlvalue(@$arr2[jalan_drainase], true)."',"; 
									$sql_insert2 .= "  jalan_trotoar='".sqlvalue(@$arr2[jalan_trotoar], true)."',"; 
									$sql_insert2 .= "  jalan_pengaman='".sqlvalue(@$arr2[jalan_pengaman], true)."',"; 
									$sql_insert2 .= "  jalan_talud='".sqlvalue(@$arr2[jalan_talud], true)."',"; 
									$sql_insert2 .= "  jalan_lain='".sqlvalue(@$arr2[jalan_lain], true)."',"; 
									$sql_insert2 .= "  keterangan='".sqlvalue(@$arr2[keterangan], true)."'";
									$sql_insert2 .= " WHERE id_auto_daerah='".$arr[id_fjl_08_main]."'";
									 
									$tmp_sqlresult = $tb_import->Execute($sql_insert2);
							} else { //jika data belum ada else 2
								$sql_insert2 =  " INSERT INTO `".$tbl_name_detail."` (`id_fjl_08_main`, `nama_paket`, `lhr`, `jalan_perkerasan`, `jalan_bahu`, 
		`jalan_drainase`, `jalan_trotoar`, `jalan_pengaman`, `jalan_talud`, `jalan_lain`, `keterangan`,id_auto_daerah) VALUES (
		(SELECT MAX(id_fjl_08_main) FROM ".$tbl_name_main."), 
		...
		" .sqlvalue(@$arr[id_fjl_08_main], true) .")";

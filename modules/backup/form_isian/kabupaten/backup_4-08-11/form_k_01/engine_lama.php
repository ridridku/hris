...
// $tb_import->debug = true;
 // $db->debug = true;
  ...
  if(!$conn)
	...
	$sql_ambil_data = " SELECT * FROM tbl_form_k_01_main  where no_propinsi=".$SES_NO_PROP; //localhost atw daerah
	...
	if (@$count!='') {
				...
				while ($arr=$get_recordSet->FetchRow()) { // loop data di daerah
					...
					$sql_cek="select * from tbl_form_k_01_main where id_auto_daerah=".sqlvalue(@$arr["id_k_01_main"], true)." AND no_propinsi=".sqlvalue(@$arr["no_propinsi"], true)." and no_kabupaten=".sqlvalue(@$arr["no_kabupaten"], true);
					$rs_cari =   $tb_import->Execute($sql_cek); 

                    $id_auto_daerah = $rs_cari->fields['id_auto_daerah'];
					$id_k_01_main_cek = $rs_cari->fields['id_k_01_main'];

					if ($id_auto_daerah !='') { //data sudah ada di pusat
						 
						// script update DI PUSAT
 
						$tmp_sql = " UPDATE tbl_form_k_01_main  SET tanggal='".$arr[id_k_01_main]."'," ;
						...
						$tmp_sql.= "  where id_k_01_main ='".$id_k_01_main_cek."' " ;
						$tmp_sqlresult = $tb_import->Execute($tmp_sql);	
						
						 $sql_del = "DELETE FROM tbl_form_k_01_detail WHERE id_form_k_01_main='$id_k_01_main_cek' ";
						 $tb_import->Execute($sql_del);

						 $sql_del = "DELETE FROM tbl_form_kondisi_k_01_detail WHERE id_form_k_01_main='$id_k_01_main_cek' ";
						 $tb_import->Execute($sql_del);
							...
							$sql_insert = " INSERT INTO tbl_form_k_01_main (`tanggal`, `tahun`, `no_propinsi`, `no_kabupaten`, `id_jns_jln` , id_auto_daerah ) 
							 VALUES (" .sqlvalue(@$arr["tanggal"], true) .", " .sqlvalue(@$arr["tahun"], true) .", 
							 " .sqlvalue(@$arr["no_propinsi"], false) .", " .sqlvalue(@$arr["no_kabupaten"], false) .", " .sqlvalue(@$arr["id_jns_jln"], false) ." , " .sqlvalue(@$arr["id_k_01_main"], false) ." )";
					...
					if($get_recordSet2->RecordCount()>0){
						while ($arr2=$get_recordSet2->FetchRow()) {
							...
							if ($id_auto_daerah !='') { //jika  data induk sudah ada 
									...
									`kondisi_permukaan`, `kondisi`, `hambatan_lalin`, `bln_th_perenc_akhir`, `tipe_thn_pekerj_akhir`, `kondisi_thn_pekerj_akhir`, 
									`kelas_renc_lalin`, `lhr_roda_4`, `lhr_ekivalen_roda_4`, `bln_thn_perubahan_data`) VALUES (  
									$id_k_01_main_cek ," .sqlvalue(@$arr2["no_ruas"], true) .", 
									...
									" .sqlvalue(@$arr2["t_awal_rs_jln_lintang"], true).", 
									" .sqlvalue(@$arr2["t_awal_rs_jln_bujur"], true).", 
									" .sqlvalue(@$arr2["t_akhir_rs_jln_lintang"], true).", 
									" .sqlvalue(@$arr2["t_akhir_rs_jln_bujur"], true).", 				
									" .sqlvalue(@$arr2["titik_awal_kegiatan"], true).", 
									" .sqlvalue(@$arr2["titik_akhir_kegiatan"], true).", 
									" .sqlvalue(@$arr2["t_awal_kegiatan_lintang"], true).", 
									" .sqlvalue(@$arr2["t_awal_kegiatan_bujur"], true).", 
									" .sqlvalue(@$arr2["t_akhir_kegiatan_lintang"], true).", 
									" .sqlvalue(@$arr2["t_akhir_kegiatan_bujur"], true).", 						
									...
									$sqlinsert3 = " INSERT INTO tbl_form_kondisi_k_01_detail (`id_form_k_01_main`, `no_ruas`, `kondisi_permukaan`) 
											VALUES ($arr[id_k_01_main]," .sqlvalue(@$arr2["no_ruas"], true) .", 
											" .covt(@$_POST["kondisi_permukaan"]) .")";
									...
									$sql_exec2 = $tb_import->Execute($sql_insert2);
									 ...
									 // SIMANPAN DI TABEL ASLI SERVER
									$sqlinsert3 = " INSERT INTO tbl_form_kondisi_k_01_detail (`id_form_k_01_main`, `no_ruas`, `kondisi_permukaan`) 
											VALUES ((SELECT MAX(id_k_01_main) FROM tbl_form_k_01_main), " .sqlvalue(@$arr2["no_ruas"], true) .", 
											" .covt(@$_POST["kondisi_permukaan"]) .")";
									 $sqlexec3 = $tb_import->Execute($sqlinsert3);

...
$sql .="WHERE id_form_ur_01_jp_main = '$id'";
...
$sql = "update `".$tbl_name_main."` set `tanggal`=" .sqlvalue(@$_POST["tanggal"], true) .", 
		...
		where " ."(`id_form_ur_01_jp_main`=" .sqlvalue(@$_POST["xid_form_ur_01_jp_main"], false) .")";
		
$sqlresult = $db->Execute($sql);

$sql = "update `".$tbl_name_detail."` set `kode_proyek`=" .sqlvalue(@$_POST["kode_proyek"], true) .", 
		`nama_proyek`=" .sqlvalue(@$_POST["nama_proyek"], true) .", `no_ruas`=" .sqlvalue(@$_POST["no_ruas"], true) .", 
		...
		`biaya_per_km`=" .sqlvalue(@$_POST["biaya_per_km"], false) .", 
		`npv_per_km`=" .sqlvalue(@$_POST["npv_per_km"], false) .", 
		`inpv_per_km`=" .sqlvalue(@$_POST["inpv_per_km"], false) .", 
		`irr`=" .sqlvalue(@$_POST["irr"], false) .", 
		`kelayakan`=" .sqlvalue(@$_POST["kelayakan"], false) .", 
		`keterangan`=" .sqlvalue(@$_POST["keterangan"], true) ." 
		where " ."(`id_form_ur_01_jp_detail`=" .sqlvalue(@$_POST["xid_form_ur_01_jp_detail"], false) .")";
...
$sql = "insert into `".$tbl_name_detail."` (`id_form_ur_01_jp_main`,`kode_proyek`, `nama_proyek`, `no_ruas`, `nama_ruas`, `panjang_jalan`, 
		`sasaran`, `kabupaten`, `panjang_proyek`, `status_awal_jalan`, `status_akhir_jalan`, `jenis_penanganan`, 
		`tipe_pkrsn`, `lebar_pkrsn`, `biaya_jalan`, `jumlah_jembatan`, `panjang_jembatan`, `biaya_jembatan`, 
		`total_biaya`, `pad1`, `du_dpp`, `pjp_dpp`, `jenis_loan_phln`, `jumlah_loan_phln`, `jenis_sumber_dana_lain`, 
		`jumlah_sumber_dana_lain`, `biaya_per_km`, `npv_per_km`, `inpv_per_km`, `irr`, `kelayakan`, `keterangan`) values (
		(SELECT MAX(id_form_ur_01_jp_main) FROM ".$tbl_name_main."), 
		...
		" .sqlvalue(@$_POST["biaya_per_km"], false) .", 
		" .sqlvalue(@$_POST["npv_per_km"], false) .", 
		" .sqlvalue(@$_POST["inpv_per_km"], false) .", 
		" .sqlvalue(@$_POST["irr"], false) .", 
		" .sqlvalue(@$_POST["kelayakan"], false) .", 
...
case 0:
	if (Privi($mod_id, $user_id, 'insert') != 'no')
	{
		...
		Header("Location:index.php?mod_id=$mod_id&no_propinsi=".$_POST[no_propinsi]."&no_kabupaten=".$_POST[no_kabupaten]."&search=1&limit=".$_POST[limit]."&SORT=".$_POST['SORT']."&page=".$_POST[page]."&Date_Year=".$_POST[Date_Year]);
	}
break;
case 1:
...
case 2:
	...
	{
		lockTables($tbl_name_main);
		delete_($_GET['id_form_ur_01_jp_main']);
		unlockTables($tbl_name_main);		

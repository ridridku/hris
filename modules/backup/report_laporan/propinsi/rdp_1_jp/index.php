...
require_once($DIR_HOME.'/laporan_excell/inc.rdp_01.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/report_laporan/propinsi/rdp_1_jp';
...
if ($_GET[get_tahun_proyek]==1) 
{
	...
	if($no_prop!='')
	{
		$sql_tahun = "SELECT DISTINCT (YEAR(tanggal)) as tahun FROM tbl_form_rd_01_jp_main WHERE no_propinsi='".$no_prop."' ORDER BY YEAR(tanggal) ASC";
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				while ($arr=$recordSet->FetchRow()) {
					$content_data .= print_content($arr[kode_proyek],$arr[nama_proyek],$arr[no_ruas],$arr[nama_ruas],$arr[panjang_jalan],$arr[sasaran],$arr[kabupaten],$arr[panjang_proyek],$arr[status_awal_jalan],$arr[status_akhir_jalan],$arr[jenis_penanganan],$arr[tipe_pkrsn],$arr[lebar_pkrsn],$arr[biaya_jalan],$arr[jumlah_jembatan],$arr[panjang_jembatan],$arr[biaya_jembatan],$arr[total_biaya],$arr[pad1],$arr[du_prop],$arr[pjp_prop],$arr[jumlah_loan_phln],$arr[jenis_loan_phln],$arr[jumlah_sumber_dana_lain],$arr[jenis_sumber_dana_lain],$arr[ur_01_jp_konsistensi],$arr[biaya_per_km_konsistensi],$arr[kelayakan_konsistensi],$arr[catatan_konsistensi],$arr[keterangan]);
				...
				$file= $DIR_HOME."/files/rdp_01_".$no_propinsi."_".$no_kabupaten."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/rdp_01_".$no_propinsi."_".$no_kabupaten."_".$tahun.".xls";
}
...
$smarty->assign ("TITLE", _TITLE_PRINT_RDP_1_JP);
$smarty->assign ("HEAD_TITLE", _HEAD_TITLE_PRINT_RDP_1_JP);
$smarty->assign ("TB_PROPINSI", _NAMA_PROP);
$smarty->assign ("TB_TAHUN", _TAHUN);
//$smarty->assign ("TB_KODE_PROYEK", _NO_URUT);
...
$smarty->assign ("LIST_ME", _LIST_PRINT_RDP_1_JP);

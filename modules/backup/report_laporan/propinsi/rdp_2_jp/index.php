...
require_once($DIR_HOME.'/laporan_excell/inc.rdp_02.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/report_laporan/propinsi/rdp_2_jp';
...
if ($_GET[get_tahun_proyek]==1) 
{
	...
	if($no_prop!='')
	{
		$sql_tahun = "SELECT DISTINCT (YEAR(tanggal)) as tahun FROM tbl_form_rd_02_jp_main WHERE no_propinsi='".$no_prop."' ORDER BY YEAR(tanggal) ASC";
...
if ($_GET[get_kode_proyek] == 1)
{
	$tahun_anggaran = $_GET[tahun];
			if($tahun_anggaran!=''){	
					$sql_kode_combo 	  = "SELECT DISTINCT(a.kode_proyek) as kode_proyek FROM ".$tbl_name_detail." a LEFT JOIN ".$tbl_name_main." b ON a.id_form_rd_02_jp_main=b.id_form_rd_02_jp_main where YEAR(b.tanggal)='$tahun_anggaran'";
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				while ($arr=$recordSet->FetchRow()) {
					$content_data .= print_content($arr[kode_proyek],$arr[jenis_penanganan],$arr[jumlah_proyek_per_paket],$arr[jumlah_kabupaten],$arr[total_panjang_ruas],$arr[panjang_proyek_jalan],$arr[biaya_jalan],$arr[jumlah_jembatan],$arr[panjang_jembatan],$arr[biaya_jembatan],$arr[biaya_umum],$arr[total_biaya],$arr[pad1],$arr[du_prop],$arr[pjp_prop],$arr[jumlah_loan_phln],$arr[jenis_loan_phln],$arr[jumlah_sumber_dana_lain],$arr[jenis_sumber_dana_lain],$arr[keterangan]);
				...
				$file= $DIR_HOME."/files/rdp_02_".$no_propinsi."_".$no_kabupaten."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/rdp_02_".$no_propinsi."_".$no_kabupaten."_".$tahun.".xls";
}
...
$smarty->assign ("TITLE", _TITLE_PRINT_RDP_2_JP);
$smarty->assign ("HEAD_TITLE", _HEAD_TITLE_PRINT_RDP_2_JP);
...
$smarty->assign ("LIST_ME", _LIST_PRINT_RDP_2_JP);

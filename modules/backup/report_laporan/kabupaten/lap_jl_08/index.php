...
require_once($DIR_HOME.'/laporan_excell/inc.jl_08.php');
...
$str_completer = "mod_id=".$mod_id."&limit=".$LIMIT."&SORT=".$SORT."&no_propinsi=".$no_propinsi."&no_kabupaten=".$no_kabupaten."&search=1&triwulan_search=".$triwulan_search."&Search_Year=".$Search_Year."&jns_jln=".$jns_jln;
$str_completer_ = "limit=".$LIMIT."&SORT=".$SORT."&page=".$page."&no_propinsi=".$no_propinsi."&no_kabupaten=".$no_kabupaten."&search=1&triwulan_search=".$triwulan_search."&Search_Year=".$Search_Year."&jns_jln=".$jns_jln;
	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				***/
				...
				if($no_propinsi!='' AND $no_kabupaten!='' AND $jns_jln!=''){
					$sql .= "AND b.no_propinsi='".$no_propinsi."' AND b.no_kabupaten='".$no_kabupaten."' AND b.triwulan='".$triwulan_search."' AND YEAR(b.tanggal)='".$Search_Year."' AND b.id_jns_jln='".$jns_jln."' ";
				}
				***/
				if($no_propinsi!='' AND $no_kabupaten!='' AND $Search_Year!='' AND $triwulan_search!='' AND $id_jns_jln==''){
					$sql .= "AND (b.no_propinsi='".$no_propinsi."' $f_no_propinsi) AND (b.no_kabupaten='".$no_kabupaten."' $f_no_kabupaten) AND b.triwulan='".$triwulan_search."' AND YEAR(b.tanggal)='".$Search_Year."' $f_id_jns_jln ";
				} else if ($no_propinsi!='' AND $no_kabupaten!='' AND $Search_Year!=''  AND $id_jns_jln!=''){
					$sql .= "AND (b.no_propinsi='".$no_propinsi."' $f_no_propinsi) AND (b.no_kabupaten='".$no_kabupaten."' $f_no_kabupaten) AND (b.id_jns_jln='".$id_jns_jln."' $f_id_jns_jln) AND b.triwulan='".$triwulan_search."' AND YEAR(b.tanggal)='".$Search_Year."' ";
				} else {
				...
				while ($arr=$recordSet->FetchRow()) {
					$status_progres = $daftar_status_progres[$arr[status_progress]];
					/*** Disabled on 07-09-2010
					$content_data .= print_content($z+1,$arr[nama_paket],$arr[lhr],$arr[jalan_perkerasan],$arr[jalan_bahu],$arr[jalan_drainase],$arr[jalan_trotoar],$arr[jalan_pengaman],$arr[jalan_talud],$arr[jalan_lain],$arr[jembatan_beton],$arr[jembatan_kayu],$arr[jembatan_oprit],$arr[jembatan_pengecatan],$arr[jembatan_lainnya],$arr[keterangan]);
					***/
					/*** Disabled on 14-09-2010
					$tgl = explode("-", $arr[status_progress], strlen($arr[status_progress]));
					$status_progres = !empty($tgl[2])?$tgl[2]."-".$tgl[1]."-".$tgl[0]:"";
					***/
					$content_data .= print_content($z+1,$arr[nama_paket],$arr[lhr],$arr[jalan_perkerasan],$arr[jalan_bahu],$arr[jalan_drainase],$arr[jalan_trotoar],$arr[jalan_pengaman],$arr[jalan_talud],$arr[jalan_lain],$arr[keterangan]);
				...
				$file= $DIR_HOME."/files/jl_08_".$no_propinsi."_".$no_kabupaten."_".$jns_jln."_".$Search_Year.".xls";
				...
				$content = print_header($nama_propinsi,$nama_kabupaten,$Search_Year,$triwulan_search,$status_progres,$jenis_jalan);
				$content .= $content_data;
				$content .= print_footer();
				
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/jl_08_".$no_propinsi."_".$no_kabupaten."_".$jns_jln."_".$Search_Year.".xls";
}
...
$smarty->assign ("TITLE", _PRINT_TITLE_JL_08_MAIN);
$smarty->assign ("HEAD_TITLE", _PRINT_HEAD_TITLE_JL_08_MAIN);
...
$smarty->assign ("LIST_ME", _PRINT_LIST_JL_08_MAIN);

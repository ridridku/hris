...
require_once($DIR_HOME.'/laporan_excell/inc.jl_10.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/report_laporan/kabupaten/lap_jl_10';

#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/report_laporan');
$FILE_JS  = $JS_MODUL.'/kabupaten/lap_jl_10';
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				***/
				...
				$data_tahun1_baik = array();
				$data_tahun1_sedang = array();
				$data_tahun1_rusak = array();
				$data_tahun1_rusak_berat = array();
				$data_tahun2_baik = array();
				$data_tahun2_sedang = array();
				$data_tahun2_rusak = array();
				$data_tahun2_rusak_berat = array();
																
				$z=0;
				$tahun1 = $recordSet->fields[tahun1];
				$tahun2 = $recordSet->fields[tahun2];
				
				while ($arr=$recordSet->FetchRow()) {
					$content_data .= print_content($z+1,$arr[no_kabupaten],$arr[no_ruas],$arr[nama_ruas],$arr[panjang_km],$arr[panjang_m],$arr[tahun1_baik],$arr[tahun1_sedang],$arr[tahun1_rusak],$arr[tahun1_rusak_berat],$arr[tahun2_baik],$arr[tahun2_sedang],$arr[tahun2_rusak],$arr[tahun2_rusak_berat],$arr[keterangan]);

					$jml_1 = $arr[tahun1_baik]+$arr[tahun1_sedang]+$arr[tahun1_rusak]+$arr[tahun1_rusak_berat];
					$b_per_baik_1 	= number_format((($arr[tahun1_baik]/$jml_1)*100),2,'.','');
					$b_per_sedang_1 = number_format((($arr[tahun1_sedang]/$jml_1)*100),2,'.','');
					$b_per_rr_1		= number_format((($arr[tahun1_rusak]/$jml_1)*100),2,'.','');
					$b_per_rb_1		= number_format((($arr[tahun1_rusak_berat]/$jml_1)*100),2,'.','');
					$jml_2 = $arr[tahun2_baik]+$arr[tahun2_sedang]+$arr[tahun2_rusak]+$arr[tahun2_rusak_berat];
					$b_per_baik_2 	= number_format((($arr[tahun2_baik]/$jml_1)*100),2,'.','');
					$b_per_sedang_2 = number_format((($arr[tahun2_sedang]/$jml_1)*100),2,'.','');
					$b_per_rr_2		= number_format((($arr[tahun2_rusak]/$jml_1)*100),2,'.','');
					$b_per_rb_2		= number_format((($arr[tahun2_rusak_berat]/$jml_1)*100),2,'.','');
					
					$panjang_km = number_format($arr[panjang_km],2,',','.');
					$panjang_m = number_format($arr[panjang_m],2,',','.');
					$tahun1_baik = number_format($arr[tahun1_baik],2,',','.');
					$tahun1_sedang = number_format($arr[tahun1_sedang],2,',','.');
					$tahun1_rusak = number_format($arr[tahun1_rusak],2,',','.');
					$tahun1_rusak_berat = number_format($arr[tahun1_rusak_berat],2,',','.');
					$tahun2_baik = number_format($arr[tahun2_baik],2,',','.');
					$tahun2_sedang = number_format($arr[tahun2_sedang],2,',','.');
					$tahun2_rusak = number_format($arr[tahun2_rusak],2,',','.');
					$tahun2_rusak_berat = number_format($arr[tahun2_rusak_berat],2,',','.');
																				
					array_push($data_tb, $arr);
					array_push($kondisi1_th1, $b_per_baik_1);
					array_push($kondisi2_th1, $b_per_sedang_1);
					array_push($kondisi3_th1, $b_per_rr_1);
					array_push($kondisi4_th1, $b_per_rb_1);
					array_push($kondisi1_th2, $b_per_baik_2);
					array_push($kondisi2_th2, $b_per_sedang_2);
					array_push($kondisi3_th2, $b_per_rr_2);
					array_push($kondisi4_th2, $b_per_rb_2);
					array_push($data_panjang_km, $panjang_km);
					array_push($data_panjang_m, $panjang_m);
					array_push($data_tahun1_baik, $tahun1_baik);
					array_push($data_tahun1_sedang, $tahun1_sedang);
					array_push($data_tahun1_rusak, $tahun1_rusak);
					array_push($data_tahun1_rusak_berat, $tahun1_rusak_berat);
					array_push($data_tahun2_baik, $tahun2_baik);
					array_push($data_tahun2_sedang, $tahun2_sedang);
					array_push($data_tahun2_rusak, $tahun2_rusak);
					array_push($data_tahun2_rusak_berat, $tahun2_rusak_berat);
				...
				$file= $DIR_HOME."/files/jl_10_".$no_propinsi."_".$no_kabupaten."_".$jns_jln."_".$Search_Year.".xls";
				...
				$content = print_header($nama_propinsi,$nama_kabupaten,$Search_Year,$tahun1,$tahun2,$jenis_jalan);
				$content .= $content_data;
				$content .= print_footer();
				
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/jl_10_".$no_propinsi."_".$no_kabupaten."_".$jns_jln."_".$Search_Year.".xls";
...
$smarty->assign ("TB_TAHUN1", $tahun1);
$smarty->assign ("TB_TAHUN2", $tahun2);
...
$smarty->assign ("KONDISI_BAIK", $kondisi1_th1);
$smarty->assign ("KONDISI_SEDANG", $kondisi2_th1);
$smarty->assign ("KONDISI_RUSAK_RINGAN", $kondisi3_th1);
$smarty->assign ("KONDISI_RUSAK_BERAT", $kondisi4_th1);
$smarty->assign ("KONDISI_BAIK_TH2", $kondisi1_th2);
$smarty->assign ("KONDISI_SEDANG_TH2", $kondisi2_th2);
$smarty->assign ("KONDISI_RUSAK_RINGAN_TH2", $kondisi3_th2);
$smarty->assign ("KONDISI_RUSAK_BERAT_TH2", $kondisi4_th2);
$smarty->assign ("DATA_PANJANG_KM",$data_panjang_km);
$smarty->assign ("DATA_PANJANG_M",$data_panjang_m);
$smarty->assign ("DATA_TAHUN1_BAIK",$data_tahun1_baik);
$smarty->assign ("DATA_TAHUN1_SEDANG",$data_tahun1_sedang);
$smarty->assign ("DATA_TAHUN1_RUSAK",$data_tahun1_rusak);
$smarty->assign ("DATA_TAHUN1_RUSAK_BERAT",$data_tahun1_rusak_berat);
$smarty->assign ("DATA_TAHUN2_BAIK",$data_tahun2_baik);
$smarty->assign ("DATA_TAHUN2_SEDANG",$data_tahun2_sedang);
$smarty->assign ("DATA_TAHUN2_RUSAK",$data_tahun2_rusak);
$smarty->assign ("DATA_TAHUN2_RUSAK_BERAT",$data_tahun2_rusak_berat);					

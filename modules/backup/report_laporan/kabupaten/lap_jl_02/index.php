...
require_once($DIR_HOME.'/laporan_excell/inc.jl_02.php');
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				***/
				...
				$data_sedang = array();
				$data_rusak = array();
				$data_rusak_berat = array();
				$data_jumlah_penduduk = array();
				
				$z=0;

				while ($arr=$recordSet->FetchRow()) {
					
					$content_data .= print_content($z+1,$arr[no_kabupaten],$arr[no_ruas],$arr[nama_ruas],$arr[panjang_km],$arr[panjang_m],$arr[kondisi_baik],$arr[kondisi_sedang],$arr[kondisi_rusak],$arr[kondisi_rusak_berat],$arr[lhr_rata_rata],$arr[akses_jalan],$arr[jumlah_penduduk],$arr[keterangan]);
					
					$jml_1 = $arr[kondisi_baik]+$arr[kondisi_sedang]+$arr[kondisi_rusak]+$arr[kondisi_rusak_berat];
					$b_per_baik_1 = number_format((($arr['kondisi_baik']/$jml_1)*100),2,',','.');
					$b_per_sedang_1 = number_format((($arr['kondisi_sedang']/$jml_1)*100),2,',','.');
					$b_per_rr_1		= number_format((($arr['kondisi_rusak']/$jml_1)*100),2,',','.');
					$b_per_rb_1		= number_format((($arr['kondisi_rusak_berat']/$jml_1)*100),2,',','.');
					$panjang_km = number_format($arr[panjang_km],2,',','.');
					$panjang_m =  number_format($arr[panjang_m],2,',','.');
					$baik = number_format($arr[kondisi_baik],2,',','.'); 
					$sedang = number_format($arr[kondisi_sedang],2,',','.');
					$rusak = number_format($arr[kondisi_rusak],2,',','.');
					$rusak_berat = number_format($arr[kondisi_rusak_berat],2,',','.');
					$jumlah_penduduk = number_format($arr[jumlah_penduduk],0,'','.');
					
					array_push($data_tb, $arr);
					array_push($kondisi1, $b_per_baik_1);
					array_push($kondisi2, $b_per_sedang_1);
					array_push($kondisi3, $b_per_rr_1);
					array_push($kondisi4, $b_per_rb_1);
					array_push($data_panjang_km, $panjang_km);
					array_push($data_panjang_m, $panjang_m);
					array_push($data_baik, $baik);
					array_push($data_sedang, $sedang);
					array_push($data_rusak, $rusak);
					array_push($data_rusak_berat, $rusak_berat);
					array_push($data_jumlah_penduduk, $jumlah_penduduk);
				...
				$file= $DIR_HOME."/files/jl_02_".$no_propinsi."_".$no_kabupaten."_".$jns_jln."_".$Search_Year.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/jl_02_".$no_propinsi."_".$no_kabupaten."_".$jns_jln."_".$Search_Year.".xls";
...
$smarty->assign ("TITLE", _PRINT_TITLE_JL_02_MAIN);
$smarty->assign ("HEAD_TITLE", _PRINT_HEAD_TITLE_JL_02_MAIN);
...
$smarty->assign ("LIST_ME", _PRINT_LIST_JL_02_MAIN);
...
$smarty->assign ("KONDISI_BAIK", $kondisi1);
$smarty->assign ("KONDISI_SEDANG", $kondisi2);
$smarty->assign ("KONDISI_RUSAK_RINGAN", $kondisi3);
$smarty->assign ("KONDISI_RUSAK_BERAT", $kondisi4);
$smarty->assign ("DATA_PANJANG_KM",$data_panjang_km);
$smarty->assign ("DATA_PANJANG_M",$data_panjang_m);
$smarty->assign ("DATA_BAIK",$data_baik);
$smarty->assign ("DATA_SEDANG",$data_sedang);
$smarty->assign ("DATA_RUSAK",$data_rusak);
$smarty->assign ("DATA_RUSAK_BERAT",$data_rusak_berat);
$smarty->assign ("DATA_JUMLAH_PENDUDUK",$data_jumlah_penduduk);

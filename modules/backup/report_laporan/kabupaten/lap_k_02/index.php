...
require_once($DIR_HOME.'/laporan_excell/inc.k_02.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/report_laporan/kabupaten/lap_k_02';


#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/report_laporan');
$FILE_JS  = $JS_MODUL.'/kabupaten/lap_k_02';
	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				***/
				...
				if($no_propinsi!='' AND $no_kabupaten!='' AND $Search_Year!='' AND $jns_jln==''){
					$sql .= "AND (b.no_propinsi='".$no_propinsi."' $f_no_propinsi) AND (b.no_kabupaten='".$no_kabupaten."' $f_no_kabupaten) AND YEAR(b.tanggal_usulan)='".$Search_Year."' $f_id_jns_jln ";
				} else if ($no_propinsi!='' AND $no_kabupaten!='' AND $Search_Year!=''  AND $jns_jln!=''){
					$sql .= "AND (b.no_propinsi='".$no_propinsi."' $f_no_propinsi) AND (b.no_kabupaten='".$no_kabupaten."' $f_no_kabupaten) AND (a.id_jns_jln='".$jns_jln."' $f_id_jns_jln) AND YEAR(b.tanggal_usulan)='".$Search_Year."' ";
				} else {
				...
				while ($arr=$recordSet->FetchRow()) {
					...
					$diisi_oleh = $arr[nama_penggisi_usulan];
					if ($arr[kelompok_prioritas]=="A") {
					$content_data .= print_content($arr[no_ruas],$arr[nama_ruas],$arr[panjang],$arr[kondisi],$arr[lebar],$arr[lhr_roda_4],$arr[kota_aktivitas_dilayani]);
					} elseif ($arr[kelompok_prioritas]=="B") {
					$content_data_2 .= print_content($arr[no_ruas],$arr[nama_ruas],$arr[panjang],$arr[kondisi],$arr[lebar],$arr[lhr_roda_4],$arr[kota_aktivitas_dilayani]);
					}
				...
				$file= $DIR_HOME."/files/k_02_".$no_propinsi."_".$no_kabupaten."_".$jns_jln."_".$Search_Year.".xls";
				...
				$content = print_header($nama_propinsi,$nama_kabupaten,$diisi_oleh,$Search_Year,$jenis_jalan);
				$content .= $content_data.print_content_2().$content_data_2;
				$content .= print_footer();
				
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/k_02_".$no_propinsi."_".$no_kabupaten."_".$jns_jln."_".$Search_Year.".xls";
...
$smarty->assign ("TITLE", _PRINT_TITLE_K_02_MAIN);
$smarty->assign ("HEAD_TITLE", _PRINT_HEAD_TITLE_K_02_MAIN);
...
$smarty->assign ("LIST_ME", _PRINT_LIST_K_02_MAIN);

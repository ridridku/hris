...
require_once($DIR_HOME.'/laporan_excell/inc.jl_07.php');
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				***/
				...
				$sql = "SELECT *,c.nama_paket as nama_paket 
				...
				if($no_propinsi!='' AND $no_kabupaten!='' AND $jns_jln!=''){
					$sql .= "AND b.no_propinsi='".$no_propinsi."' AND b.no_kabupaten='".$no_kabupaten."' AND b.triwulan='".$triwulan_search."' AND b.tahun='".$Search_Year."' AND a.id_jns_jln='".$jns_jln."' ";
				}
				***/
				if($no_propinsi!='' AND $no_kabupaten!='' AND $Search_Year!='' AND $triwulan_search!='' AND $jns_jln==''){
					$sql .= "AND (b.no_propinsi='".$no_propinsi."' $f_no_propinsi) AND (b.no_kabupaten='".$no_kabupaten."' $f_no_kabupaten) AND b.tahun='".$Search_Year."' AND b.triwulan='".$triwulan_search."' $f_id_jns_jln ";
				} else if ($no_propinsi!='' AND $no_kabupaten!='' AND $Search_Year!='' AND $triwulan_search!=''  AND $jns_jln!=''){
					$sql .= "AND (b.no_propinsi='".$no_propinsi."' $f_no_propinsi) AND (b.no_kabupaten='".$no_kabupaten."' $f_no_kabupaten) AND (a.id_jns_jln='".$jns_jln."' $f_id_jns_jln) AND b.tahun='".$Search_Year."' AND b.triwulan='".$triwulan_search."' ";
				} else {
				...
				while ($arr=$recordSet->FetchRow()) {
					$status_progres = $daftar_status_progres[$arr[status_progres]];
					/*** Disabled on 14-09-2010
					$tgl = explode("-", $arr[status_progres], strlen($arr[status_progres]));
					$status_progres = $tgl[2]."-".$tgl[1]."-".$tgl[0];
					***/					
					$content_data .= print_content($z+1,$arr[nama_paket],$arr[masalah],$arr[pemecahan],$arr[instansi],$arr[status_perkembangan],$arr[0]);
				...
				$file= $DIR_HOME."/files/jl_07_".$no_propinsi."_".$no_kabupaten."_".$jns_jln."_".$Search_Year.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/jl_07_".$no_propinsi."_".$no_kabupaten."_".$jns_jln."_".$Search_Year.".xls";
}
...
$smarty->assign ("TITLE", _PRINT_TITLE_JL_07_MAIN);
$smarty->assign ("HEAD_TITLE", _PRINT_HEAD_TITLE_JL_07_MAIN);
...
$smarty->assign ("LIST_ME", _PRINT_LIST_JL_07_MAIN);

...
require_once($DIR_HOME.'/laporan_excell/inc.jl_06.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/report_laporan/kabupaten/lap_jl_06';


#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/report_laporan');
$FILE_JS  = $JS_MODUL.'/kabupaten/lap_jl_06';
...
$str_completer = "mod_id=".$mod_id."&limit=".$LIMIT."&SORT=".$SORT."&no_propinsi=".$no_propinsi."&no_kabupaten=".$no_kabupaten."&search=1&Search_Year=".$Search_Year."&jns_jln=".$jns_jln."&triwulan_search=".triwulan_search;
$str_completer_ = "limit=".$LIMIT."&SORT=".$SORT."&page=".$page."&no_propinsi=".$no_propinsi."&no_kabupaten=".$no_kabupaten."&search=1&Search_Year=".$Search_Year."&jns_jln=".$jns_jln."&triwulan_search=".triwulan_search;
	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				***/
				if($no_propinsi!='' AND $no_kabupaten!='' AND $Search_Year!='' AND $jns_jln=='') {
					$sql .= "AND (b.no_propinsi='".$no_propinsi."' $f_no_propinsi) AND (b.no_kabupaten='".$no_kabupaten."' $f_no_kabupaten) AND YEAR(b.tanggal)='".$Search_Year."' $f_id_jns_jln AND b.triwulan='".$triwulan_search."' ";
				} else if ($no_propinsi!='' AND $no_kabupaten!='' AND $Search_Year!=''  AND $jns_jln!='') {
					$sql .= "AND (b.no_propinsi='".$no_propinsi."' $f_no_propinsi) AND (b.no_kabupaten='".$no_kabupaten."' $f_no_kabupaten) AND (b.id_jns_jln='".$jns_jln."' $f_id_jns_jln) AND YEAR(b.tanggal)='".$Search_Year."' AND b.triwulan='".$triwulan_search."' ";
				} else {
				...
				while ($arr=$recordSet->FetchRow()) {
					...
					$content_data .= print_content($z+1,$arr[nama_paket],$arr[target_km],$arr[target_m],$arr[biaya],$arr[rencana_fisik],$arr[rencana_keuangan],$arr[realisasi_fisik],$arr[realisasi_keuangan],$arr[deviasi_fisik],$arr[deviasi_keuangan],$arr[masalah],$arr[jumlah_tenaga],$arr[keterangan]);
				...
				$file= $DIR_HOME."/files/jl_06_".$no_propinsi."_".$no_kabupaten."_".$jns_jln."_".$Search_Year.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/jl_06_".$no_propinsi."_".$no_kabupaten."_".$jns_jln."_".$Search_Year.".xls";				
}
...
$smarty->assign ("TITLE", _PRINT_TITLE_JL_06_MAIN);
$smarty->assign ("HEAD_TITLE", _PRINT_HEAD_TITLE_JL_06_MAIN);
...
$smarty->assign ("LIST_ME", _PRINT_LIST_JL_06_MAIN);

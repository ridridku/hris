...
$smarty->assign ("HREF_HOME_PATH_AJAX", $HREF_HOME.'/modules/master/jabatan');
...
FROM
  r_jabatan 
  INNER JOIN r_level ON r_jabatan.r_jabatan__level=r_level.r_level__id GROUP BY r_jabatan__level ORDER BY r_level__id ;";
...
if($opt=="1"){
	...
	$sql_ = "SELECT
                ...
                FROM
                  r_jabatan 
                  INNER JOIN r_level ON r_jabatan.r_jabatan__level=r_level.r_level__id WHERE r_jabatan__id='".$_GET['id']."' ";
       
 $resultSet = $db->Execute($sql_);
$edit_r_jabatan__id = $resultSet->fields[r_jabatan__id];
$edit_r_jabatan__ket=$resultSet->fields[r_jabatan__ket];
$edit_r_jabatan__level=$resultSet->fields[r_jabatan__level];
$edit_r_level__jabatan=$resultSet->fields[r_level__jabatan];
...
$smarty->assign ("EDIT_JABATAN__ID",$edit_r_jabatan__id);
$smarty->assign ("EDIT_JABATAN__KET",$edit_r_jabatan__ket);
$smarty->assign ("EDIT_JABATAN__LEVEL",$edit_r_jabatan__level);
$smarty->assign ("EDIT_LEVEL__JABATAN",$edit_r_level__jabatan);
$smarty->assign ("EDIT_LEVEL__ID",$edit_r_level__id);
...
if ($_GET['search'] == '1')
	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
		  ...
		  if($jenis_user=='2'){
                                  ...
                                  $sql  = "SELECT
                                            r_jabatan__id as r_jabatan__id,
                                            UPPER(r_jabatan__ket) as r_jabatan__ket,
                                            r_jabatan__level AS r_jabatan__level,
                                            UPPER(r_level.r_level__ket) AS r_level__ket,
                                            r_level.r_level__jabatan AS r_level__jabatan,
                                            r_level.r_level__id AS r_level__id
                                            FROM
                                            r_jabatan 
                                            INNER JOIN r_level ON r_jabatan.r_jabatan__level=r_level.r_level__id where 1=1";
                                ...
                                if($level_cari!=''){
                                        ...
                                        $sql .= " AND r_jabatan__level = '".addslashes($level_cari)."'";
				}
                                if($jabatan_cari!=''){

                                        $sql .= " AND r_jabatan__ket  LIKE '%".$jabatan_cari."%'  ";
				}
                              

			 	$sql .= " ORDER BY  trim(r_jabatan__level) asc ";
				...
				$sql  = "SELECT
                            ...
                            if($level_cari!=''){
                             ...
                             if($jabatan_cari!=''){
                                       ...
                                       $sql .= " AND r_jabatan__ket  LIKE = '%".addslashes($jabatan_cari)."'%";
				}

	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
		  ...
		  if($jenis_user=='2'){
                        $sql  = "SELECT A.* FROM (SELECT 
                                ('$periode_awal')as t_rkp__awal,
                                ('$periode_akhir') as t_rkp__akhir,
                                (v_peg.mutasi)as t_rkp__no_mutasi,
                                IF((SELECT  r_resign__mutasi FROM r_resign WHERE r_resign.r_resign__mutasi=v_peg.mutasi)is null,'0','1')as ket_keluar,
                                ...
                                (v_peg.JML_HARI-v_peg.JML_LBR)+1 as mx_day,v_peg.*
                                FROM (
                                ...
                                where t_libur.r_libur__tgl>='$periode_awal' and t_libur.r_libur__tgl<='$periode_akhir'
                                GROUP BY show_peg.r_pnpt__id_pegawai)v_peg
                                LEFT JOIN t_rekap_absensi ON t_rekap_absensi.t_rkp__no_mutasi=v_peg.mutasi)A
                                WHERE A.t_rkp__awal>='$periode_awal' and A.t_rkp__akhir<='$periode_akhir' AND A.r_cabang__id='$kode_pw_ses'  
                                GROUP BY A.r_pnpt__id_pegawai";
				...
				$sql  = "SELECT A.* FROM (SELECT 
                                ...
                                GROUP BY show_peg.r_pnpt__id_pegawai)v_peg
                                LEFT JOIN t_rekap_absensi ON t_rekap_absensi.t_rkp__no_mutasi=v_peg.mutasi)A
                                WHERE A.t_rkp__awal>='$periode_awal' and A.t_rkp__akhir<='$periode_akhir' AND 1=1'  
                                GROUP BY A.r_pnpt__id_pegawai";
...
/*
				...
				if($kode_sumber!=''){
			 	...
			 	//$sql .= "ORDER BY D.r_pegawai__id ASC ";

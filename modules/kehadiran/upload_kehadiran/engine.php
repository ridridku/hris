...
//                        $t_abs__jam_keluar= $data->val($i,5);
                         $t_abs__jam_masuk = substr_replace(preg_replace("/[^0-9,.]/", ":", $data->val($i, 4)),'',8);
                          $t_abs__jam_keluar=substr_replace(preg_replace("/[^0-9,.]/", ":", $data->val($i, 5)),'',8);
                 ...
                 $CekFingerPrint="SELECT
                    ...
                    A.r_shift__id AS r_shift__id,r_shift__terlambat
                    FROM
                    r_shift A
            ...
            IF($rule_jam_total!='' )
           {
            ...
            else
            {
                ...
                $time_keluar =5;
                        ...
                        IF(($total_jam_kerja2<'03:00:00'))
                           {$status='0';}else{$status='1';}

                        //terlambat xls   
                          IF($t_abs__jam_masuk>$rule_jam_masuk AND  $t_abs__jam_masuk!='' && $t_abs__jam_keluar!='' ) 
                           { 
                          ...
                          IF($t_abs__jam_masuk<$rule_jam_masuk && $t_abs__jam_masuk!='' && $t_abs__jam_keluar!='') 
                           {
                            ...
                            else
                            {
                             ...
                             // LESSTIME
                                $rule_masuk = strtotime("$rule_jam_total");
...
elseif($tipe_file==2 and $file_ext=='dat') //upload_secure_data 
  {
        ...
        $sql_cek="SELECT A.t_abs__fingerprint AS t_abs__fingerprint,"
            ...
            $worktime_cek=$rs_val->fields['worktime'];
               ...
               $sql_overday_cek="SELECT A.t_abs__fingerprint AS t_abs__fingerprint,"
            . " A.t_abs__tgl AS t_abs__tgl,"
            . " A.t_abs__jam_masuk AS t_abs__jam_masuk,"
            . " A.t_abs__jam_keluar AS t_abs__jam_keluar, "
            . " A.t_abs__status AS t_abs__status,"
            ...
            . " where A.t_abs__fingerprint='$finger' "
            . " AND A.t_abs__tgl>='$periode_awal' AND A.t_abs__tgl <= '$periode_akhir'"
            . " ORDER BY t_abs__date_updated DESC limit 1  ";

            $rs_val = $db->Execute($sql_overday_cek);
            $overday_cek_finger= $rs_val->fields['t_abs__fingerprint'];
            $overday_cek_tgl= $rs_val->fields['t_abs__tgl'];
            $overday_cek_masuk=$rs_val->fields['t_abs__jam_masuk'];
            $overday_cek_keluar=$rs_val->fields['t_abs__jam_keluar'];
            
          
$date__yesterday = $overday_cek_tgl;
$date_over_aday = str_replace('-', '/', $date__yesterday);
$tomorrow = date('Y-m-d',strtotime($date_over_aday  . "+1 days"));



          
       
   if (($date>=$periode_awal and $date<=$periode_akhir) AND ($tgl_cek=='')AND ($time<=$rule_jam_masuk)&&($rule_shift<6))  
    {
    ...
    elseif(($date>=$periode_awal and $date<=$periode_akhir) AND ($tgl_cek=='') AND ($time>=$rule_jam_masuk)&&($rule_shift<6))
    {
     ...
     elseif(($date>=$periode_awal and $date<=$periode_akhir) AND ($tgl_cek=='') AND ($time>=$rule_jam_masuk)&&($rule_shift==6) &&($time>='15:00:00' and $time<='20:00:00'))
    {
    ...
    elseif($finger_cek!='' && $tgl_cek!='' && $rule_shift<6 )
      {
    ...
    elseif ($overday_cek_finger!='' && $date==$tomorrow && $time<'09:00:00' and $time>='04:00:00' ) 
        {
     
            
              
                 //EARLY
                IF($overday_cek_masuk<$rule_jam_masuk)
                    {
                          
                        $t3=$rule_jam_masuk;
                        $t4=$overday_cek_masuk;
                        $a1 = explode(":",$t3);
                        $a2 = explode(":",$t4);
                        $time1 = (($a1[0]*60*60)+($a1[1]*60)+($a1[2]));
                        $time2 = (($a2[0]*60*60)+($a2[1]*60)+($a2[2]));
                    ...
                    //TERLAMBAT LATELY
                        ...
                        //worktime total kerja
                                    IF($time!='' && $overday_cek_masuk)
                                       {
                                                  $t1=$overday_cek_masuk;
                                                  $t2=$time;
                                                 
                                        $datetime1=$overday_cek_tgl.' '.$overday_cek_masuk;     
                                        $date1 = new DateTime($datetime1);
                                        $result_tgl1 = $date1->format('Y-m-d H:i:s');
                                        ...
                                        $result_tgl2 = $date2->format('Y-m-d H:i:s');
                                         ...
                                         $total_kerja = datediff($result_tgl1, $result_tgl2); 
                        ...
                        $sql_edit  ="UPDATE $tbl_name";
                        $sql_edit .=" SET t_abs__fingerprint ='$finger', ";
                        $sql_edit .= " t_abs__tgl ='$overday_cek_tgl',";
                        $sql_edit .= " t_abs__id_shift = '$rule_shift',";
                        $sql_edit .= " t_abs__jam_masuk = '$overday_cek_masuk',";
                        $sql_edit .= " t_abs__jam_keluar = '$time',";
                        $sql_edit .= " t_abs__early = '$early_time',";
                        $sql_edit .= " t_abs__lately = '$keterlambatan',";
                        $sql_edit .= " t_abs__approval = '',";
                        ...
                        $sql_edit .= " t_abs__user_created = '$id_peg',";
                        $sql_edit .= " t_abs__user_updated = '$id_peg'";
                        $sql_edit .="  WHERE  t_abs__fingerprint='$finger' AND t_abs__tgl ='$overday_cek_tgl'";
         ...
         elseif($status==0 && $worktime_cek<0)
            {

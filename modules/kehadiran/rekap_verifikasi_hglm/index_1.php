...
$smarty->assign ("HREF_HOME_PATH_AJAX", $HREF_HOME.'/modules/kehadiran/rekap_verifikasi_hrd');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/kehadiran/rekap_verifikasi_hrd/';


#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/kehadiran');
$FILE_JS  = $JS_MODUL.'/rekap_verifikasi_hrd';
...
if ($_GET['search'] == '1')
	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
		  ...
		  if($jenis_user=='2'){
                    $sql  = "SELECT
                                    IF((t_rkp__no_mutasi)is null,'0',t_rkp__no_mutasi)as t_rkp__no_mutasi,
                                    IF((t_rkp__awal)is null,'0',t_rkp__awal)as t_rkp__awal,
                                    IF((t_rkp__akhir)is null,'0',t_rkp__akhir)as t_rkp__akhir,
                                    IF((t_rkp__approval)is null,'0',t_rkp__approval)as t_rkp__approval,
                                    IF((t_rkp__hadir)is null,'0',t_rkp__hadir)as t_rkp__hadir,
                                    IF((t_rkp__sakit)is null,'0',t_rkp__sakit)as t_rkp__sakit,
                                    IF((t_rkp__izin)is null,'0',t_rkp__izin)as t_rkp__izin,
                                    IF((t_rkp__alpa)is null,'0',t_rkp__alpa)as t_rkp__alpa,
                                    IF((t_rkp__dinas)is null,'0',t_rkp__dinas)as t_rkp__dinas,
                                    IF((t_rkp__cuti)is null,'0',t_rkp__cuti)as t_rkp__cuti,
                                    IF((t_rkp__keterangan)is null,'',t_rkp__keterangan)as t_rkp__keterangan,
                                    r_pegawai__nama,
                                    r_cabang__nama,
                                    r_cabang__id,
                                    r_subcab__id,
                                    ...
                                    r_dept__id,
                                    r_dept__ket,
                                    r_pnpt__finger_print FROM 
                                    (SELECT * FROm t_rekap_absensi WHERE  t_rekap_absensi.t_rkp__awal>='$periode_awal' AND t_rekap_absensi.t_rkp__akhir<='$periode_akhir'
                                    )rkp
                                    RIGHT  JOIN v_pegawai peg ON peg.r_pnpt__no_mutasi=rkp.t_rkp__no_mutasi 
                                    WHERE  r_cabang__id='$kode_pw_ses' and r_pnpt__aktif=1 AND r_pnpt__no_mutasi NOT IN 
                                    (select DISTINCT r_resign__mutasi FROM r_resign 
                                    WHERE r_resign.r_resign__tgl <= '$periode_akhir'- INTERVAL 10 DAY and r_resign.r_resign__approval=1)";
                        ...
                        $sql  = "SELECT
                                    ...
                                    RIGHT  JOIN v_pegawai peg ON peg.r_pnpt__no_mutasi=rkp.t_rkp__no_mutasi 
                                    WHERE  1=1 and r_pnpt__aktif=1 AND r_pnpt__no_mutasi NOT IN 
                                    (select DISTINCT r_resign__mutasi FROM r_resign 
                                    WHERE r_resign.r_resign__tgl <= '$periode_akhir'- INTERVAL 10 DAY and r_resign.r_resign__approval=1)";	
				...
				if($kode_subcab_cari!=''){
					$sql .= "    AND r_subcab__id  = '".$kode_subcab_cari."' ";
                                ...
                                if($id_finger_cari!=''){
					$sql .= "    AND r_pnpt__finger_print  = '".$id_finger_cari."' ";
				} 
                                if ($bulan !='') {
                                       $sql.=" and t_rkp__awal='$bulan'  ";
                                }

                                 if ($tahun !='') {
                                       $sql.=" AND t_rkp__akhir='$tahun'  ";
                                }

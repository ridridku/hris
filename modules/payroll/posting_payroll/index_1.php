...
$sql_2="SELECT
    IF(D.r_level__id<9,'1','2')status_staf,
D.hadir as absen_hadir,
D.gapok AS gapok,
D.dasar_bpjs AS dasar_bpjs,
D.gaji_makan AS tunj_makan,
D.gaji_transport AS tunj_trans,
D.tunj_jabatan as tunj_jabatan,
D.tunj_kemahalan AS tunj_kemahalan,
D.tunj_lain AS  tunj_lain,
D.kosan AS  tunj_kost,
D.angsuran AS angsuran,
D.potongan_bpjs,
ROUND(D.gapok+D.gaji_makan+D.gaji_transport+D.tunj_jabatan+D.tunj_kemahalan+D.tunj_lain+D.kosan) as gaji_gross,
ROUND((D.gapok+D.gaji_makan+D.gaji_transport+D.tunj_jabatan+D.tunj_kemahalan+D.tunj_lain+D.kosan)-(D.angsuran+D.potongan_bpjs)) as gaji_netto,
D.*

FROM(SELECT 
IF((C.r_gapok__nilai)is null,'0',C.r_gapok__nilai)as gapok,
IF((C.r_gapok__dasar_bpjs)is null,'0',C.r_gapok__dasar_bpjs)as dasar_bpjs,
IF((C.r_gapok__jabatan)is null,'0',C.r_gapok__jabatan)as tunj_jabatan,
IF((C.r_gapok__kemahalan)is null,'0',C.r_gapok__kemahalan)as tunj_kemahalan,
IF((C.r_gapok__lain1)is null,'0',C.r_gapok__lain1)as tunj_lain,
IF((C.r_gapok__kosan)is null,'0',C.r_gapok__kosan)as kosan,
IF(ROUND(C.r_gapok__makan*(C.hadir/C.max_hari)) IS NULL,'0',ROUND(C.r_gapok__makan*(C.hadir/C.max_hari))) AS gaji_makan,
IF(ROUND(C.r_gapok__transport*(C.hadir/max_hari))IS NULL,'0',ROUND(C.r_gapok__transport*(C.hadir/max_hari))) AS gaji_transport,
IF(ROUND(C.r_gapok__dasar_bpjs * 0.0424	)IS NULL,'0',(ROUND(C.r_gapok__dasar_bpjs * 0.0424	)))AS bpjs_tk_tmw,
IF(ROUND(C.r_gapok__dasar_bpjs * 0.02)IS NULL,'0',ROUND(C.r_gapok__dasar_bpjs * 0.02)) AS bpjs_tk_peg,
IF(ROUND(C.r_gapok__dasar_bpjs * 0.040)IS NULL,'0',ROUND(C.r_gapok__dasar_bpjs * 0.040	)) AS bpjs_kes_tmw,
IF(ROUND(C.r_gapok__dasar_bpjs * 0.010)IS NULL,'0',ROUND(C.r_gapok__dasar_bpjs * 0.010	)) AS bpjs_kes_peg,
IF((ROUND(C.r_gapok__dasar_bpjs * 0.02)+ROUND(C.r_gapok__dasar_bpjs * 0.010))IS NULL,'0',(ROUND(C.r_gapok__dasar_bpjs * 0.02)+ROUND(C.r_gapok__dasar_bpjs * 0.010)))as potongan_bpjs,
IF((SELECT SUM(t_ang__nilai_angsuran) as angsuran from t_angsuran_pinjaman
WHERE t_angsuran_pinjaman.t_ang__idkaryawan=C.peg_id 
...
CASE C.r_pnpt__no_mutasi WHEN (SELECT r_resign.r_resign__mutasi FROM r_resign WHERE r_resign.r_resign__mutasi = C.r_pnpt__no_mutasi AND r_resign__approval = 1
) THEN '1' ELSE '0' END ket_resign,C.*

FROM

(SELECT B.*,r_gapok.*
 FROM (SELECT A.r_pegawai__id as peg_id,sum(t_rekap_absensi.t_rkp__hadir) as hadir,
CASE r_cabang.r_cabang__id WHEN r_cabang__id = 1 THEN '20' ELSE '25' END max_hari,
 r_cost_center.*,r_departement.*,r_subdepartement.*,r_cabang.*,t_rekap_absensi.*,r_level.*,r_jabatan.*, r_subcabang.*,A.* FROM (SELECT r_penempatan.*,r_pegawai.*,
TIMESTAMPDIFF(MONTH,r_pegawai__tgl_masuk,DATE_FORMAT(NOW(), '%Y-%m-%d')) AS lama_bln
FROM r_penempatan INNER JOIN r_pegawai ON r_pegawai.r_pegawai__id = r_penempatan.r_pnpt__id_pegawai ORDER BY r_penempatan.r_pnpt__date_updated DESC) A
INNER JOIN r_subcabang ON r_subcabang.r_subcab__id = A.r_pnpt__subcab
INNER JOIN r_jabatan ON r_jabatan.r_jabatan__id = A.r_pnpt__jabatan
INNER JOIN r_level ON r_jabatan.r_jabatan__level = r_level.r_level__id
INNER JOIN t_rekap_absensi ON t_rekap_absensi.t_rkp__no_mutasi = A.r_pnpt__no_mutasi
INNER JOIN r_cabang ON r_cabang.r_cabang__id = r_subcabang.r_subcab__cabang
INNER JOIN r_subdepartement ON r_subdepartement.r_subdept__id = A.r_pnpt__subdept
...
WHERE t_rekap_absensi.t_rkp__awal='$pre_active' AND t_rekap_absensi.t_rkp__akhir='$end_active' 
GROUP BY A.r_pegawai__id)B LEFT join r_gapok on r_gapok.r_gapok__nip=B.r_pnpt__nip)C)D
WHERE D.r_cabang__id='".$_GET['id']."' AND D.t_rkp__approval=4 
...
GROUP BY D.r_pegawai__id ORDER BY D.r_pegawai__nama ASC ";

...
$sql_2="SELECT 
F.gapok,
ROUND(F.gapok+F.gaji_makan+F.gaji_transport+F.tunj_jabatan+F.tunj_kemahalan+F.tunj_lain1+F.tunj_lain2) AS gaji_gross,
IF((F.ket_resign=1),ROUND(((F.gapok+F.gaji_makan+F.gaji_transport+F.tunj_jabatan+F.tunj_kemahalan+F.tunj_lain1+F.tunj_lain2)-(F.angsuran+F.potongan_bpjs))),ROUND((F.gapok+F.gaji_makan+F.gaji_transport+F.tunj_jabatan+F.tunj_kemahalan+F.tunj_lain1+F.tunj_lain2)-(F.angsuran+F.potongan_bpjs))) as gaji_netto,
F.max_hari,
IF(F.r_jabatan__level<9,'1','2')status_staf,
F.hadir as absen_hadir,
F.gapok AS gapok,
F.dasar_bpjs AS dasar_bpjs,
F.tunj_kemahalan AS  tunj_kemahalan,
F.tunj_jabatan AS tunj_jabatan,
F.tunj_lain1 AS lain1,
F.tunj_lain2 AS lain2,
F.gaji_makan AS tunj_makan,
F.gaji_transport AS tunj_trans,
F.angsuran AS angsuran,
F.potongan_bpjs AS potongan_bpjs,
F.*
FROM (SELECT 
DISTINCT( E.r_pnpt__id_pegawai) as r_pnpt__id_pegawai2,
 CASE  
  WHEN E.r_jabatan__id ='46' AND E.ket_resign=0  THEN (ROUND(E.gapok_hitung *E.hadir/E.max_hari))
  WHEN  E.ket_resign=1 THEN (ROUND(E.gapok_hitung *E.hadir/E.max_hari))
	ELSE E.gapok_hitung
 END as gapok,
E.*
 FROM (
SELECT 
 ...
 IF((D.r_gapok__nilai)is null,'0',D.r_gapok__nilai)as gapok_hitung,
 IF((D.r_gapok__lain1)is null,'0',D.r_gapok__lain1)as tunj_lain1,
 IF((D.r_gapok__lain2)is null,'0',D.r_gapok__lain2)as tunj_lain2,
 IF(ROUND(r_class_cabang.r_class__makan*(D.hadir/D.max_hari)) IS NULL,'0',ROUND(r_class_cabang.r_class__makan*(D.hadir/D.max_hari))) AS gaji_makan,
 IF(ROUND(r_class_cabang.r_class__transport*(D.hadir/D.max_hari))IS NULL,'0',ROUND(r_class_cabang.r_class__transport*(D.hadir/max_hari))) AS gaji_transport,
...
WHERE t_angsuran_pinjaman.t_ang__idkaryawan=D.r_pnpt__id_pegawai 
...
FROM(SELECT 
r_gapok.*,C.*,
(C.t_rkp__hadir+C.t_rkp__dinas+C.t_rkp__izin+C.t_rkp__cuti+C.t_rkp__sakit)as jml_hadir,
(C.t_rkp__hadir+C.t_rkp__dinas)as hadir

 FROM (SELECT 
(B.JML_HARI-B.JML_LBR)+1 as max_hari,
IF((SELECT  r_resign__mutasi FROM r_resign WHERE r_resign.r_resign__mutasi=B.mutasi AND r_resign.r_resign__approval=1)is null,'0','1')as ket_resign,
IF((rkp.t_rkp__awal)is null,'0',rkp.t_rkp__awal)as t_rkp__awal,
IF((rkp.t_rkp__akhir)is null,'0',rkp.t_rkp__akhir)as t_rkp__akhir,
IF((rkp.t_rkp__approval)is null,'0',rkp.t_rkp__approval)as t_rkp__approval,
IF((rkp.t_rkp__hadir)is null,'0',rkp.t_rkp__hadir)as t_rkp__hadir,
IF((rkp.t_rkp__sakit)is null,'0',rkp.t_rkp__sakit)as t_rkp__sakit,
IF((rkp.t_rkp__izin)is null,'0',rkp.t_rkp__izin)as t_rkp__izin,
IF((rkp.t_rkp__alpa)is null,'0',rkp.t_rkp__alpa)as t_rkp__alpa,
IF((rkp.t_rkp__dinas)is null,'0',rkp.t_rkp__dinas)as t_rkp__dinas,

IF((rkp.t_rkp__cuti)is null,'0',rkp.t_rkp__cuti)as t_rkp__cuti,
IF((rkp.t_rkp__keterangan)is null,'',rkp.t_rkp__keterangan)as t_rkp__keterangan, B.*
FROM (SELECT A.* FROM 
(SELECT v_peg.*  FROM (
SELECT  datediff('$end_active','$pre_active')AS JML_HARI ,COUNT(*) AS JML_LBR,show_peg.*
FROM (SELECT 
TIMESTAMPDIFF(MONTH,peg.r_pegawai__tgl_masuk,DATE_FORMAT(NOW(), '%Y-%m-%d')) AS lama_bln,
r_level.r_level__id, r_level.r_level__ket,
r_penempatan.r_pnpt__no_mutasi,r_penempatan.r_pnpt__finger_print,
peg.mutasi ,peg.r_pnpt__id_pegawai,peg.r_pegawai__nama,peg.r_pegawai__tgl_masuk,
...
FROM (SELECT mutasi.r_pnpt__id_pegawai,mutasi.r_pnpt__no_mutasi as mutasi,r_pegawai.r_pegawai__nama,r_pegawai.r_pegawai__tgl_masuk FROM 
...
GROUP BY show_peg.r_pnpt__id_pegawai )v_peg)A
)B LEFT JOIN t_rekap_absensi  rkp ON rkp.t_rkp__idpeg=B.r_pnpt__id_pegawai and 
rkp.t_rkp__awal>='$pre_active' and rkp.t_rkp__akhir<='$end_active')C
LEFT JOIN r_gapok ON r_gapok.r_gapok__mutasi=C.r_pnpt__no_mutasi
WHERE C.r_cabang__id='".$_GET['id']."'  and C.t_rkp__approval=4)D
INNER JOIN r_class_cabang On r_class_cabang.r_class__id=D.r_cabang__kelas)E
WHERE E.jml_hadir>0)F
GROUP BY F.r_pnpt__id_pegawai 
order BY F.r_pnpt__id_pegawai asc";
...
//----CARI TUNJANGAN MAKAN DAN TRANSPORT HARI KERJA--//
 $sql_uang_makan="SELECT 
C.mx_day,
C.r_class__makan,
C.r_class__transport
...
INNER JOIN r_cabang ON r_cabang.r_cabang__id=r_subcabang.r_subcab__cabang
INNER JOIN r_class_cabang ON r_class_cabang.r_class__id=r_cabang.r_cabang__kelas
...
GROUP BY B.r_pegawai__id )C  WHERE C.r_cabang__id='".$_GET['id']."' LIMIT 1 ";

        $rs_val = $db->Execute($sql_uang_makan);
        $uang_makan= $rs_val->fields['r_class__makan'];
        $uang_transport= $rs_val->fields['r_class__transport'];   
        $hari_kerja= $rs_val->fields['mx_day'];   
        
        $smarty->assign ("U_MAKAN", $uang_makan);
        $smarty->assign ("U_TRANSPORT", $uang_transport);
        $smarty->assign ("HARI_KERJA", $hari_kerja);
//----CLOSE TUNJANGAN MAKAN DAN TRANSPORT HARI KERJA--//
    ...
    
{
			...
			if($jenis_user=='2'){
                                    ...
                                    $sql = "SELECT 
                                            IF((C.posting_payroll )is null,'0',C.posting_payroll)as posting_payroll ,
                                            IF((C.t_gaji__awal )is null,'$pre_active',C.t_gaji__awal)as awal ,
                                            IF((C.t_gaji__akhir )is null,'$end_active',C.t_gaji__akhir)as akhir,
                                            (C.rkp_peg-C.rkp_resign)as peg_aktif,
                                            C.* 
                                            FROM (SELECT 
                                            (B.JML_HARI-B.jml)+1 as mx_day,
                                            ...
                                            count(B.t_gaji__id_peg) as posting_payroll,
                                            ...
                                            INNER JOIN r_subcabang ON r_subcabang.r_subcab__id=peg.r_pnpt__subcab
                                            INNER JOIN r_cabang ON r_cabang.r_cabang__id=r_subcabang.r_subcab__cabang
                                            LEFT JOIN t_gaji ON t_gaji.t_gaji__id_peg=peg.r_pnpt__id_pegawai and t_gaji__awal='$pre_active' and t_gaji.t_gaji__akhir='$end_active'
                                            INNER JOIN r_jabatan ON r_jabatan__id=peg.r_pnpt__jabatan
                                            INNER JOIN r_subdepartement ON peg.r_pnpt__subdept=r_subdepartement.r_subdept__id
                                            INNER JOIN r_departement ON r_departement.r_dept__id=r_subdepartement.r_subdept__dept
                                            GROUP BY peg.r_pnpt__id_pegawai ORDER BY peg.r_pnpt__no_mutasi DESC)A

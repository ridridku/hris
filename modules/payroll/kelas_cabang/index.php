...
$smarty->assign ("HREF_HOME_PATH_AJAX", $HREF_HOME.'/modules/payroll/kelas_cabang');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/payroll/kelas_cabang/';
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/payroll');
$FILE_JS  = $JS_MODUL.'/kelas_cabang';
...
//-----------SHOW DATA SHIFT-----------------------//
$sql_class = "SELECT * FROM r_class_cabang";
//var_dump($sql_shift)or die();
$result_class = $db->Execute($sql_class);
$initSet = array();
$data_class = array();
$z=0;
while ($arr=$result_class->FetchRow()) {
	array_push($data_class, $arr);
	array_push($initSet, $z);
	$z++;
}
$smarty->assign ("DATA_KELAS", $data_class);
//-----------CLOSE DATA SHIFT-----------------------//
...
$sql_="SELECT
        ...
        WHERE r_cabang.r_cabang__id='".$_GET['id']."' ";
...
$edit_r_cabang__kelas=$resultSet->fields[r_cabang__kelas];
$edit_r_class__id = $resultSet->fields[r_class__id];
$edit_r_class__makan = $resultSet->fields[r_class__makan];
$edit_r_class__transport = $resultSet->fields[r_class__transport];

$edit = 1;
$smarty->assign ("CABANG_NAMA", $edit_r_cabang__nama);
$smarty->assign ("CABANG_ID", $edit_r_cabang__id);
$smarty->assign ("CABANG_KELAS", $edit_r_cabang__kelas);
$smarty->assign ("KELAS_ID", $edit_r_class__id);
$smarty->assign ("KELAS_MAKAN", $edit_r_class__makan);
$smarty->assign ("KELAS_TRANSPORT", $edit_r_class__transport);
	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
		  ...
		  if($jenis_user=='2'){
                                   $sql  = "SELECT
                                            ...
                                            r_class_cabang.r_class__date_updated
                                            FROM r_cabang
                                            inner join r_class_cabang ON r_class_cabang.r_class__id=r_cabang.r_cabang__kelas  WHERE 1=1";	                 
                                    } else {
				$sql  = "SELECT
                                            r_cabang.r_cabang__id,
                                            r_cabang.r_cabang__akronim,

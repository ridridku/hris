...
$sql_2="SELECT C.hadir as absen_hadir,
C.r_gapok__nilai AS gapok,
C.r_gapok__dasar_bpjs AS dasar_bpjs,
C.gaji_makan AS tunj_makan,
C.gaji_transport AS tunj_trans,
C.r_gapok__jabatan tunj_jabatan,
r_gapok__kemahalan AS tunj_kemahalan,
C.r_gapok__lain AS  tunj_lain,
C.kosan AS  tunj_kost,
C.angsuran AS angsuran,
C.potongan_bpjs,
ROUND(C.r_gapok__nilai+C.gaji_makan+C.gaji_transport+r_gapok__jabatan+r_gapok__kemahalan+C.kosan) as gaji_gross,
ROUND((C.r_gapok__nilai+C.gaji_makan+C.gaji_transport+r_gapok__jabatan+r_gapok__kemahalan+C.kosan)-(C.angsuran+C.potongan_bpjs)) as gaji_netto,
C.*FROM (SELECT 
IF((B.r_gapok__kosan)is null,'0',B.r_gapok__kosan)as kosan,
ROUND(B.r_gapok__makan*(B.hadir/B.max_hari)) AS gaji_makan,
ROUND(B.r_gapok__transport*(B.hadir/max_hari)) AS gaji_transport,
ROUND(B.r_gapok__dasar_bpjs * 0.0424	) AS bpjs_tk_tmw,
ROUND(B.r_gapok__dasar_bpjs * 0.02	) AS bpjs_tk_peg,
ROUND(B.r_gapok__dasar_bpjs * 0.040	) AS bpjs_kes_tmw,
ROUND(B.r_gapok__dasar_bpjs * 0.010	) AS bpjs_kes_peg,
(ROUND(B.r_gapok__dasar_bpjs * 0.02)+ROUND(B.r_gapok__dasar_bpjs * 0.010)) as potongan_bpjs,
IF((SELECT SUM(t_ang__nilai_angsuran) as angsuran from t_angsuran_pinjaman
WHERE t_angsuran_pinjaman.t_ang__idkaryawan=B.r_pegawai__id GROUP BY t_angsuran_pinjaman.t_ang__idkaryawan
AND t_angsuran_pinjaman.t_ang__awal='$pre_active' and t_angsuran_pinjaman.t_ang__akhir='$end_active')IS NULL,'0',
(SELECT SUM(t_ang__nilai_angsuran) as angsuran from t_angsuran_pinjaman
WHERE t_angsuran_pinjaman.t_ang__idkaryawan=B.r_pegawai__id GROUP BY t_angsuran_pinjaman.t_ang__idkaryawan
AND t_angsuran_pinjaman.t_ang__awal='$pre_active' and t_angsuran_pinjaman.t_ang__akhir='$end_active')
) AS angsuran,
CASE B.r_pnpt__no_mutasi WHEN (SELECT r_resign.r_resign__mutasi FROM r_resign WHERE r_resign.r_resign__mutasi = B.r_pnpt__no_mutasi AND r_resign__approval = 1
) THEN '1' ELSE '0' END ket_resign ,B.* FROM (SELECT A.r_pegawai__id as peg_id, r_gapok.*,sum(t_rekap_absensi.t_rkp__hadir) as hadir,
...
INNER JOIN r_gapok ON r_gapok__idpeg=A.r_pnpt__id_pegawai
WHERE t_rekap_absensi.t_rkp__awal='$pre_active' AND t_rekap_absensi.t_rkp__akhir='$end_active') B )C WHERE C.r_cabang__id='".$_GET['id']."' AND C.t_rkp__approval=4
AND C.t_rkp__no_mutasi NOT IN (select DISTINCT r_resign__mutasi FROM r_resign WHERE r_resign.r_resign__tgl <= CURRENT_DATE - INTERVAL 35 DAY and r_resign.r_resign__approval=1 ) ";

//var_dump($sql_2)or die();$pre_active
//$end_active
//$pre_active ".$_GET['id']."

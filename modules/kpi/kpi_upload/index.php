...
$smarty->assign ("HREF_HOME_PATH_AJAX", $HREF_HOME.'/modules/kpi/kpi_upload');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/kpi/kpi_upload/';


#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/kpi');
$FILE_JS  = $JS_MODUL.'/kpi_upload';
...
if ($_GET['search'] == '1')
	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
		  ...
		  if($jenis_user=='2'){
                    $sql  = "   SELECT
                                r_uploadkpi__id AS r_uploadkpi__id,
                                r_uploadkpi__dep AS r_uploadkpi__dep,
                                r_uploadkpi__user AS r_uploadkpi__user,
                                r_uploadkpi__filename AS r_uploadkpi__filename,
                                r_uploadkpi__date_created AS r_uploadkpi__date_created,
                                r_uploadkpi__date_updated AS r_uploadkpi__date_updated,
                                D.r_dept__akronim AS r_dept__akronim,
                                D.r_dept__id AS r_dept__id,
                                D.r_dept__ket AS r_dept__ket,
                                E.r_pegawai__nama AS r_pegawai__nama,
                                F.r_cabang__nama,
                                F.r_cabang__id
                                FROM
                                r_upload_kpi A
                                INNER JOIN r_penempatan B ON A.r_uploadkpi__user = B.r_pnpt__nip
                                INNER JOIN r_subdepartement C ON C.r_subdept__id = B.r_pnpt__subdept
                                INNER JOIN r_departement D ON D.r_dept__id = A.r_uploadkpi__dep
                                INNER JOIN r_pegawai E ON E.r_pegawai__id = B.r_pnpt__id_pegawai
                                INNER JOIN r_cabang F ON F.r_cabang__id = B.r_pnpt__subcab
                                INNER JOIN r_subcabang G ON F.r_cabang__id = G.r_subcab__cabang
			  ...
			  $sql .= " ORDER BY  r_uploadkpi__date_created desc";

...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$rs__=$db->Execute($sql__);
				 ...
				 $smarty->assign ("NM_PERWAKILAN", $nm_perwakilan);
						...
						$sql = " select kode_kat_kasus, tbl_kasus_pengaduan.*, tbl_wni.* ,nama_kabupaten, nama_kasus, nama_pengaduan,
						 nm_perwakilan,
						...
						if ( $tahun !=''   ) {
						 ...
						 if ($kode_perwakilan !='') {
						  ...
						  if ($kode_wni !='') {
						 	$sql.=" and tbl_wni.kode_wni='$kode_wni' ";
						 }
	...
	$label="Nama :". $arr[nama]. "<p> No KTP :".$arr[no_paspor]." <p> No Pasport :".$arr[kode_wni]." <p> Alamat : ".$arr[alamat]." <p>
		...
		if ($arr[bulan_tl]==1){ $bulantl = "Januari"; }
		if ($arr[bulan_tl]==2){ $bulantl = "Februari"; }
		if ($arr[bulan_tl]==3){ $bulantl = "Maret"; }
		if ($arr[bulan_tl]==4){ $bulantl = "April"; }
		if ($arr[bulan_tl]==5){ $bulantl = "Mei"; }
		if ($arr[bulan_tl]==6){ $bulantl = "Juni"; }
		if ($arr[bulan_tl]==7){ $bulantl = "Juli"; }
		if ($arr[bulan_tl]==8){ $bulantl = "Agustus"; }
		if ($arr[bulan_tl]==9){ $bulantl = "September"; }
		if ($arr[bulan_tl]==10){ $bulantl = "Oktober"; }
		if ($arr[bulan_tl]==11){ $bulantl = "November"; }
		if ($arr[bulan_tl]==12){ $bulantl = "Desember"; }
		
		$tgltl = $arr[tanggal_tl]." ".$bulantl." ".$arr[tahun_tl];
			   
$content_data .= " <tr height=18 style='height:13.5pt'>
  <td height=18 class=xl28 align=right style='height:13.5pt'>".$label."</td>
  <td class=xl29>".$arr[nm_perwakilan]."</td>
  <td class=xl29 align=right >".$arr[sumber_informasi]."</td>
  <td class=xl29 align=right >".$tgltl."</td>
  <td class=xl29 align=right >".$arr[kronologis_permasalahan]."</td>
  <td class=xl29 align=right >".$arr[bantuan]."</td> ";
 
				


		
				$sql_detail = "select * from tbl_tindak_lanjut inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_tindak_lanjut.kode_form_pengaduan where kode_jenis_penindak=1 ";
 ...
 if ($kode_wni !='') {
				...
				$tindak="";	
				if ($arr2[kode_form_pengaduan]=$arr[kode_form_pengaduan]) {
						 	$tindak.= $tindak."<p> - ".$arr2[tindak_lanjut];
			  
				}
				
				}
				$content_data .= "<td class=xl29 align=right >".$tindak."</td> ";
				
			   $smarty->assign ("DATA_TB2", $data_tb2);
				$smarty->assign ("TINDAK", $tindak);


			   $sql_alat_pinjam="select * from tbl_tindak_lanjut  inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_tindak_lanjut.kode_form_pengaduan where kode_jenis_penindak=2 ";
 ...
 if ($kode_wni !='') {
		...
		/*		$tindak2='';
				if ($arr3[kode_form_pengaduan]=$arr[kode_form_pengaduan]) {
						 	$tindak2.= $tindak2."<p> - ".$arr3[tindak_lanjut];
							
							
			    }
					
					$content_data .= "<td class=xl29 align=right >".$tindak2."</td> "; */
			  ...
			  $smarty->assign ("DATA_TB3", $data_tb3);
					    ...
					    $sql_detail2="select * from tbl_hak_tki  inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_hak_tki.kode_form_pengaduan where 1=1 ";
		...
		/*				
						$penye='';
				if ($arr4[kode_form_pengaduan]=$arr[kode_form_pengaduan]) {
						 	$penye.= $penye."<p> - pemberian ".$arr4[jenis_hak]." sebesar Rp. ".$arr4[ekuivalent_rp];
			    }
					
					$content_data .= "<td class=xl29 align=right >".$penye."</td></tr> "; */
				...
				// asuransi TKI
					    ...
					    $sql_detail5="select * from tbl_asuransi_tki inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_asuransi_tki.kode_form_pengaduan where 1=1 ";

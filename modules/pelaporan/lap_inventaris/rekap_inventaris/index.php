...
require_once($DIR_HOME.'../laporan/inc.rekap_inventaris.php');  
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/lap_inventaris/rekap_inventaris';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan/lap_inventaris');
$FILE_JS  = $JS_MODUL.'/rekap_inventaris';
...
if ($_GET['search'] == '1')
{
	...
	$sql__="SELECT *,UPPER(r_cabang__nama) AS nm_perwakilan FROM r_cabang WHERE  r_cabang.r_cabang__id='$kode_cabang_cari' ";
        ...
        $smarty->assign ("NM_PERWAKILAN", $nm_perwakilan);	
              ...
              if($jenis_user=='2'){
				           $sql  = "SELECT B.r_inventaris__id AS r_inventaris__id,
                                    B.r_inventaris__mutasi AS r_inventaris__mutasi,
                                    B.r_inventaris__tgl_pinjam AS r_inventaris__tgl_pinjam,
                                    B.r_inventaris__tgl_kembali AS r_inventaris__tgl_kembali,
                                    B.r_inventaris__alat AS r_inventaris__alat,
                                    B.r_inventaris__kode AS r_inventaris__kode,
                                    B.r_inventaris__qty AS r_inventaris__qty,
                                    B.r_inventaris__kondisi AS r_inventaris__kondisi,
                                    B.r_inventaris__kepemilikan AS r_inventaris__kepemilikan,
                                    B.r_inventaris__gambar AS r_inventaris__gambar,
                                    B.r_inventaris__lokasi AS r_inventaris__lokasi,
                                    B.r_inventaris__keterangan AS r_inventaris__keterangan,
                                    B.r_inventaris__status AS r_inventaris__status,
                                    B.r_inventaris__date_created AS r_inventaris__date_created,
                                    B.r_inventaris__date_updated AS r_inventaris__date_updated,
                                    B.r_inventaris__user_created AS r_inventaris__user_created,
                                    B.r_inventaris__user_updated  AS r_inventaris__user_updated,
					   ...
					   $sql  = "SELECT B.r_inventaris__id AS r_inventaris__id,
						...
						if ($departemen_cari !=''  ) {
						 	$sql.="  and r_dept__id='$departemen_cari' ";
                                                 ...
                                                 if ($date_awal !='' AND $date_akhir !='')
                                                     {
                                                     $sql.=" AND date(r_inventaris__tgl_pinjam) BETWEEN '$date_awal' AND '$date_akhir' ";
                                                     }
						if ($nama_karyawan_cari !='') {
						 	...
						 	$sql.="and r_pnpt__finger_print='".$finger_cari."'  ";
                                                  ...
                                                  if ($jenis_cari !='') {
						 	$sql.="and r_jenis__id='".$jenis_cari."'  ";
						 }
                                                 
                                                   if ($status_cari !='') {
						 	$sql.="and r_inventaris__status='".$status_cari."'  ";
						 }
                                                 
                                                 
                                            
						  $sql.=" order by r_inventaris__id ASC  ";
							...
							while ($arr=$recordSet->FetchRow()) {
								array_push($data_tb, $arr);
                                                        ...
                                                        $smarty->assign ("MUTASI_PEG", $kpi_mutasi);
                                                         ...
                                                         $status=$arr[r_inventaris__status];
                                                         
                                                         if($status==1){
                                                             
                                                             $status_ket='Sudah Kembali';
                                                         }  else {
                                                               $status_ket='Dipinjam';
                                                         }
                                                        ...
                                                        $arr[r_dept__ket],
                                                        $arr[r_jenis__nama],
                                                        $arr[r_inventaris__alat],
                                                        $arr[r_inventaris__kode],
                                                        $arr[r_inventaris__tgl_pinjam],
                                                        $arr[r_inventaris__id],
                                                        $status_ket);	
                                                ...
                                                //total karyawan
                                                $sql_total="SELECT COUNT(B.r_inventaris__id) AS total_orang,
						...
						if ($departemen_cari !=''  ) {
						 	$sql_total.="  and r_dept__id='$departemen_cari' ";
                                                 ...
                                                 if ($date_awal !='' AND $date_akhir !='')
                                                     {
                                                     $sql_total.=" AND date(r_inventaris__tgl_pinjam) BETWEEN '$date_awal' AND '$date_akhir' ";
                                                     }
						if ($nama_karyawan_cari !='') {
						 	...
						 	$sql_total.="and r_pnpt__finger_print = '".$finger_cari."'  ";
                                                  ...
                                                  if ($jenis_cari !='') {
						 	$sql_total.="and r_jenis__id='".$jenis_cari."'  ";
						 }
                                                     if ($status_cari !='') {
						 	$sql_total.="and r_inventaris__status='".$status_cari."'  ";
						 }
//                                               
						  $sql.=" order by r_inventaris__id Asc ";

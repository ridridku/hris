...
require_once($DIR_HOME.'../laporan/inc.rekap_sp.php');  
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/lap_kepegawaian/data_sp';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan/lap_kepegawaian');
$FILE_JS  = $JS_MODUL.'/data_sp';
        ...
        $smarty->assign ("NM_PERWAKILAN", $nm_perwakilan);	
              ...
              if($jenis_user=='2'){
				           $sql  = "SELECT 
                                            ...
                                            WHERE r_pnpt__aktif=1 AND peg.r_cabang__id= '".$kode_pw_ses."' ";
			} else {
					   $sql  = "SELECT 
                                            A.t_sp__no AS t_sp__no,
                                            A.t_sp__mutasi AS t_sp__mutasi,
						...
						if ($departemen_cari !=''  ) {
						  ...
						  if ($nama_karyawan_cari !='') {
						 	$sql.="and peg.r_pegawai__nama LIKE '%".$nama_karyawan_cari."%'  ";
                                              ...
                                              if ($bulan !='') {
                                                    $sql.=" AND MONTH(t_sp__tgl_expired)='$bulan'";
                                             }
                           ...
                           $status=$arr[selisih];
                            $jenis=$arr[t_sp__jenis];
                         
                           IF ($status==0)
                                {   
                                    $status_label='Masih Berlaku';
                                }
                                else 
                                {
                             ...
                             IF ($jenis==1)
                                {   
                                    $status_jenis='Surat Peringatan 1';
                                }
                                elseif ($jenis==2)
                                {
                                    $status_jenis='Surat Peringatan 2';
                                }
                                else
                                {
                                    $status_jenis='Surat Peringatan 3';
                                }
                                
                                
				    $tgl_keluar_sp1=TanggalIndo($arr[t_sp__tgl]);
                                    $tgl_keluar_sp2=TanggalIndo($arr[t_sp__tgl_expired]);
				 $finger=sprintf("%06s",$arr[r_pnpt__finger_print]);
			   ...
			   $content_data .= print_content(
			    ...
			    $smarty->assign ("DATA_TB", $data_tb);
                       ...
                       if($jenis_user=='2'){      
                            //total karyawan
                          $sql_total="SELECT COUNT(*) as total_orang  FROM t_surat_peringatan A
                                      INNER JOIN v_pegawai peg ON peg.r_pnpt__no_mutasi = A.t_sp__mutasi
                                 ...
                                 $sql_total="SELECT COUNT(*) as total_orang  FROM t_surat_peringatan A
                                      INNER JOIN v_pegawai peg ON peg.r_pnpt__no_mutasi = A.t_sp__mutasi
                                        WHERE peg.r_pnpt__aktif = 1 ";

                            }
                             if ($kode_cabang_cari !='') {
                                       $sql_total.=" and  peg.r_cabang__id =".$kode_cabang_cari."  ";
                                }

                                if ($kode_subcab_cari !='') {
                                       $sql_total.=" and  peg.r_subcab__id ='$kode_subcab_cari' ";
                                }

                               if ($departemen_cari !=''  ) {
                                       $sql_total.="  and peg.r_dept__id='$departemen_cari' ";
                                } 

                                if ($nama_karyawan_cari !='') {
                                    $sql_total.="and  peg.r_pegawai__nama LIKE '%".$nama_karyawan_cari."%'  ";
						 }
                                if ($bulan !='') {
                                    $sql_total.=" AND MONTH(t_sp__tgl_expired)='$bulan'";
                                 }

                                if ($tahun !='') {
                                      $sql_total.=" AND YEAR(t_sp__tgl)='$tahun'";
                               }
                               if($finger_cari!='')
                                   {
                                ...
                                if($selisih_cari!='')
                               {
                                   $sql_total.=" AND (UNIX_TIMESTAMP(DATE(NOW())))-(UNIX_TIMESTAMP(A.t_sp__tgl_expired))$selisih_cari";
                               }   
                           /*
                    SELECT *FROM  t_surat_peringatan A WHERE  MONTH(A.t_sp__tgl_expired) BETWEEN '01' AND '12';
                            */
				...
				array_push($initSet4, $z);
					...
					$content_data .= print_content("","","","","","","","","","",$label);
                               ...
                               //TUTUP TOTAL 
				$file= $DIR_HOME."/files/rekap_sp".$nm_perwakilan."_".$tahun.".xls";
				$fp=@fopen($file,"w");
				if(!is_resource($fp))
				return false;
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/rekap_sp".$nm_perwakilan."_".$tahun.".xls";	

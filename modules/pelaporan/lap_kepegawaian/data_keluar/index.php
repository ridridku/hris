...
require_once($DIR_HOME.'../laporan/inc.rekap_keluar.php');  
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/lap_kepegawaian/data_keluar';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan/lap_kepegawaian');
$FILE_JS  = $JS_MODUL.'/data_keluar';
        ...
        $smarty->assign ("NM_PERWAKILAN", $nm_perwakilan);	
              ...
              if($jenis_user=='2'){
				           $sql  = "SELECT
                                                ...
                                                DATE_FORMAT(A.r_resign__tgl,'%d %b %Y') AS r_resign__tgl ,
                                                    ...
                                                    peg.r_level__ket AS  r_level__ket
                                                    from r_resign A
                                                    INNER JOIN v_pegawai peg ON A.r_resign__mutasi=peg.r_pnpt__no_mutasi
                                                    where peg.r_pnpt__aktif=0 AND peg.r_cabang__id= '".$kode_pw_ses."' ";
			} else {
					   $sql  = "SELECT
                                                    A.r_resign__no AS  r_resign__no,
                                                    DATE_FORMAT(A.r_resign__tgl,'%d %b %Y') AS r_resign__tgl ,
						...
						if ($departemen_cari !=''  ) {
						  ...
						  if ($nama_karyawan_cari !='') {
						 	$sql.="and peg.r_pegawai__nama LIKE '%".$nama_karyawan_cari."%'  ";
                                                  ...
                                                  if ($bulan !='') {
						 	$sql.="and MONTH(A.r_resign__tgl) = '".$bulan."'  ";
						 }
                                                   if ($tahun !='') {
						 	$sql.="and YEAR(A.r_resign__tgl) = '".$tahun."'  ";
						 }
                           ...
                           $status_resign=$arr[r_resign__approval];
                           IF ( $status_resign==0)
                                {   
                                    $status_label='Tidak Disetujui';
                                }
                                else 
                                {
			   ...
			   $content_data .= print_content(
			    ...
			    $smarty->assign ("DATA_TB", $data_tb);
                       ...
                       if($jenis_user=='2'){      
                            //total karyawan
                          $sql_total="SELECT COUNT(*) as total_orang  FROM r_resign A
                                        INNER JOIN v_pegawai peg ON A.r_resign__mutasi = peg.r_pnpt__no_mutasi
                                        WHERE peg.r_pnpt__aktif = 0 AND peg.r_cabang__id= '".$kode_pw_ses."'  ";
                                 }
                            else {
				...
				$sql_total.="and  peg.r_pegawai__nama LIKE '%".$nama_karyawan_cari."%'  ";
                                ...
                                if ($bulan !='') {
                                            $sql_total.="and MONTH(A.r_resign__tgl) = '".$bulan."'  ";
                                     }
                                    if ($tahun !='') {
                                            $sql_total.="and YEAR(A.r_resign__tgl) = '".$tahun."'  ";
                                     }
				...
				array_push($initSet4, $z);
					...
					$content_data .= print_content("","","","","","","","","","",$label);
                               ...
                               //TUTUP TOTAL 
				$file= $DIR_HOME."/files/laporan_resign_".$nm_perwakilan."_".$tahun.".xls";
				$fp=@fopen($file,"w");
				if(!is_resource($fp))
				return false;
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_resign_".$nm_perwakilan."_".$tahun.".xls";	

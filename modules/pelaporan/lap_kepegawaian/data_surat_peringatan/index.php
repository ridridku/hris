...
//require_once($DIR_HOME.'/laporan/inc.sp3.php');  
...
$bulan_romawai=Romawi($bulan_session);
$smarty->assign ("BLN_ROMAWI", $bulan_romawai);
...
$smarty->assign ("CV_JABATAN", $data_jabatan_history);
    ...
    if($jenis_user=='2'){                       $sql  ="SELECT 
                                                A.t_sp__no AS t_sp__no,
                                                A.t_sp__mutasi AS t_sp__mutasi,
                                                UPPER(A.t_sp__hglm) AS t_sp__hglm,
                                                UPPER(A.t_sp__hglm_mutasi) AS t_sp__hglm_mutasi,
                                                UPPER(A.t_sp__atasan) AS t_sp__atasan,
                                                UPPER(A.t_sp__atasan_jabatan) AS t_sp__atasan_jabatan,
                                                ...
                                                UPPER(peg.r_jabatan__ket) AS r_jabatan__ket,
                                                ...
                                                peg.r_pegawai__id AS r_pegawai__id,
                                                 UPPER(peg.r_pegawai__nama) AS r_pegawai__nama
                                                FROM
...
//                                       $jenis_sp);
				...
				$next_prev = $p->nextPrevCustom($page, $pages, "ORDER=".$ORDER."&".$str_completer); 
                                ...
                                $t_sp__tgl_expired= $data_tb[0][t_sp__tgl_expired];
                              ...
                              $lama_sp=($m2-$m1)+1;
                              $smarty->assign ("LAMA_SP", $lama_sp);  
                                
                                
                                
                                $file_2= $HREF_HOME."/laporan/inc.sp3.php?nm_perwakilan=".$nm_perwakilan."&bulan=".$bulan."&tahun=".$tahun."&nip=".$nip."&no=".$no."&mutasi=".$mutasi; 

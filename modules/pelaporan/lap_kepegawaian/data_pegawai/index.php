...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/lap_kepegawaian/data_pegawai';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan/lap_kepegawaian');
$FILE_JS  = $JS_MODUL.'/data_pegawai';
...
if ($_GET['search'] == '1')
{
	...
	$sql__="SELECT *,UPPER(r_cabang__nama) AS nm_perwakilan FROM r_cabang WHERE  r_cabang.r_cabang__id='$kode_cabang_cari' ";
        ...
        $smarty->assign ("NM_PERWAKILAN", $nm_perwakilan);	
              ...
              if($jenis_user=='2'){
				           $sql  = "SELECT * FROM v_pegawai peg where peg.r_cabang__id= '".$kode_pw_ses."' AND peg.r_pnpt__aktif =1";
			} else {
					   $sql  = " SELECT * FROM v_pegawai peg where 1=1 AND peg.r_pnpt__aktif =1 ";	
			   ...
			   $content_data .= print_content(
			    ...
			    $smarty->assign ("DATA_TB", $data_tb);
                            ...
                            //total karyawan
                          $sql_total="SELECT COUNT(*) as total_orang FROM v_pegawai peg where 1=1 AND peg.r_pnpt__aktif =1";
                             if ($kode_cabang_cari !='') {
                               ...
                               if ($departemen_cari !=''  ) {
                                ...
                                if ($bulan !=''  ) {
                                        $sql_total.="  and MONTH(r_pnpt__kon_akhir)='$bulan' ";
                                                      } 
                                if ($tahun !=''  ) {
                                       $sql_total.="  and YEAR(r_pnpt__kon_akhir)='$tahun' ";
                                } 

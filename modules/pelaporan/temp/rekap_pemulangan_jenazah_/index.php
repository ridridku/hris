...
(SELECT COUNT(c.kode_wni) from tbl_jenazah c left join tbl_mast_perwakilan d on (c.kode_perwakilan=d.kode_perwakilan) where d.kode_perwakilan=a.kode_perwakilan and kode_pilihan_meninggal=1 and LEFT(c.tgl_tiba,4) = '$tahun'
 and MONTH(c.tgl_tiba) =$bulan ) as orang
FROM tbl_mast_perwakilan a  ";
	}
	
	if ($bulan =='') {
		$sql_detail = "SELECT distinct nm_perwakilan,
(SELECT COUNT(c.kode_wni) from tbl_jenazah c left join tbl_mast_perwakilan d on (c.kode_perwakilan=d.kode_perwakilan) where d.kode_perwakilan=a.kode_perwakilan and kode_pilihan_meninggal=1 and LEFT(c.tgl_tiba,4) = '$tahun') as orang
FROM tbl_mast_perwakilan a  ";
	}
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					$content_data .= print_content($arr2[nm_perwakilan],$arr2[orang]);
...
where kode_pilihan_meninggal=1	and LEFT(tgl_pulang,4) ='$tahun' ";
...
FROM tbl_mast_kawasan a";
			  ...
			  $sql_total="SELECT COUNT(kode_wni) as total_orang from tbl_jenazah where kode_pilihan_meninggal=1 and LEFT(tgl_tiba,4) ='$tahun'  ";
if ($bulan !='') {	
			   ...
			   array_push($row_class4, $ROW_CLASSNAME2);
				 array_push($initSet4, $z);
					$z++;
						...
						$tot="JUMLAH ORANG : ".$arr4[total_orang];
					$content_data .= print_content("",$tot);
				}

			  $smarty->assign ("DATA_TB4", $data_tb4);
			  
				$file= $DIR_HOME."/files/rekap_pemulangan_jenazah_".$nm_perwakilan."_".$tahun.".xls";
				...
				$content = print_header($tahun,$nama_bulan);
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/rekap_pemulangan_jenazah_".$tahun."_".$nama_bulan.".xls";		

...
require_once($DIR_HOME.'/laporan/inc.penahanan_wni.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/penahanan';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/penahanan';
...
if ($_GET['tahun'] != '')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
		  ...
		  if ($kode_perwakilan !='') {
					 ...
					 $sql="select distinct bulan, tahun , (select count(*) from tbl_penahanan inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_penahanan.kode_form_pengaduan inner join  tbl_wni on tbl_wni.kode_wni = tbl_kasus_pengaduan.kode_wni  where LEFT(tgl_ditahan,4)='$tahun'  and SUBSTRING(tgl_ditahan,6,2)=v_penahanan.bulan  and tbl_kasus_pengaduan.kode_perwakilan = v_penahanan.kode_perwakilan   )  as jml_meninggal from v_penahanan where tahun='$tahun'    ";

			
				 } else {
					
					 $sql="select distinct bulan, tahun , (select count(*) from tbl_penahanan inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_penahanan.kode_form_pengaduan inner join  tbl_wni on tbl_wni.kode_wni = tbl_kasus_pengaduan.kode_wni  where LEFT(tgl_ditahan,4)='$tahun'  and SUBSTRING(tgl_ditahan,6,2)=v_penahanan.bulan     )  as jml_meninggal from v_penahanan where tahun='$tahun'    ";
					...
					if ($kode_perwakilan !='') {
						 $sql.=" and v_penahanan.kode_perwakilan='$kode_perwakilan' ";
				...
				tempat_penahanan,
				tgl_ditahan,
				tbl_kasus_pengaduan.keterangan 
				,tbl_kasus_pengaduan.kode_form_pengaduan
				, SUBSTRING(tgl_ditahan,6,2)  as bulan , LEFT(tgl_ditahan,4) as tahun , RIGHT(tgl_ditahan,2) as tanggal  , nama_kasus
				from tbl_penahanan 
					inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_penahanan.kode_form_pengaduan
				...
				left join tbl_mast_kasus on tbl_mast_kasus.kode_kasus = tbl_kasus_pengaduan.kode_kasus
				 ...
				 and LEFT(tgl_ditahan,4) ='$tahun'   ";

				  if ($bulan !='') {
						 $sql_detail.=" and SUBSTRING(tgl_ditahan,6,2)='$bulan' ";
					}
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					$content_data .= print_content($arr2[nama],$arr2[no_paspor],$arr2[jk],$arr2[nm_perwakilan],$arr2[tgl_ditahan],$arr2[tempat_penahanan],
									$arr2[masa_penahanan],$arr2[keterangan]);
					...
					$tot="TOTAL LAPORAN PENAHANAN WNI: ".$arr[jml_meninggal];
...
//print 	$sql_detail ;
			   ...
			   $sql_detail3="select * from tbl_kasus_pengaduan_detail  inner join tbl_mast_kasus on tbl_mast_kasus.kode_kasus =tbl_kasus_pengaduan_detail.kode_kasus    ";
				   ...
				   $smarty->assign ("DATA_TB3", $data_tb3);
                        $file= $DIR_HOME."/files/laporan_penahanan_wni_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_penahanan_wni_".$nm_perwakilan."_".$tahun.".xls";

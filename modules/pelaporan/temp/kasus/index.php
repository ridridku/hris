	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$rs__=$db->Execute($sql__);
				 ...
				 $smarty->assign ("NM_PERWAKILAN", $nm_perwakilan);
						...
						$sql = "SELECT
  kronologis_permasalahan,
  bantuan,
  kode_kat_kasus,
  tbl_kasus_pengaduan.kode_wni,
  tbl_wni.nama,
  tbl_wni.alamat,
  no_paspor,
  tbl_tindak_lanjut.kode_form_pengaduan,
   tbl_tindak_lanjut.tindak_lanjut as tindakan,
  nm_perwakilan,
  nama_kabupaten,
  nama_kasus,
  nama_pengaduan,
...
FROM
  tbl_kasus_pengaduan
  INNER JOIN tbl_wni
    ON tbl_wni.kode_wni = tbl_kasus_pengaduan.kode_wni
  LEFT JOIN tbl_mast_pengaduan
    ON tbl_mast_pengaduan.kode_pengaduan = tbl_kasus_pengaduan.kode_pengaduan
  LEFT JOIN tbl_mast_kasus
    ON tbl_mast_kasus.kode_kasus = tbl_kasus_pengaduan.kode_kasus
  LEFT JOIN tbl_kerja_tki
    ON tbl_kerja_tki.kode_kerja_tki = tbl_kasus_pengaduan.kode_kerja_tki
  INNER JOIN tbl_mast_perwakilan
    ON tbl_mast_perwakilan.kode_perwakilan = tbl_kasus_pengaduan.kode_perwakilan
  LEFT JOIN tbl_mast_wil_kabupaten
    ON tbl_mast_wil_kabupaten.id_kabupaten = tbl_wni.id_kabupaten
  LEFT JOIN tbl_tindak_lanjut
  ON tbl_kasus_pengaduan.kode_form_pengaduan=tbl_tindak_lanjut.kode_form_pengaduan
WHERE
  1 = 1 ";
						//var_dump($sql)or die();
						 if ($kode_kat_kasus =='') {
						...
						if ( $tahun !=''   ) {
						 ...
						 if ($kode_perwakilan !='') {
						  ...
						  if ($nama !='') {
							...
							$label="Nama :". $arr[nama]. "<p> No KTP :".$arr[no_paspor]." <p> No Pasport :".$arr[kode_wni]." <p> Alamat : ".$arr[alamat]." <p> TINDAK LANJUT PWNI:=".$arr[tindakan]."Daerah Asal : ".$arr[nama_kabupaten]." <p>"; 					
				...
				$sql_detail = "select tbl_tindak_lanjut.tindak_lanjut,tbl_tindak_lanjut.kode_form_pengaduan  from tbl_tindak_lanjut inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_tindak_lanjut.kode_form_pengaduan where kode_jenis_penindak=1 ";
//var_dump($sql_detail) or die();
			   ...
			   $content_data .= print_content($label,$arr[nm_perwakilan],$arr[sumber_informasi],$labeltgl,$arr[kronologis_permasalahan],$arr[bantuan],$arr[tindakan],"","");	
			   ...
			   //var_dump($sql_alat_pinjam) or die();

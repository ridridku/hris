...
require_once($DIR_HOME.'/laporan/inc.rekap_menikah_dengan_wna.php');  
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/rekap_menikah_dengan_wna';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/rekap_menikah_dengan_wna';
...
$sql_detail="SELECT distinct nm_perwakilan, (SELECT COUNT(c.kode_wni) from tbl_nikah_wna c  where c.kode_perwakilan=a.kode_perwakilan  ";


    if ($bulan !='') {
		$sql_detail .= "and MONTH(c.tgl_nikah) ='$bulan' ";
	}

	 if ($tahun !='') {
		$sql_detail .= "and  LEFT(c.tgl_nikah,4) = '$tahun' ";
	}
...
where 1=1 ";
 ...
 if ($kode_kawasan !=''){
	$sql_detail .= " and tbl_mast_kawasan.kode_kawasan ='$kode_kawasan'";
 }
 ...
 $sql_detail .=" order by nm_perwakilan ";
	 ...
	 $sql_kawasan="SELECT distinct nama_kawasan, (SELECT COUNT(tbl_nikah_wna.kode_wni)
	from tbl_nikah_wna
	inner join tbl_mast_perwakilan on tbl_nikah_wna.kode_perwakilan=tbl_mast_perwakilan.kode_perwakilan
	...
	LEFT JOIN tbl_mast_kawasan ON tbl_mast_negara.kode_kawasan=tbl_mast_kawasan.kode_kawasan where 1=1   ";
			...
			if ($bulan !='') {
				 $sql_kawasan .= "and MONTH(tgl_nikah)= '$bulan' ";
			}

			if ($tahun !='') {
			  $sql_kawasan .= "and YEAR(tgl_nikah) ='$tahun'  ";
			}


			 $sql_kawasan .= " and tbl_mast_kawasan.kode_kawasan=a.kode_kawasan) as orang  FROM tbl_mast_kawasan a where  1=1 ";

			if ($kode_kawasan !=''){
				 $sql_kawasan .= " and kode_kawasan='$kode_kawasan'  ";
			}
 ...
 $sql_total=" SELECT COUNT(kode_wni) as total_orang from tbl_nikah_wna
left join tbl_mast_perwakilan on tbl_nikah_wna.kode_perwakilan = tbl_mast_perwakilan.kode_perwakilan
LEFT JOIN tbl_mast_negara ON tbl_mast_perwakilan.kode_negara=tbl_mast_negara.kode_negara
LEFT JOIN tbl_mast_kawasan ON tbl_mast_negara.kode_kawasan=tbl_mast_kawasan.kode_kawasan where 1=1   ";

			if ($bulan !='') {
				$sql_total.= " 	and MONTH(tgl_nikah) = '$bulan' ";
			}

			if ($tahun !='') {
				$sql_total.= " 	and LEFT(tgl_nikah,4) ='$tahun' ";
			}
			...
			if ($kode_kawasan !='') {
				...
				while ($arr4=$recordSet4->FetchRow()) {
					...
					$content_data .= print_content("","<b>TOTAL WNI MENIKAH DENGAN WNA</b>",$tot2);
				 	 $content_data2 .= print_content("","<b>TOTAL WNI MENIKAH DENGAN WNA</b>",$tot2);
		...
		$file= $DIR_HOME."/files/rekap_menikah_dengan_wna_".$nama_bulan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/rekap_menikah_dengan_wna_".$nama_bulan."_".$tahun.".xls";	
}

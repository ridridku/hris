...
require_once($DIR_HOME.'/laporan/inc.rekap_relaas_dalam_negeri.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/rekap_relaas_dalam_negeri';

#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/rekap_relaas_dalam_negeri';
...
$var_cari="kode_jenis_panggil=".$kode_jenis_panggil."&kode_asal=".$kode_asal."&no_propinsi=".$no_propinsi."&kode_pn=".$kode_pn."&bulan=".$bulan."&tahun=".$tahun;
...
if ($search != '')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
			...
			$sql_detail = " SELECT distinct nm_perwakilan,(SELECT COUNT(c.kode_terpanggil) from tbl_dok_pengadilan c left join tbl_mast_pengadilan on tbl_mast_pengadilan.kode_pn = c.kode_pn where c.kode_perwakilan=a.kode_perwakilan ";
			...
			if ($kode_kawasan !='') {
			  ...
			  $sql_total="SELECT COUNT(tbl_dok_pengadilan.kode_terpanggil) as total_orang from tbl_dok_pengadilan
			  inner join tbl_mast_perwakilan on tbl_mast_perwakilan.kode_perwakilan =tbl_dok_pengadilan.kode_perwakilan LEFT JOIN tbl_mast_negara ON tbl_mast_negara.kode_negara = tbl_mast_perwakilan.kode_negara  left join tbl_mast_pengadilan on tbl_mast_pengadilan.kode_pn = tbl_dok_pengadilan.kode_pn where 1=1 ";

				if ($tahun !='') {
					$sql_total.= " and LEFT(tbl_dok_pengadilan.tgl_surat_keluar,4) ='$tahun'  ";
				}
 
				if ($bulan !='') {
					$sql_total.= " and MONTH(tbl_dok_pengadilan.tgl_surat_keluar) = '$bulan' ";
				}
				...
				if ($kode_asal !='') {
				 ...
				 if ($kode_pn !='') {
						$sql_total.= "and  tbl_dok_pengadilan.kode_pn ='$kode_pn' ";
					 }

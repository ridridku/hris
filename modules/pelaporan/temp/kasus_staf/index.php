...
require_once($DIR_INC.'/class.pager.php');
  require_once($DIR_HOME.'/laporan/inc.kasus_staf.php');   
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/kasus_staf';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/kasus_staf';
...
$sql_instansi = "SELECT *  FROM tbl_mast_perwakilan_asing order by nama_perwakilan asc";
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$sql = "  select tbl_kasus_pengaduan_staf.*, tbl_mast_staf.* ,  nama_pengaduan,
						 nama_perwakilan,
						...
						( year(now()) -  LEFT(tgl_lahir,4)) as usia
						 from  tbl_kasus_pengaduan_staf
						 inner join  tbl_mast_staf on tbl_mast_staf.kode_staf = tbl_kasus_pengaduan_staf.kode_staf
						 left  join tbl_mast_pengaduan on tbl_mast_pengaduan.kode_pengaduan = tbl_kasus_pengaduan_staf.kode_pengaduan
						...
						if ($bulan !=''  ) {
						 ...
						 if ($kode_perwakilan !='') {
						 	$sql.=" and tbl_kasus_pengaduan_staf.kode_perwakilan_asing='$kode_perwakilan' ";
						 }


						   if ($nama !='') {
								$sql.=" and tbl_mast_staf.nama LIKE '%".$nama."%' "; 
						  }
...
$sql.=" order by tgl_pengaduan asc ";
				...
				$sql_detail = "select * from tbl_tindak_lanjut_staf  ";
	...
	$content_data .= print_content($arr[nama],$arr[jk],$arr[usia],$arr[kode_form_pengaduan],$labeltgl,$arr[alamat],$arr[nm_perwakilan],$arr[kronologis_permasalahan],$arr[keterangan]);		
			    ...
			    $smarty->assign ("DATA_TB", $data_tb);
				  ...
				  $sql_alat_pinjam="select * from tbl_penyelesaian_staf inner join tbl_kasus_pengaduan_staf on tbl_kasus_pengaduan_staf.kode_form_pengaduan = tbl_penyelesaian_staf.kode_form_pengaduan ";
			  ...
			  $smarty->assign ("DATA_TB3", $data_tb3);
				...
				$file= $DIR_HOME."/files/laporan_kasus_staf_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_kasus_staf_".$nm_perwakilan."_".$tahun.".xls";			 

...
require_once($DIR_HOME.'/laporan/inc.dokumen_pengadilan_asing.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/dokumen_pengadilan_asing';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/dokumen_pengadilan_asing';
...
$sql_instansi = "SELECT *  FROM tbl_mast_perwakilan_asing order by nama_perwakilan  ";
...
if ($_GET['tahun'] != '')
{
	...
	{
				$sql__="select * , upper(nama_perwakilan) as nama_perwakilan from tbl_mast_perwakilan_asing where kode_perwakilan_asing='$kode_perwakilan_asing' ";
				$rs__=$db->Execute($sql__);			  
				 $nama_perwakilan = $rs__->fields['nama_perwakilan']; 				 
				 $smarty->assign ("NM_PERWAKILAN", $nama_perwakilan); 
				
				  

				    if ($kode_perwakilan_asing !='') {
					 
						 $sql="select distinct bulan, tahun , (select count(*) from tbl_dok_pengadilan_asing where LEFT(tgl_surat_keluar,4)='$tahun'  and SUBSTRING(tgl_surat_keluar,6,2)=v_dok_pengadilan_asing.bulan   and tbl_dok_pengadilan_asing.kode_perwakilan_asing = v_dok_pengadilan_asing.kode_perwakilan_asing   )  as jml_dok from v_dok_pengadilan_asing where tahun='$tahun'  and v_dok_pengadilan_asing.kode_perwakilan_asing='$kode_perwakilan_asing'  ";
			  		
				 } else {
					
						 $sql="select distinct bulan, tahun , (select count(*) from tbl_dok_pengadilan_asing where LEFT(tgl_surat_keluar,4)='$tahun'  and SUBSTRING(tgl_surat_keluar,6,2)=v_dok_pengadilan_asing.bulan    )  as jml_dok from v_dok_pengadilan_asing where tahun='$tahun'    ";
			    ...
			    left  join tbl_mast_pengadilan on tbl_mast_pengadilan.kode_pn = tbl_dok_pengadilan_asing.kode_pn  
				 where  LEFT(tgl_surat_keluar,4) ='$tahun'   "; 
					...
					if ($kode_perwakilan_asing !='') {
						 $sql_detail.=" and tbl_dok_pengadilan_asing.kode_perwakilan_asing='$kode_perwakilan_asing' ";
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					$content_data .= print_content($arr2[nama_perwakilan],$arr2[no_surat_diplomatik],$arr2[nama_terpanggil],$arr2[nama_pn],$arr2[no_surat_keluar],$arr2[tgl_surat_keluar],
									$arr2[no_surat_balasan],$arr2[no_surat_diplomatik],$arr2[keterangan]);
					...
					$tot="<b>TOTAL LAPORAN DOKUMEN ASING : </b>".$arr[jml_dok];
...
//print 	$sql_detail ;
			   ...
			   $smarty->assign ("DATA_TB2", $data_tb2);
				...
				$file= $DIR_HOME."/files/laporan_dokumen_pengadilan_asing_".$nama_perwakilan."_".$tahun.".xls";
				...
				$content = print_header($nama_perwakilan,$tahun);
				$content .= $content_data;
				$content .= print_footer();
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_dokumen_pengadilan_asing_".$nama_perwakilan."_".$tahun.".xls";

...
require_once($DIR_HOME.'/laporan/inc.rekap_melepaskan_kewarganegaraan.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/rekap_melepaskan_kewarganegaraan';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/rekap_melepaskan_kewarganegaraan';
...
if ($_GET['tahun'] != '')
{
	...
	(SELECT COUNT(c.kode_wni) from tbl_lepas_wn c where c.kode_perwakilan=a.kode_perwakilan     ";

 if ($bulan !='') {
	$sql_detail .= "and MONTH(c.tgl_lepas_wn) ='$bulan' ";
 }
 if ($tahun !='') {
	$sql_detail .= " and LEFT(c.tgl_lepas_wn,4) = '$tahun'   ";
 }
...
if ($kode_kawasan !=''){
		...
		FROM tbl_mast_kawasan a where kode_kawasan='$kode_kawasan' order by nama_kawasan ";
			}
...
FROM tbl_mast_kawasan a order by nama_kawasan ";
 ...
 if ($kode_kawasan !=''){
 $sql_total="
SELECT COUNT(kode_wni) as total_orang from tbl_lepas_wn
 inner join tbl_mast_perwakilan on tbl_lepas_wn.kode_perwakilan=tbl_mast_perwakilan.kode_perwakilan
LEFT JOIN tbl_mast_negara ON tbl_mast_perwakilan.kode_negara=tbl_mast_negara.kode_negara
LEFT JOIN tbl_mast_kawasan ON tbl_mast_negara.kode_kawasan=tbl_mast_kawasan.kode_kawasan
 where  LEFT(tgl_lepas_wn,4) ='$tahun' and tbl_mast_kawasan.kode_kawasan ='$_GET[kode_kawasan]' ";

	 if ($bulan !='') {
		$sql_total.= " 	and MONTH(tgl_lepas_wn) = $bulan ";
		}
}

if ($kode_kawasan ==''){
		  $sql_total="SELECT COUNT(kode_wni) as total_orang from tbl_lepas_wn where LEFT(tgl_lepas_wn,4) ='$tahun'  ";
		if ($bulan !='') {	
		 $sql_total.= " 	and MONTH(tgl_lepas_wn) = $bulan ";
		 }   
			...
			$numresults4=$db->Execute($sql_total);
				...
				while ($arr4=$recordSet4->FetchRow()) {
					...
					$content_data .= print_content("","<b>TOTAL YANG MELEPAS KEWARGANEGARAAN </b>",$tot2);
				 	 $content_data2 .= print_content("","<b>TOTAL YANG MELEPAS KEWARGANEGARAAN </b>",$tot2);
...
//$file= $DIR_HOME."/files/laporan_melepaskan_kewarganegaraan_".$nm_perwakilan."_".$tahun.".xls";
				...
				$file= $DIR_HOME."/files/rekap_melepaskan_kewarganegaraan_".$nama_bulan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/rekap_melepaskan_kewarganegaraan_".$nama_bulan."_".$tahun.".xls";	
}

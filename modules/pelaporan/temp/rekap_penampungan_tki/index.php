...
require_once($DIR_HOME.'/laporan/inc.rekap_penampungan_tki.php'); 
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/rekap_penampungan_tki';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/rekap_penampungan_tki';
...
$var_cari="no_propinsi=".$no_propinsi."&id_kab=".$id_kab."&usia=".$usia."&jk=".$jk."&kode_perwakilan=".$kode_perwakilan."&kode_kat_kasus=".$kode_kat_kasus."&kode_kawasan=".$kode_kawasan."&bulan=".$bulan."&tahun=".$tahun;
...
(SELECT COUNT(c.kode_wni) from tbl_wni_penampungan c 
 inner join tbl_mast_perwakilan d on (c.kode_perwakilan=d.kode_perwakilan)   where c.kode_perwakilan=a.kode_perwakilan
  and c.status=1   and LEFT(c.tgl_masuk,4) = '$tahun' and MONTH(c.tgl_masuk) =$bulan) as orang,
(SELECT COUNT(c.kode_wni) from tbl_wni_penampungan c   inner join tbl_mast_perwakilan d on (c.kode_perwakilan=d.kode_perwakilan) 
  where c.kode_perwakilan=a.kode_perwakilan   and c.status=2   and LEFT(c.tgl_masuk,4) = '$tahun' and MONTH(c.tgl_masuk) =$bulan) as orang2 FROM tbl_mast_perwakilan a LEFT JOIN tbl_mast_negara ON tbl_mast_negara.kode_negara = a.kode_negara LEFT JOIN tbl_mast_kawasan ON tbl_mast_kawasan.kode_kawasan = tbl_mast_negara.kode_kawasan
...
order by nm_perwakilan  ";
	...
	if ($bulan =='') {
		$sql_detail = "SELECT distinct nm_perwakilan,
(SELECT COUNT(c.kode_wni) from tbl_wni_penampungan c  inner join tbl_mast_perwakilan d on (c.kode_perwakilan=d.kode_perwakilan)
   where c.kode_perwakilan=a.kode_perwakilan and c.status=1  and LEFT(c.tgl_masuk,4) = '$tahun') as orang ,
(SELECT COUNT(c.kode_wni) from tbl_wni_penampungan c   inner join tbl_mast_perwakilan d on (c.kode_perwakilan=d.kode_perwakilan) 
  where c.kode_perwakilan=a.kode_perwakilan  and c.status=2   and LEFT(c.tgl_masuk,4) = '$tahun') as orang2
FROM tbl_mast_perwakilan a LEFT JOIN tbl_mast_negara ON tbl_mast_negara.kode_negara = a.kode_negara
LEFT JOIN tbl_mast_kawasan ON tbl_mast_kawasan.kode_kawasan = tbl_mast_negara.kode_kawasan 
 ...
 order by nm_perwakilan  ";	
	...
	$sql_detail = "SELECT distinct nm_perwakilan,
(SELECT COUNT(c.kode_wni) from tbl_wni_penampungan c 
 inner join tbl_mast_perwakilan d on (c.kode_perwakilan=d.kode_perwakilan) 
  where c.kode_perwakilan=a.kode_perwakilan
  and c.status=1 
  and LEFT(c.tgl_masuk,4) = '$tahun' and MONTH(c.tgl_masuk) =$bulan) as orang,
(SELECT COUNT(c.kode_wni) from tbl_wni_penampungan c 
  inner join tbl_mast_perwakilan d on (c.kode_perwakilan=d.kode_perwakilan) 
  where c.kode_perwakilan=a.kode_perwakilan
  and c.status=2 
  and LEFT(c.tgl_masuk,4) = '$tahun' and MONTH(c.tgl_masuk) =$bulan) as orang2
...
(SELECT COUNT(c.kode_wni) from tbl_wni_penampungan c 
  ...
  where c.kode_perwakilan=a.kode_perwakilan
  and c.status=1 
  and LEFT(c.tgl_masuk,4) = '$tahun') as orang ,
(SELECT COUNT(c.kode_wni) from tbl_wni_penampungan c 
  inner join tbl_mast_perwakilan d on (c.kode_perwakilan=d.kode_perwakilan) 
  where c.kode_perwakilan=a.kode_perwakilan
  and c.status=2 
  and LEFT(c.tgl_masuk,4) = '$tahun') as orang2
FROM tbl_mast_perwakilan a  order by nm_perwakilan  ";	
	}
	}
...
if ($kode_kawasan !=''){
			   $sql_kawasan="SELECT distinct nama_kawasan,(SELECT COUNT(tbl_wni_penampungan.kode_wni) from tbl_wni_penampungan
				...
				where tbl_wni_penampungan.status=2	and LEFT(tbl_wni_penampungan.tgl_masuk,4) ='$tahun'   and MONTH(tgl_masuk)= $bulan and tbl_mast_kawasan.kode_kawasan=a.kode_kawasan) as orang2
					FROM tbl_mast_kawasan a  where kode_kawasan ='$_GET[kode_kawasan]' "; 
					}
 ...
 if ($kode_kawasan ==''){
			...
			if ($kode_kawasan !=''){
			  $sql_total2="SELECT COUNT(kode_wni) as total_orang2 from tbl_wni_penampungan A
			   left join tbl_mast_perwakilan on A.kode_perwakilan=tbl_mast_perwakilan.kode_perwakilan
LEFT JOIN tbl_mast_negara ON tbl_mast_perwakilan.kode_negara=tbl_mast_negara.kode_negara
LEFT JOIN tbl_mast_kawasan ON tbl_mast_negara.kode_kawasan=tbl_mast_kawasan.kode_kawasan
				  where A.status=2 and LEFT(A.tgl_masuk,4) ='$tahun' and tbl_mast_kawasan.kode_kawasan ='$_GET[kode_kawasan]' ";
				if ($bulan !='') {	
					$sql_total2.= " and MONTH(A.tgl_masuk) = $bulan ";
				}
				}
				
				if ($kode_kawasan ==''){
			  $sql_total2="SELECT COUNT(kode_wni) as total_orang2 from tbl_wni_penampungan A
				  where A.status=2 and LEFT(A.tgl_masuk,4) ='$tahun' ";
				if ($bulan !='') {	
					$sql_total2.= " and MONTH(A.tgl_masuk) = $bulan ";
				}
				}
					$rs_total=$db->Execute($sql_total2);
					 $total_orang2 = $rs_total->fields['total_orang2'];
					 $smarty->assign ("TOTAL_ORANG2", $total_orang2);


if ($kode_kawasan !=''){
			  $sql_total="SELECT COUNT(kode_wni) as total_orang from tbl_wni_penampungan A
			  left join tbl_mast_perwakilan on A.kode_perwakilan=tbl_mast_perwakilan.kode_perwakilan
LEFT JOIN tbl_mast_negara ON tbl_mast_perwakilan.kode_negara=tbl_mast_negara.kode_negara
LEFT JOIN tbl_mast_kawasan ON tbl_mast_negara.kode_kawasan=tbl_mast_kawasan.kode_kawasan 
  where A.status=1 and LEFT(A.tgl_masuk,4) ='$tahun' and tbl_mast_kawasan.kode_kawasan ='$_GET[kode_kawasan]' ";
if ($bulan !='') {	
	$sql_total.= " 	and MONTH(A.tgl_masuk) = $bulan ";
}
}

if ($kode_kawasan ==''){
			  $sql_total="SELECT COUNT(kode_wni) as total_orang from tbl_wni_penampungan A
  where A.status=1 and LEFT(A.tgl_masuk,4) ='$tahun' ";
if ($bulan !='') {	
	$sql_total.= " 	and MONTH(A.tgl_masuk) = $bulan ";
}
}
				...
				while ($arr4=$recordSet4->FetchRow()) {
					 ...
					 $content_data .= print_content("","<b>TOTAL WNI/TKI DI PENAMPUNGAN</b>",$tot2);
				 	 $content_data2 .= print_content("","<b>TOTAL WNI/TKI DI PENAMPUNGAN</b>",$tot2);
			  ...
			  $smarty->assign ("DATA_TB4", $data_tb4);
				$file= $DIR_HOME."/files/rekap_tki_wni_dipenjara_".$nama_bulan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/rekap_tki_wni_dipenjara_".$nama_bulan."_".$tahun.".xls";		

...
require_once($DIR_HOME.'/laporan/inc.rekap_jenazah.php'); 
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/rekap_pemulangan_jenazah';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/rekap_pemulangan_jenazah';
...
if ($_GET['tahun'] != '')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				if ($kode_kawasan !=''){
						...
						$sql_detail = "SELECT distinct nm_perwakilan,  (SELECT COUNT(c.kode_wni) from tbl_jenazah c left join tbl_mast_perwakilan d on           (c.kode_perwakilan=d.kode_perwakilan)    left join tbl_mast_sebab_kematian e on (c.kode_sebab=e.kode_sebab)  where d.kode_perwakilan=a.kode_perwakilan and kode_pilihan_meninggal=1 and LEFT(c.tgl_meninggal,4) = '$tahun'  ";

						  if ($bulan !='') {
						$sql_detail .= "and MONTH(c.tgl_tiba) =$bulan ";
						}
					   if ($kode_sebab !='') {
						$sql_detail .= "and e.kode_sebab ='$_GET[kode_sebab]'";
						}

						 $sql_detail .= ") as orang  FROM tbl_mast_perwakilan  a
						 ...
						 where tbl_mast_kawasan.kode_kawasan ='$kode_kawasan'
			   	...
			   	if ($kode_kawasan ==''){
						...
						$sql_detail .= ") as orang FROM tbl_mast_perwakilan a order by nm_perwakilan  ";
				}
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					//$tot="TOTAL PEMULANGAN JENAZAH : ".$arr[jml_meninggal];
...
where kode_pilihan_meninggal=1	and LEFT(tgl_meninggal,4) ='$tahun' ";
 if ($kode_sebab !='') {
	$sql_kawasan .= "and tbl_mast_sebab_kematian.kode_sebab ='$_GET[kode_sebab]'";
	}
if ($bulan !='') {
  $sql_kawasan .= "and MONTH(tgl_tiba)= $bulan and tbl_mast_kawasan.kode_kawasan=a.kode_kawasan) as orang
FROM tbl_mast_kawasan a where kode_kawasan ='$kode_kawasan'";
	}


	$sql_kawasan .= " and tbl_mast_kawasan.kode_kawasan=a.kode_kawasan) as orang
FROM tbl_mast_kawasan a
 where kode_kawasan ='$kode_kawasan'  ";
...
where kode_pilihan_meninggal=1	and LEFT(tgl_meninggal,4) ='$tahun' ";
 if ($kode_sebab !='') {
	$sql_kawasan .= "and tbl_mast_sebab_kematian.kode_sebab ='$kode_sebab'";
	}
if ($bulan !='') {
  $sql_kawasan .= "and MONTH(tgl_tiba)= $bulan and tbl_mast_kawasan.kode_kawasan=a.kode_kawasan) as orang
...
LEFT JOIN tbl_mast_kawasan ON tbl_mast_negara.kode_kawasan=tbl_mast_kawasan.kode_kawasan
 ...
 where kode_pilihan_meninggal=1 and LEFT(tgl_meninggal,4) ='$tahun' and tbl_mast_kawasan.kode_kawasan ='$_GET[kode_kawasan]' ";
if ($bulan !='') {
...
if ($kode_kawasan ==''){
			   ...
			   $sql_total="SELECT COUNT(kode_wni) as total_orang from tbl_jenazah left join tbl_mast_sebab_kematian on tbl_jenazah.kode_sebab=tbl_mast_sebab_kematian.kode_sebab where kode_pilihan_meninggal=1 and LEFT(tgl_meninggal,4) ='$tahun'
                
                  ";
               if ($bulan !='') {
				$sql_total.= " 	and MONTH(tgl_tiba) = $bulan ";
				}
                 if ($kode_sebab !='') {
	$sql_total .= "and tbl_mast_sebab_kematian.kode_sebab ='$_GET[kode_sebab]'";
	}}
				...
				while ($arr4=$recordSet4->FetchRow()) {
					 ...
					 $content_data .= print_content("","<b>TOTAL PEMULANGAN JENAZAH </b>",$tot2);
				 	 $content_data2 .= print_content("","<b>TOTAL PEMULANGAN JENAZAH </b>",$tot2);
			  ...
			  $smarty->assign ("DATA_TB4", $data_tb4);
				$file= $DIR_HOME."/files/rekap_pemulangan_jenazah_".$nama_bulan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/rekap_pemulangan_jenazah_".$nama_bulan."_".$tahun.".xls";		

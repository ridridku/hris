...
require_once($DIR_HOME.'/laporan/inc.legalisasi_dokument.php');  
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/legalisasi_dokument';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/legalisasi_dokument';
...
if ($_GET['tahun'] != '')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$rs__=$db->Execute($sql__);			  
				 ...
				 if ($kode_perwakilan !='') {
					 $sql="select distinct  bulan, tahun , (select count(*) from tbl_berita_masuk where LEFT(tanggal_stempel,4)='$tahun'  and SUBSTRING(tanggal_stempel,6,2)=v_legalisasi_dokument.bulan  and tbl_berita_masuk.kode_perwakilan = v_legalisasi_dokument.kode_perwakilan  )  as jml_dokument from v_legalisasi_dokument where tahun='$tahun'  and v_legalisasi_dokument.kode_perwakilan='$kode_perwakilan'  ";
			
				 } 
				 else {
					 $sql="select distinct bulan, tahun , (select count(*) from tbl_berita_masuk where LEFT(tanggal_stempel,4)='$tahun'  and SUBSTRING(tanggal_stempel,6,2)=v_legalisasi_dokument.bulan  )  as jml_dokument from v_legalisasi_dokument where tahun='$tahun'    ";
				...
				$sql.=" order by bulan  asc ";
							...
							while ($arr=$recordSet->FetchRow()) {
								...
								$label="<b>Bulan ".$nama_bulan."</bulan>";
								
								$content_data .= print_content($label,"","","","","","",""); 
										
										
					$sql_detail = " select * , SUBSTRING(tanggal_stempel,6,2) as bulan , 
  LEFT(tanggal_stempel,4) as tahun , RIGHT(tanggal_stempel,2) as tanggal ,kode_jenis_dok,tanggal_stempel
  from tbl_berita_masuk 
  inner join tbl_wni on tbl_wni.kode_wni = tbl_berita_masuk.kode_wni 
  inner join tbl_mast_perwakilan on tbl_mast_perwakilan.kode_perwakilan = tbl_berita_masuk.kode_perwakilan 
  left join tbl_mast_wil_kabupaten on tbl_mast_wil_kabupaten.id_kabupaten = tbl_wni.id_kabupaten ";

				    if ($bulan !='') {
					 $sql_detail.=" and SUBSTRING(tanggal_stempel,6,2)='$bulan' ";
					}

					if ($kode_perwakilan !='') {
					 $sql_detail.=" and tbl_berita_masuk.kode_perwakilan='$kode_perwakilan' ";
...
//var_dump($sql_detail) or die();
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					if($arr2[kode_jenis_dok]==1){
					$jenis_dokumen="Akte Cerai ";
					} elseif($arr2[kode_jenis_dok]==2){
					$jenis_dokumen="Akte Cerai Kelahiran";
					}elseif($arr2[kode_jenis_dok]==3){
					$jenis_dokumen="Akte Kematian";
					}elseif($arr2[kode_jenis_dok]==4){
					$jenis_dokumen="Akte lain";
					}
					
					if(($arr2[bulan]==$arr[bulan])){
					$content_data .= print_content($arr2[nama],$arr2[kode_wni],$jenis_kelamin,$arr2[nm_perwakilan],$arr2[tanggal_stempel],$jenis_dokumen);
				...
				$tot="</b>TOTAL LEGALISASI DOKUMEN : </b>".$arr[jml_dokument];
			  ...
			  $smarty->assign ("DATA_TB3", $data_tb3);
				...
				$file= $DIR_HOME."/files/laporan_legalisasi_dokument_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_legalisasi_dokument_".$nm_perwakilan."_".$tahun.".xls";		

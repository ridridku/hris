...
require_once($DIR_HOME.'/laporan/inc.pelayanan_perlindungan.php');  
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/pelayanan_perlindungan';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/pelayanan_perlindungan';
...
// SHOW DATA PELAYANAN ========================
$sql_pel = "SELECT  distinct nama_jenis_pelayanan  FROM tbl_mast_jenis_pelayanan
inner join tbl_mast_jenis_pelayanan_detail on tbl_mast_jenis_pelayanan.kode_jenis_pelayanan =  tbl_mast_jenis_pelayanan_detail.kode_jenis_pelayanan
 inner join tbl_pelayanan on tbl_mast_jenis_pelayanan_detail.kode_sub_jenis_pelayanan=tbl_pelayanan.kode_sub_jenis_pelayanan  ";
$result_pelayanan = $db->Execute($sql_pel);
$initSet = array();
...
$sql = "SELECT distinct nama_jenis_pelayanan,
(SELECT COUNT(c.kode_sub_jenis_pelayanan) 
from tbl_pelayanan c 
left join tbl_mast_jenis_pelayanan_detail d on (c.kode_sub_jenis_pelayanan=d.kode_sub_jenis_pelayanan) where d.kode_jenis_pelayanan=a.kode_jenis_pelayanan and YEAR(c.tgl_pelayanan) = '$tahun'";
 if ($bulan !='') {
					 $sql.=" and MONTH(c.tgl_pelayanan)='$bulan' ";
					}

					if ($kode_perwakilan !='') {
					 $sql.=" and c.kode_perwakilan='$kode_perwakilan' ";

					}

$sql.=") as jml_pelayanan
FROM tbl_mast_jenis_pelayanan a
inner join tbl_mast_jenis_pelayanan_detail on a.kode_jenis_pelayanan =  tbl_mast_jenis_pelayanan_detail.kode_jenis_pelayanan
 inner join tbl_pelayanan on tbl_mast_jenis_pelayanan_detail.kode_sub_jenis_pelayanan=tbl_pelayanan.kode_sub_jenis_pelayanan 
 where YEAR(tgl_pelayanan) ='$tahun'   ";

				    if ($bulan !='') {
					 $sql.=" and MONTH(tgl_pelayanan)='$bulan' ";
					}

					if ($kode_perwakilan !='') {
					 $sql.=" and tbl_pelayanan.kode_perwakilan='$kode_perwakilan' ";
							...
							while ($arr=$recordSet->FetchRow()) {
										...
										$label="<b>PELAYANAN ".$arr[nama_jenis_pelayanan]."</b>";
								...
								$content_data .= print_content($label,"","","","",""); 			
									...
									$sql_detail = "select 
nama_jenis_pelayanan, nama_pelayanan,
nama,
no_surat,
tgl_pelayanan,
keterangan, nm_perwakilan, MONTH(tgl_pelayanan)  as bulan , YEAR(tgl_pelayanan) as tahun , DAY(tgl_pelayanan) as tanggal  
				from tbl_pelayanan 
				inner join tbl_mast_jenis_pelayanan_detail on tbl_mast_jenis_pelayanan_detail.kode_sub_jenis_pelayanan =tbl_pelayanan.kode_sub_jenis_pelayanan  
				inner join tbl_mast_jenis_pelayanan on tbl_mast_jenis_pelayanan.kode_jenis_pelayanan =  tbl_mast_jenis_pelayanan_detail.kode_jenis_pelayanan 
left join tbl_wni on tbl_wni.kode_wni = tbl_pelayanan.kode_wni
inner join tbl_mast_perwakilan on tbl_mast_perwakilan.kode_perwakilan=  tbl_pelayanan.kode_perwakilan 
				  where YEAR(tgl_pelayanan) ='$tahun'   ";

				    if ($bulan !='') {
					 $sql_detail.=" and MONTH(tgl_pelayanan)='$bulan' ";
					}

					if ($kode_perwakilan !='') {
					 $sql_detail.=" and tbl_pelayanan.kode_perwakilan='$kode_perwakilan' ";
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					if(($arr2[nama_jenis_pelayanan]==$arr[nama_jenis_pelayanan])){
					$content_data .= print_content($arr2[nama_pelayanan],$arr2[nm_perwakilan],$arr2[nama],$arr2[no_surat],$arr2[tgl_pelayanan],$arr2[keterangan]);
					...
					$tot="<b>TOTAL PELAYANAN :</b> ".$arr[jml_pelayanan];
		...
		$file= $DIR_HOME."/files/laporan_pelayanan_perlindungan_".$nm_perwakilan."_".$tahun."_".$bulan.".xls";
				...
				$content = print_header($nm_perwakilan,$tahun,$bulan);
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_pelayanan_perlindungan_".$nm_perwakilan."_".$tahun."_".$bulan.".xls";			

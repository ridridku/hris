...
require_once($DIR_HOME.'/laporan/inc.asuransi.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/asuransi';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/asuransi';
...
if ($_GET['tahun'] != '')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
			 ...
			 $sql="select distinct bulan, tahun , 
			 (select count(*) from tbl_asuransi_tki  inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_asuransi_tki.kode_form_pengaduan inner join  tbl_wni on tbl_wni.kode_wni = tbl_kasus_pengaduan.kode_wni  where LEFT(tgl_penyerahan,4)='$tahun'  and SUBSTRING(tgl_penyerahan,6,2)=v_asuransi.bulan  and tbl_kasus_pengaduan.kode_perwakilan = v_asuransi.kode_perwakilan   )  as jml_meninggal
			 
			 
			 , (select sum(ekuivalent_rp) from tbl_asuransi_tki  inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_asuransi_tki.kode_form_pengaduan  inner join  tbl_wni on tbl_wni.kode_wni = tbl_kasus_pengaduan.kode_wni where LEFT(tgl_penyerahan,4)='$tahun'  and SUBSTRING(tgl_penyerahan,6,2)=v_asuransi.bulan  and tbl_kasus_pengaduan.kode_perwakilan = v_asuransi.kode_perwakilan     )  as jml_rp from v_asuransi where tahun='$tahun'  and v_asuransi.kode_perwakilan='$kode_perwakilan' ";
			
			} else {
					
			 $sql="select distinct bulan, tahun , 
			 (select count(*) from tbl_asuransi_tki  inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_asuransi_tki.kode_form_pengaduan inner join  tbl_wni on tbl_wni.kode_wni = tbl_kasus_pengaduan.kode_wni  where LEFT(tgl_penyerahan,4)='$tahun'  and SUBSTRING(tgl_penyerahan,6,2)=v_asuransi.bulan )  as jml_meninggal
			 
			 
			 , (select sum(ekuivalent_rp) from tbl_asuransi_tki  inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_asuransi_tki.kode_form_pengaduan inner join  tbl_wni on tbl_wni.kode_wni = tbl_kasus_pengaduan.kode_wni where LEFT(tgl_penyerahan,4)='$tahun'  and SUBSTRING(tgl_penyerahan,6,2)=v_asuransi.bulan )  as jml_rp from v_asuransi where tahun='$tahun' ";
 ...
 jumlah,
 ekuivalent_rp,
 tbl_asuransi_tki.kode_form_pengaduan ,
 no_bast ,

tbl_asuransi_tki.keterangan 	from tbl_asuransi_tki
					inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_asuransi_tki.kode_form_pengaduan
...
tbl_asuransi_tki.keterangan from tbl_asuransi_tki
               ...
               $numresults2=$db->Execute($sql_detail);
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					$content_data .= print_content($arr2[nama],$arr2[no_paspor],$arr2[jk],$arr2[nm_perwakilan],$arr2[tgl_penyerahan],$arr2[jenis_asuransi],
									$arr2[valas],$arr2[jumlah],$arr2[nama_kasus],$arr2[ekuivalent_rp]);
...
//print 	$sql_detail ;
			   ...
			   $smarty->assign ("DATA_TB2", $data_tb2);
				   ...
				   $smarty->assign ("DATA_TB3", $data_tb3);
                    ...
                    $file= $DIR_HOME."/files/laporan_asuransi_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_asuransi_".$nm_perwakilan."_".$tahun.".xls";

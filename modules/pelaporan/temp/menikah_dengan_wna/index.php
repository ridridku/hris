...
require_once($DIR_HOME.'/laporan/inc.menikah.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/menikah_dengan_wna';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/menikah_dengan_wna';
...
if ($_GET['tahun'] != '')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$rs__=$db->Execute($sql__);
				 ...
				 if ($kode_perwakilan !='') {
					 $sql="select distinct bulan, tahun , (select count(*) from tbl_nikah_wna where LEFT(tgl_nikah,4)='$tahun'  and SUBSTRING(tgl_nikah,6,2)=v_nikah_wna.bulan  and tbl_nikah_wna.kode_perwakilan = v_nikah_wna.kode_perwakilan  )  as jml_nikah from v_nikah_wna where tahun='$tahun'";			
				 } else {
					 $sql="select distinct bulan, tahun , (select count(*) from tbl_nikah_wna where LEFT(tgl_nikah,4)='$tahun'  and SUBSTRING(tgl_nikah,6,2)=v_nikah_wna.bulan   )  as jml_nikah from v_nikah_wna where tahun='$tahun'";
				 ...
				 if ($bulan !='') {
					...
					if ($kode_perwakilan !='') {
						 $sql.=" and v_nikah_wna.kode_perwakilan='$kode_perwakilan' ";
				...
				$sql.=" order by bulan  asc ";
                	...
                	$sql_detail = "select * , tbl_nikah_wna.alamat_ln as alamat_di_ln, SUBSTRING(tgl_nikah,6,2)  as bulan , LEFT(tgl_nikah,4) as tahun , RIGHT(tgl_nikah,2) as tanggal
				from tbl_nikah_wna
				left join tbl_wni on tbl_wni.kode_wni = tbl_nikah_wna.kode_wni
				left join tbl_mast_perwakilan on tbl_mast_perwakilan.kode_perwakilan = tbl_nikah_wna.kode_perwakilan

				 and LEFT(tgl_nikah,4) ='$tahun'   ";


				 if ($bulan !='') {
						$sql_detail.=" and SUBSTRING(tgl_nikah,6,2) = '$bulan' ";
					}
					if ($kode_perwakilan !='') {
						 $sql_detail.=" and tbl_nikah_wna.kode_perwakilan='$kode_perwakilan' ";
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					$content_data .= print_content($arr2[nama],$arr2[nama_wna],$arr2[nm_perwakilan],$arr2[alamat_ln],$arr2[keterangan]);
					...
					$tot="<b>TOTAL LAPORAN MENIKAH WNA :</b> ".$arr[jml_nikah];
...
$sql_detail = "select * , tbl_nikah_wna.alamat_ln as alamat_di_ln, SUBSTRING(tgl_nikah,6,2)  as bulan , LEFT(tgl_nikah,4) as tahun , RIGHT(tgl_nikah,2) as tanggal
			   ...
			   $smarty->assign ("DATA_TB2", $data_tb2);
               $file= $DIR_HOME."/files/laporan_menikah_wna_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_menikah_wna_".$nm_perwakilan."_".$tahun.".xls";

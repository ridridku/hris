...
//----------Menampilkan Data Ruas---------------------------
 ...
 if ($_GET['tahun'] != '')
 {
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				/*if ($bulan =='') {	
					 $sql="select bulan, tahun , (select count(*) from tbl_adopsi where LEFT(tgl_adopsi,4)='$tahun'  and SUBSTRING(tgl_adopsi,6,2)=v_adopsi.bulan   )  as jml_adopsi from v_adopsi where tahun='$tahun'    ";
				}

				if ($bulan !='') {					
					  $sql="select bulan, tahun , (select count(*) from tbl_adopsi where LEFT(tgl_adopsi,4)='$tahun' and SUBSTRING(tgl_adopsi,6,2)=$bulan   ) as jml_adopsi from v_adopsi where tahun='$tahun' and bulan='$bulan'  ";
				}*/
				if ($kode_kawasan ==''){
					$sql = "SELECT a.nm_perwakilan,
						...
						where tbl_kasus_pengaduan.kode_perwakilan = a.kode_perwakilan and YEAR(tgl_ditahan)='$tahun'";
						
						if ($bulan !='') {	
 						 $sql .= "and MONTH(tgl_ditahan)='$bulan' "; 
									}

					
						$sql .= " ) as jumlah,
						IFNULL((select sum(tbl_penahanan.kode_form_pengaduan) from tbl_penahanan
						LEFT JOIN tbl_kasus_pengaduan ON tbl_kasus_pengaduan.kode_form_pengaduan = tbl_penahanan.kode_form_pengaduan
						LEFT JOIN  tbl_mast_perwakilan on  tbl_mast_perwakilan.kode_perwakilan =  tbl_kasus_pengaduan.kode_perwakilan
						LEFT JOIN tbl_mast_negara ON tbl_mast_negara.kode_negara = tbl_mast_perwakilan.kode_negara
						LEFT JOIN tbl_mast_kawasan ON tbl_mast_kawasan.kode_kawasan = tbl_mast_negara.kode_kawasan
						where tbl_kasus_pengaduan.kode_perwakilan = a.kode_perwakilan and YEAR(tgl_ditahan)='$tahun'";
			   ...
			   $smarty->assign ("DATA_TB2", $data_tb2);
				...
				$sql_total="select distinct (select count(kode_wni)  as total_orang from tbl_kasus_pengaduan
						...
						LEFT JOIN tbl_mast_negara ON tbl_mast_negara.kode_negara = tbl_mast_perwakilan.kode_negara
						LEFT JOIN tbl_mast_kawasan ON tbl_mast_kawasan.kode_kawasan = tbl_mast_negara.kode_kawasan
						where tbl_mast_kawasan.kode_kawasan ='$_GET[kode_kawasan]' and YEAR(tgl_ditahan)='$tahun'";
						
						if ($bulan !='') {	
 						 $sql_total .= "and MONTH(tgl_ditahan)='$bulan' "; 
									}
				...
				$sql_total="select distinct (select count(kode_wni)  as total_orang from tbl_kasus_pengaduan
						...
						LEFT JOIN  tbl_mast_perwakilan on  tbl_mast_perwakilan.kode_perwakilan =  tbl_kasus_pengaduan.kode_perwakilan
						LEFT JOIN tbl_mast_negara ON tbl_mast_negara.kode_negara = tbl_mast_perwakilan.kode_negara
						LEFT JOIN tbl_mast_kawasan ON tbl_mast_kawasan.kode_kawasan = tbl_mast_negara.kode_kawasan where  YEAR(tgl_ditahan)='$tahun' ";
				...
				while ($arr3=$recordSet3->FetchRow()) {
					 ...
					 $tot1="<b>TOTAL PENAHANAN WNI : </b>".$arr3[tot_wni];
			...
			$file= $DIR_HOME."/files/rekap_tki_wni_ditahan_".$nm_kawasan.".xls";
				...
				$content = print_header($nm_kawasan,$nama_bulan,$tahun);
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/rekap_tki_wni_ditahan_".$nm_kawasan.".xls";		
}

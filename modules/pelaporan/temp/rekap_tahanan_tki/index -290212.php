...
require_once($DIR_HOME.'/laporan/inc.rekap_tahanan_tki.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/rekap_tahanan_tki';

#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/rekap_tahanan_tki';
...
(SELECT COUNT(tbl_kasus_pengaduan.kode_wni) from tbl_penahanan c
inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = c.kode_form_pengaduan
  left join tbl_mast_perwakilan d on  tbl_kasus_pengaduan.kode_perwakilan=d.kode_perwakilan
  where d.kode_perwakilan=a.kode_perwakilan

  and LEFT(c.tgl_ditahan,4) = '$tahun' and month(c.tgl_ditahan) = '$bulan' ) as orang
...
inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = c.kode_form_pengaduan
  ...
  and LEFT(c.tgl_ditahan,4) = '$tahun'  ) as orang
FROM tbl_mast_perwakilan a order by nm_perwakilan  ";
		...
		if ($_GET['page']) $start = $p->findStartGet($LIMIT); else $start = $p->findStartPost($LIMIT);
			   ...
			   $sql_kawasan="SELECT distinct nama_kawasan,(SELECT COUNT(tbl_kasus_pengaduan.kode_wni) from tbl_penahanan
			   inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_penahanan.kode_form_pengaduan
				LEFT join tbl_mast_perwakilan on tbl_kasus_pengaduan.kode_perwakilan=tbl_mast_perwakilan.kode_perwakilan
				...
				where   LEFT(tbl_penahanan.tgl_ditahan,4) ='$tahun' ";


				if ($bulan !='') {
				  $sql_kawasan .= "and MONTH(tgl_ditahan)= $bulan and tbl_mast_kawasan.kode_kawasan=a.kode_kawasan) as orang
			  ...
			  $sql_total="SELECT COUNT(tbl_kasus_pengaduan.kode_wni) as total_orang from tbl_penahanan A
			  inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = A.kode_form_pengaduan

  where LEFT(A.tgl_ditahan,4) ='$tahun' ";
if ($bulan !='') {
	$sql_total.= " 	and MONTH(A.tgl_ditahan) = $bulan ";
}
				...
				while ($arr4=$recordSet4->FetchRow()) {
					 ...
					 $content_data .= print_content("","<b>TOTAL WNI/TKI YANG DITAHAN</b>",$tot2);
				 	 $content_data2 .= print_content("","<b>TOTAL WNI/TKI YANG DITAHAN</b>",$tot2);
			  ...
			  $smarty->assign ("DATA_TB4", $data_tb4);
				...
				$file= $DIR_HOME."/files/rekap_tki_wni_ditahan_".$nama_bulan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/rekap_tki_wni_ditahan_".$nama_bulan."_".$tahun.".xls";

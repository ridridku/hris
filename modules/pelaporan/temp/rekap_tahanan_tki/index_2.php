...
(SELECT COUNT(c.kode_wni) from tbl_wni_penjara c 
...
FROM tbl_mast_perwakilan a  ";
		...
		if ($_GET['page']) $start = $p->findStartGet($LIMIT); else $start = $p->findStartPost($LIMIT);
			   ...
			   $sql_kawasan="SELECT distinct nama_kawasan,(SELECT COUNT(tbl_wni_penjara.kode_wni) from tbl_wni_penjara
LEFT join tbl_mast_perwakilan on tbl_wni_penjara.kode_perwakilan=tbl_mast_perwakilan.kode_perwakilan
...
where tbl_wni_penjara.status=1	and LEFT(tbl_wni_penjara.tgl_masuk,4) ='$tahun' ";


if ($bulan !='') {	
  $sql_kawasan .= "and MONTH(tgl_masuk)= $bulan and tbl_mast_kawasan.kode_kawasan=a.kode_kawasan) as orang
...
FROM tbl_mast_kawasan a";
		...
		if ($_GET['page']) $start = $p->findStartGet($LIMIT); else $start = $p->findStartPost($LIMIT);
			  ...
			  $sql_total="SELECT COUNT(kode_wni) as total_orang from tbl_wni_penjara A

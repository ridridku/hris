...
require_once($DIR_INC.'/class.pager.php');
 require_once($DIR_HOME.'/laporan/inc.rekap_deportasi.php'); 
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/rekap_deportasi';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/rekap_deportasi';
	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$rs__=$db->Execute($sql__);			  
				 ...
				 $smarty->assign ("NM_KAWASAN", $nm_kawasan); 
					...
					$sql_detail = "SELECT nm_perwakilan,(SELECT COUNT(c.kode_wni) from tbl_pemulangan c where c.kode_perwakilan=a.kode_perwakilan and kode_pilihan=1 ";
					
					if ($tahun !='') {
						$sql_detail.= "and LEFT(c.tgl_pemulangan,4) ='$tahun' ";
					}

					if ($bulan !='') {
						$sql_detail.= " and MONTH(c.tgl_pemulangan) ='$bulan' ";
					}

					$sql_detail.= " ) as orang,"; 


					$sql_detail.= " (SELECT COUNT(c.kode_wni) from tbl_pemulangan c where c.kode_perwakilan=a.kode_perwakilan and kode_pilihan=2 ";
					...
					$sql_detail.= " ) as orang2,"; 


					$sql_detail.= " (SELECT COUNT(c.kode_wni) from tbl_pemulangan c where c.kode_perwakilan=a.kode_perwakilan and kode_pilihan=3 ";
					...
					$sql_detail.= " ) as orang3 "; 


					$sql_detail.= " FROM tbl_mast_perwakilan a   LEFT JOIN tbl_mast_negara ON tbl_mast_negara.kode_negara = a.kode_negara ";
			...
			if ($tahun !='') {	
			 ...
			 if ($kode_kawasan !='') {	
			  ...
			  $smarty->assign ("DATA_TB3", $data_tb3);
					...
					$sql_total=" SELECT COUNT(kode_wni) as total_orang from tbl_pemulangan  inner join tbl_mast_perwakilan on tbl_pemulangan.kode_perwakilan=tbl_mast_perwakilan.kode_perwakilan
			LEFT JOIN tbl_mast_negara ON tbl_mast_perwakilan.kode_negara=tbl_mast_negara.kode_negara
			LEFT JOIN tbl_mast_kawasan ON tbl_mast_negara.kode_kawasan=tbl_mast_kawasan.kode_kawasan where kode_pilihan=1  ";
					

					if ($tahun !='') {	
						$sql_total.= " AND LEFT(tgl_pemulangan,4) ='$tahun'  ";
					}


					if ($bulan !='') {	
						$sql_total.= " 	and MONTH(tgl_pemulangan) = '$bulan' ";
					}
				...
				while ($arr4=$recordSet4->FetchRow()) {
					 ...
					 $smarty->assign ("TOTAL_ORANG", $tot2);

					 $content_data .= print_content("","<b>TOTAL DEPORTASI </b>",$tot2);
				 	 $content_data2 .= print_content("","<b>TOTAL DEPORTASI </b>",$tot2);
			  ...
			  $smarty->assign ("DATA_TB4", $data_tb4);
					...
					$sql_total2=" SELECT COUNT(kode_wni) as total_orang2 from tbl_pemulangan  inner join tbl_mast_perwakilan on tbl_pemulangan.kode_perwakilan=tbl_mast_perwakilan.kode_perwakilan
					LEFT JOIN tbl_mast_negara ON tbl_mast_perwakilan.kode_negara=tbl_mast_negara.kode_negara
					LEFT JOIN tbl_mast_kawasan ON tbl_mast_negara.kode_kawasan=tbl_mast_kawasan.kode_kawasan where  kode_pilihan= 2";

					if ($tahun !='') {	
						$sql_total2.= " AND LEFT(tgl_pemulangan,4) ='$tahun'  ";
					}


					if ($bulan !='') {	
						$sql_total2.= " 	and MONTH(tgl_pemulangan) = '$bulan' ";
					}

					 if ($kode_kawasan !='') {	
						 $sql_total2 .= " and tbl_mast_kawasan.kode_kawasan= '$kode_kawasan' ";
					}
					...
					$rs_total=$db->Execute($sql_total2);
					 ...
					 $sql_total3=" SELECT COUNT(kode_wni) as total_orang3 from tbl_pemulangan  inner join tbl_mast_perwakilan on tbl_pemulangan.kode_perwakilan=tbl_mast_perwakilan.kode_perwakilan
			LEFT JOIN tbl_mast_negara ON tbl_mast_perwakilan.kode_negara=tbl_mast_negara.kode_negara
			LEFT JOIN tbl_mast_kawasan ON tbl_mast_negara.kode_kawasan=tbl_mast_kawasan.kode_kawasan where kode_pilihan= 3 ";

					if ($tahun !='') {	
						$sql_total3.= " AND LEFT(tgl_pemulangan,4) ='$tahun'  ";
					}


					if ($bulan !='') {	
						$sql_total3.= " 	and MONTH(tgl_pemulangan) = '$bulan' ";
					}

					 if ($kode_kawasan !='') {	
						 $sql_total3 .= " and tbl_mast_kawasan.kode_kawasan= '$kode_kawasan' ";
					}
				


					$rs_total3=$db->Execute($sql_total3);
					 $total_orang3 = $rs_total3->fields['total_orang3'];
					 $smarty->assign ("TOTAL_ORANG3", $total_orang3);



					$file= $DIR_HOME."/files/laporan_rekap_deportasi_".$nama_bulan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_rekap_deportasi_".$nama_bulan."_".$tahun.".xls";	

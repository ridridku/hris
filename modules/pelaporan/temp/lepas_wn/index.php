...
require_once($DIR_HOME.'/laporan/inc.laporan_lepas_wn.php'); 
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/lepas_wn';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/lepas_wn';
	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$rs__=$db->Execute($sql__);
				 ...
				 if ($kode_perwakilan !='') {
					 ...
					 $sql="select distinct bulan, tahun , (select count(*) from tbl_lepas_wn where LEFT(tgl_lepas_wn,4)='$tahun'  and SUBSTRING(tgl_lepas_wn,6,2)=v_lepas_wn.bulan   and tbl_lepas_wn.kode_perwakilan = v_lepas_wn.kode_perwakilan   )  as jml_lepas_wn from v_lepas_wn where tahun='$tahun'    ";

				 } else {
					
					 $sql="select distinct bulan, tahun , (select count(*) from tbl_lepas_wn where LEFT(tgl_lepas_wn,4)='$tahun'  and SUBSTRING(tgl_lepas_wn,6,2)=v_lepas_wn.bulan     )  as jml_lepas_wn from v_lepas_wn where tahun='$tahun'    ";
					...
					if ($kode_perwakilan !='') {
						 $sql.=" and v_lepas_wn.kode_perwakilan='$kode_perwakilan' ";
				...
				$sql_detail = "select *, SUBSTRING(tgl_lepas_wn,6,2)  as bulan , LEFT(tgl_lepas_wn,4) as tahun , RIGHT(tgl_lepas_wn,2) as tanggal  from tbl_lepas_wn
				inner join tbl_wni on tbl_wni.kode_wni = tbl_lepas_wn.kode_wni
				inner join tbl_mast_perwakilan on tbl_mast_perwakilan.kode_perwakilan = tbl_wni.kode_perwakilan
				left join tbl_mast_negara on tbl_mast_negara.kode_negara = tbl_lepas_wn.kode_negara
				left join tbl_mast_wil_kabupaten on tbl_mast_wil_kabupaten.id_kabupaten = tbl_wni.id_kabupaten
				and LEFT(tgl_lepas_wn,4) ='$tahun' ";


				 
				  if ($bulan !='') {
						 $sql_detail.=" and SUBSTRING(tgl_lepas_wn,6,2)='$bulan' ";
					}

					if ($kode_perwakilan !='') {
						 $sql_detail.=" and tbl_lepas_wn.kode_perwakilan='$kode_perwakilan' ";
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					$content_data .= print_content($arr2[kode_lepas],$arr2[nama],$arr2[jk],$arr2[tgl_lepas_wn],$arr2[alamat_tujuan],$arr2[kode_negara]);
					...
					$tot="<b>TOTAL LAPORAN MELEPAS KEWARGANEGARAAN :</b> ".$arr[jml_lepas_wn];
               ...
               $file= $DIR_HOME."/files/laporan_lepas_wn_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_lepas_wn".$nm_perwakilan."_".$tahun.".xls";

...
require_once($DIR_HOME.'/laporan/inc.persidangan.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/sidang';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/sidang';
	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$rs__=$db->Execute($sql__);			  
				 ...
				 $smarty->assign ("NM_PERWAKILAN", $nm_perwakilan); 
					...
					$sql="select bulan, tahun , (select count(*) from tbl_sidang_hasil inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_sidang_hasil.kode_form_pengaduan inner join  tbl_wni on tbl_wni.kode_wni = tbl_kasus_pengaduan.kode_wni  where LEFT(tgl_sidang,4)='$tahun'  and SUBSTRING(tgl_sidang,6,2)=v_sidang.bulan    and tbl_kasus_pengaduan.kode_perwakilan = v_sidang.kode_perwakilan )  as jml_meninggal from v_sidang where tahun='$tahun'    ";
  
					if ($bulan !='') {
						$sql.=" and bulan='$bulan' ";
					}
					if ($kode_perwakilan !='') {
						 $sql.=" and v_sidang.kode_perwakilan='$kode_perwakilan' ";
				...
				$sql_detail = "select
				tbl_sidang_hasil.* , SUBSTRING(tgl_sidang,6,2)  as bulan , LEFT(tgl_sidang,4) as tahun , RIGHT(tgl_sidang,2) as tanggal   ,

			(CASE  when ( kode_kat_kasus=1  or kode_kat_kasus=2 or kode_kat_kasus=3)  then (select nama from tbl_wni where tbl_wni.kode_wni=tbl_kasus_pengaduan.kode_wni)  else '2'  end ) as nama 
			 
							from tbl_sidang_hasil
							...
							inner join tbl_mast_perwakilan on tbl_mast_perwakilan.kode_perwakilan = tbl_kasus_pengaduan.kode_perwakilan
							 ...
							 where  LEFT(tgl_sidang,4) ='$tahun'   ";

					if ($bulan !='') {
						 $sql_detail.=" and SUBSTRING(tgl_sidang,6,2)='$bulan' ";
					}
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					$content_data .= print_content($arr2[nama],$arr2[no_paspor],$arr2[jk],$arr2[nm_perwakilan],$arr2[tgl_sidang],$arr2[pengacara_pendamping],
									$arr2[materi],$arr2[nama_kasus],$arr2[keterangan]);
					...
					$tot="TOTAL LAPORAN PERSIDANGAN: ".$arr[jml_meninggal];
		   ...
		   $sql_detail3="select * from tbl_kasus_pengaduan_detail  inner join tbl_mast_kasus on tbl_mast_kasus.kode_kasus =tbl_kasus_pengaduan_detail.kode_kasus    ";
				   ...
				   $smarty->assign ("DATA_TB3", $data_tb3);
                    $file= $DIR_HOME."/files/laporan_persidangan_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_persidangan_".$nm_perwakilan."_".$tahun.".xls";

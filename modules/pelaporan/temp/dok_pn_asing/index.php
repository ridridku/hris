...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/dok_pn_asing';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/dok_pn_asing';
...
if ($_GET['tahun'] != '')
{
	...
	{
				$sql__="select * , upper(nama_perwakilan) as nm_perwakilan from tbl_mast_perwakilan_asing where kode_perwakilan_asing='$kode_perwakilan_asing' ";
				$rs__=$db->Execute($sql__);			  
				 $nm_perwakilan = $rs__->fields['nama_perwakilan']; 				 
				 $smarty->assign ("NM_PERWAKILAN", $nm_perwakilan); 
				  
				
				if ($bulan =='') {	
					 $sql="select bulan, tahun , (select count(*) from tbl_dok_pengadilan_asing where LEFT(tgl_surat_keluar,4)='$tahun'  and SUBSTRING(tgl_surat_keluar,6,2)=v_pn_asing.bulan   )  as jml_dok_asing from v_pn_asing where tahun='$tahun'    ";
				}

				if ($bulan !='') {					
					  $sql="select bulan, tahun , (select count(*) from tbl_dok_pengadilan_asing where LEFT(tgl_surat_keluar,4)='$tahun' and SUBSTRING(tgl_surat_keluar,6,2)=$bulan   ) as jml_dok_asing from v_pn_asing where tahun='$tahun' and bulan='$bulan'  ";
				}
				...
				inner join tbl_mast_pengadilan on tbl_mast_pengadilan.kode_pn = tbl_dok_pengadilan_asing.kode_pn

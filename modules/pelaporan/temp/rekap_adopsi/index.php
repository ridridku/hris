...
require_once($DIR_HOME.'/laporan/inc.rekap_adopsi.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/rekap_adopsi';

#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/rekap_adopsi';
...
if ($_GET['tahun'] != '')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
  	...
  	if ($kode_kawasan !=''){
	$sql_detail = "SELECT distinct nm_perwakilan,(SELECT COUNT(c.kode_perwakilan) from tbl_adopsi c  left join tbl_mast_perwakilan d on  c.kode_perwakilan=d.kode_perwakilan
  where d.kode_perwakilan=a.kode_perwakilan  and LEFT(c.tgl_adopsi,4) = '$tahun' ";
    if ($bulan !='') {
	$sql_detail .= "and MONTH(c.tgl_adopsi) =$bulan";
	}
...
(SELECT COUNT(c.kode_perwakilan) from tbl_adopsi c
 left join tbl_mast_perwakilan d on  c.kode_perwakilan=d.kode_perwakilan
  where d.kode_perwakilan=a.kode_perwakilan

  and LEFT(c.tgl_adopsi,4) = '$tahun'    ";
...
FROM tbl_mast_perwakilan a order by nm_perwakilan  ";
		...
		if ($_GET['page']) $start = $p->findStartGet($LIMIT); else $start = $p->findStartPost($LIMIT);
			   ...
			   $sql_kawasan="SELECT distinct nama_kawasan,(SELECT COUNT(tbl_adopsi.kode_perwakilan) from tbl_adopsi
			   LEFT join tbl_mast_perwakilan on tbl_adopsi.kode_perwakilan=tbl_mast_perwakilan.kode_perwakilan
				...
				where   LEFT(tbl_adopsi.tgl_adopsi,4) ='$tahun' ";


				if ($bulan !='') {
				  $sql_kawasan .= "and MONTH(tgl_adopsi)= $bulan and tbl_mast_kawasan.kode_kawasan=a.kode_kawasan) as orang
...
if ($kode_kawasan !=''){
$sql_total="
SELECT COUNT(tbl_adopsi.kode_perwakilan) as total_orang from tbl_adopsi
left join tbl_mast_perwakilan on tbl_adopsi.kode_perwakilan=tbl_mast_perwakilan.kode_perwakilan
LEFT JOIN tbl_mast_negara ON tbl_mast_perwakilan.kode_negara=tbl_mast_negara.kode_negara
LEFT JOIN tbl_mast_kawasan ON tbl_mast_negara.kode_kawasan=tbl_mast_kawasan.kode_kawasan

 where tbl_adopsi.kode_perwakilan and LEFT(tgl_adopsi,4) ='$tahun' and tbl_mast_kawasan.kode_kawasan ='$_GET[kode_kawasan]' ";
if ($bulan !='') {
	$sql_total .= " 	and MONTH(tgl_adopsi) = $bulan ";
	}
}

if ($kode_kawasan ==''){
			  $sql_total="SELECT COUNT(tbl_adopsi.kode_perwakilan) as total_orang from tbl_adopsi
  where LEFT(tbl_adopsi.tgl_adopsi,4) ='$tahun' ";
if ($bulan !='') {
	$sql_total .= " and MONTH(tgl_adopsi) = $bulan ";
}
				...
				while ($arr4=$recordSet4->FetchRow()) {
					 ...
					 $content_data .= print_content("","<b>TOTAL ADOPSI</b>",$tot2);
				 	 $content_data2 .= print_content("","<b>TOTAL ADOPSI</b>",$tot2);
			  ...
			  $smarty->assign ("DATA_TB4", $data_tb4);
				$file= $DIR_HOME."/files/rekap_adopsi_".$nama_bulan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/rekap_adopsi_".$nama_bulan."_".$tahun.".xls";

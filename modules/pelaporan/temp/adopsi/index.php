...
require_once($DIR_HOME.'/laporan/inc.adopsi.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/adopsi';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/adopsi';
...
if ($_GET['tahun'] != '')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$rs__=$db->Execute($sql__);			  
				 ...
				 if ($kode_perwakilan !='') {
					 $sql="select distinct bulan, tahun , (select count(*) from tbl_adopsi where LEFT(tgl_adopsi,4)='$tahun'  and SUBSTRING(tgl_adopsi,6,2)=v_adopsi.bulan  and tbl_adopsi.kode_perwakilan = v_adopsi.kode_perwakilan )  as jml_adopsi from v_adopsi where tahun='$tahun'  and v_adopsi.kode_perwakilan='$kode_perwakilan'  ";				
				 } else {
					$sql="select distinct bulan, tahun , (select count(*) from tbl_adopsi where LEFT(tgl_adopsi,4)='$tahun'  and SUBSTRING(tgl_adopsi,6,2)=v_adopsi.bulan  )  as jml_adopsi from v_adopsi where tahun='$tahun'    ";	
				...
				$sql_detail = "select * , SUBSTRING(tgl_adopsi,6,2)  as bulan , LEFT(tgl_adopsi,4) as tahun , RIGHT(tgl_adopsi,2) as tanggal,

				tbl_wni.nama as wni
				from tbl_adopsi
				inner join tbl_mast_perwakilan on tbl_mast_perwakilan.kode_perwakilan = tbl_adopsi.kode_perwakilan
				inner join tbl_wni on tbl_wni.kode_wni = tbl_adopsi.kode_wni



				 where  LEFT(tgl_adopsi,4) ='$tahun'   ";
				

					if ($bulan !='') {
						$sql_detail.=" and SUBSTRING(tgl_adopsi,6,2)='$bulan' ";
					}
					if ($kode_perwakilan !='') {
						 $sql_detail.=" and tbl_adopsi.kode_perwakilan='$kode_perwakilan' ";
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					$content_data .= print_content($arr2[wni],$arr2[nama_calon_ota],$arr2[nm_perwakilan],$arr2[alamat_cota],$arr2[keterangan]);
					...
					$tot="TOTAL LAPORAN ADOPSI WNI : ".$arr[jml_adopsi];
...
$sql_detail = "select * , SUBSTRING(tgl_adopsi,6,2)  as bulan , LEFT(tgl_adopsi,4) as tahun , RIGHT(tgl_adopsi,2) as tanggal ,
				...
				tbl_wni.nama as wni
				from tbl_adopsi
				inner join tbl_mast_perwakilan on tbl_mast_perwakilan.kode_perwakilan = tbl_adopsi.kode_perwakilan inner join tbl_wni on tbl_wni.kode_wni = tbl_adopsi.kode_wni
			   ...
			   $smarty->assign ("DATA_TB2", $data_tb2);
 $file= $DIR_HOME."/files/laporan_adopsi_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_adopsi_".$nm_perwakilan."_".$tahun.".xls";

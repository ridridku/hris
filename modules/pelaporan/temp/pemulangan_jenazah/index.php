...
require_once($DIR_HOME.'/laporan/inc.pemulangan_jenazah.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/pemulangan_jenazah';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/pemulangan_jenazah';
...
if ($_GET['tahun'] != '')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$rs__=$db->Execute($sql__);			  
				 ...
				 if ($kode_perwakilan !='') {
					 $sql="select distinct  bulan, tahun , (select count(*) from tbl_jenazah where LEFT(tgl_meninggal,4)='$tahun'  and SUBSTRING(tgl_meninggal,6,2)=v_jenazah.bulan and kode_pilihan_meninggal=1 and tbl_jenazah.kode_perwakilan = v_jenazah.kode_perwakilan  )  as jml_meninggal from v_jenazah where tahun='$tahun'   and v_jenazah.kode_perwakilan='$kode_perwakilan' ";
			
				 } else {
					 $sql="select distinct bulan, tahun , (select count(*) from tbl_jenazah where LEFT(tgl_meninggal,4)='$tahun'  and SUBSTRING(tgl_meninggal,6,2)=v_jenazah.bulan and kode_pilihan_meninggal=1   )  as jml_meninggal from v_jenazah where tahun='$tahun'    ";
    			 }
				...
				$sql.=" order by bulan  asc ";
				 ...
				 $sql_detail = "select tbl_wni.nama , tbl_wni.kode_wni  , jk , nm_perwakilan, tgl_tiba, entry_point, nama_kabupaten, no_bast, tgl_bast, tbl_jenazah.keterangan , SUBSTRING(tgl_pulang,6,2)  as bulan , LEFT(tgl_pulang,4) as tahun , RIGHT(tgl_pulang,2) as tanggal  
				from tbl_jenazah inner join  tbl_wni on tbl_wni.kode_wni = tbl_jenazah.kode_wni inner join tbl_mast_perwakilan on tbl_mast_perwakilan.kode_perwakilan = tbl_jenazah.kode_perwakilan
				 left  join tbl_mast_wil_kabupaten on tbl_mast_wil_kabupaten.id_kabupaten = tbl_wni.id_kabupaten where kode_pilihan_meninggal=1
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					$content_data .= print_content($arr2[nama],$arr2[kode_wni],$jenis_kelamin,$arr2[nm_perwakilan],$arr2[tgl_tiba],$arr2[entry_point],
									$arr2[nama_kabupaten],$arr2[keterangan]);
				...
				$tot="<b>TOTAL PEMULANGAN JENAZAH : </b>".$arr[jml_meninggal];
			  ...
			  $smarty->assign ("DATA_TB3", $data_tb3);
				...
				$file= $DIR_HOME."/files/laporan_pemulangan_jenazah_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_pemulangan_jenazah_".$nm_perwakilan."_".$tahun.".xls";		

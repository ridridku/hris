...
require_once($DIR_INC.'/class.pager.php');
 require_once($DIR_HOME.'/laporan/inc.deportasi.php');  
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/deportasi';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/deportasi';
...
if ($_GET['kode_kat_kasus']) $kode_kat_kasus = $_GET['kode_kat_kasus'];
 ...
 if ($_GET['kode_pilihan']) $kode_pilihan = $_GET['kode_pilihan'];
else if ($_POST['kode_pilihan']) $kode_pilihan = $_POST['kode_pilihan'];
else $kode_pilihan="";
	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$rs__=$db->Execute($sql__);			  
				 ...
				 if ($kode_perwakilan !='') {
						...
						if ($kode_pilihan !='') {
							$sql="select distinct bulan, tahun , (select count(*) from tbl_pemulangan where LEFT(tgl_pemulangan,4)='$tahun'  and SUBSTRING(tgl_pemulangan,6,2)=v_pemulangan.bulan  and tbl_pemulangan.kode_perwakilan = v_pemulangan.kode_perwakilan and  tbl_pemulangan.kode_pilihan=v_pemulangan.kode_pilihan )  as jml_meninggal from v_pemulangan where tahun='$tahun' and v_pemulangan.kode_pilihan='$kode_pilihan' ";
						} else {
							 $sql="select distinct bulan, tahun , (select count(*) from tbl_pemulangan where LEFT(tgl_pemulangan,4)='$tahun'  and SUBSTRING(tgl_pemulangan,6,2)=v_pemulangan.bulan  and tbl_pemulangan.kode_perwakilan = v_pemulangan.kode_perwakilan  )  as jml_meninggal from v_pemulangan where tahun='$tahun'    ";
							  ...
							  if ($kode_pilihan !='') {
								 $sql="select distinct bulan, tahun , (select count(*) from tbl_pemulangan where LEFT(tgl_pemulangan,4)='$tahun'  and SUBSTRING(tgl_pemulangan,6,2)=v_pemulangan.bulan and  tbl_pemulangan.kode_pilihan=v_pemulangan.kode_pilihan  )  as jml_meninggal from v_pemulangan where tahun='$tahun'  and v_pemulangan.kode_pilihan='$kode_pilihan'  ";
							} else {
								 $sql="select distinct bulan, tahun , (select count(*) from tbl_pemulangan where LEFT(tgl_pemulangan,4)='$tahun'  and SUBSTRING(tgl_pemulangan,6,2)=v_pemulangan.bulan  )  as jml_meninggal from v_pemulangan where tahun='$tahun' ";
					...
					if ($kode_perwakilan !='') {
						 $sql.=" and v_pemulangan.kode_perwakilan='$kode_perwakilan' ";

					}

// print "<BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR>".$sql;
				...
				$sql.=" order by bulan  asc ";
							...
							while ($arr=$recordSet->FetchRow()) {
								...
								$label="<b> Bulan ".$nama_bulan."</b>";
								
								$content_data .= print_content($label,"","","","","","","","","",""); 
								
								 $sql_detail = "select  tbl_wni.nama , tbl_wni.kode_wni  , jk , nm_perwakilan  ,nama_kabupaten, 
								 tgl_tiba, entry_point, no_sk_pemulangan,tgl_sk,kode_pilihan, tbl_pemulangan.keterangan , 
								  SUBSTRING(tgl_pemulangan,6,2)  as bulan , LEFT(tgl_pemulangan,4) as tahun , RIGHT(tgl_pemulangan,2) as tanggal  , nama_kasus
								from tbl_pemulangan inner join  tbl_wni on tbl_wni.kode_wni = tbl_pemulangan.kode_wni inner join tbl_mast_perwakilan on tbl_mast_perwakilan.kode_perwakilan = tbl_pemulangan.kode_perwakilan
								...
								left join tbl_mast_kasus on tbl_mast_kasus.kode_kasus = tbl_kasus_pengaduan.kode_kasus 
								 where  LEFT(tgl_pemulangan,4) ='$tahun'   ";

					if ($bulan !='') {
						 $sql_detail.=" and SUBSTRING(tgl_pemulangan,6,2)='$bulan' ";
					}

					if ($kode_perwakilan !='') {
						 $sql_detail.=" and tbl_pemulangan.kode_perwakilan='$kode_perwakilan' ";
					}

					if ($kode_pilihan !='') {
						 $sql_detail.=" and tbl_pemulangan.kode_pilihan='$kode_pilihan' ";
					}
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					$content_data .= print_content($arr2[nama],$arr2[kode_wni],$jenis_kelamin,$arr2[nm_perwakilan],$arr2[tgl_pemulangan],
										$arr2[entry_point],$arr2[nama_kabupaten],$arr2[nama_kasus],$arr2[no_sk_pemulangan]
										,$arr2[tgl_sk],$arr2[keterangan]);
				...
				$tot="<b>TOTAL TKI/WNI  : </b>".$arr[jml_meninggal];
			   ...
			   $smarty->assign ("DATA_TB2", $data_tb2);	
				...
				$file= $DIR_HOME."/files/laporan_deportasi_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_deportasi_".$nm_perwakilan."_".$tahun.".xls";	

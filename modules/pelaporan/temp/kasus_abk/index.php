...
require_once($DIR_HOME.'/laporan/inc.kasus_abk.php');   
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/kasus_abk';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/kasus_abk';
...
if ($_GET['search'] == '1')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$sql = " select nama_kapal, bendera ,  kronologis_permasalahan, bantuan , kode_kat_kasus,tbl_kasus_pengaduan.kode_wni , tbl_wni.nama , tbl_wni.alamat , tbl_wni.alamat , jk, no_paspor ,   kode_form_pengaduan,  nama_kabupaten, nama_kasus, nama_pengaduan,
						 nm_perwakilan,
						 SUBSTRING(tgl_pengaduan,6,2)  as bulan_tl ,
						 LEFT(tgl_pengaduan,4) as tahun_tl , 
						 RIGHT(tgl_pengaduan,2) as tanggal_tl , nm_perwakilan, nama_negara,
						( YEAR(CURDATE()) - LEFT(tgl_lahir,4))  as usia  
									...
									where 1=1  and ( kode_kat_kasus=3 ) ";
...
$content_data .= print_content($arr[nama],$jenis_kelamin,$arr[usia],$arr[kode_wni],$arr[nama_kabupaten],
				$arr[nama_negara],$arr[nm_perwakilan],$arr[nama_kapal],$arr[bendera],$labeltgl,$arr[kronologis_permasalahan],"","","");
				} 
			...
			$file= $DIR_HOME."/files/laporan_kasus_abk_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_kasus_abk_".$nm_perwakilan."_".$tahun.".xls";	

...
require_once($DIR_HOME.'/laporan/inc.rekap_legalisasi.php');  
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/rekap_legalisasi';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/rekap_legalisasi';
...
if ($_GET['kode_jenis_dok']) $kode_jenis_dok = $_GET['kode_jenis_dok'];
else if ($_POST['kode_jenis_dok']) $kode_jenis_dok = $_POST['kode_jenis_dok'];
else $kode_jenis_dok="";
...
$var_cari="kode_jenis_dok=".$kode_jenis_dok."&bulan=".$bulan."&tahun=".$tahun;
...
(SELECT COUNT(c.kode_wni) from tbl_berita_masuk c left join tbl_mast_perwakilan d on (c.kode_perwakilan=d.kode_perwakilan) where d.kode_perwakilan=a.kode_perwakilan 
 
  ";		

	if ($tahun !='') {	
		$sql_detail.=" and LEFT(c.tanggal_stempel,4) = '$tahun'";
	}

	if ($bulan !='') {	
		$sql_detail.=" and MONTH(c.tanggal_stempel) ='$bulan' ";
	}

	if ($kode_jenis_dok !='') {	
		$sql_detail.=" and kode_jenis_dok ='$kode_jenis_dok' ";
	}
...
(SELECT COUNT(tbl_berita_masuk.kode_wni) from tbl_berita_masuk
 inner  join tbl_mast_perwakilan on tbl_berita_masuk.kode_perwakilan=tbl_mast_perwakilan.kode_perwakilan
...
if ($tahun !='') {	
  $sql_kawasan .= "  AND YEAR(tanggal_stempel) ='$tahun'   "; 
 }

 if ($bulan !='') {	
  $sql_kawasan .= "and MONTH(tanggal_stempel)= '$bulan'    "; 
 }

 if ($kode_jenis_dok !='') {	
 $sql_kawasan.=" and kode_jenis_dok ='$kode_jenis_dok' ";
 }


 $sql_kawasan.=" ) as orang
...
if ($kode_kawasan !='') {
 ...
 $sql_kawasan.= " order by nama_kawasan ";
			  ...
			  $sql_total="SELECT COUNT(kode_wni) as total_orang from tbl_berita_masuk inner  join tbl_mast_perwakilan on tbl_berita_masuk.kode_perwakilan=tbl_mast_perwakilan.kode_perwakilan
LEFT JOIN tbl_mast_negara ON tbl_mast_perwakilan.kode_negara=tbl_mast_negara.kode_negara
LEFT JOIN tbl_mast_kawasan ON tbl_mast_negara.kode_kawasan=tbl_mast_kawasan.kode_kawasan where 1=1  ";


	if ($tahun !='') {	
	$sql_total.= " AND 	YEAR(tanggal_stempel) ='$tahun'   ";
	}


	if ($bulan !='') {	
	$sql_total.= " 	and MONTH(tanggal_stempel) = '$bulan' ";
	}

	 if ($kode_jenis_dok !='') {	
	 $sql_total.=" and kode_jenis_dok ='$kode_jenis_dok' ";
	 }
	 ...
	 $sql_total.="   and   tbl_mast_kawasan.kode_kawasan = '$kode_kawasan' ";
				...
				while ($arr4=$recordSet4->FetchRow()) {
					...
					$content_data .= print_content("","<b>TOTAL LEGALISASI DOKUMEN </b>",$tot2);
				 	 $content_data2 .= print_content("","<b>TOTAL LEGALISASI DOKUMEN </b>",$tot2);
			  ...
			  $smarty->assign ("DATA_TB4", $data_tb4);
				...
				$file= $DIR_HOME."/files/rekap_legalisasi_".$nama_bulan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/rekap_legalisasi_".$nama_bulan."_".$tahun.".xls";	
}

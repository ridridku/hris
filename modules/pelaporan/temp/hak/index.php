...
require_once($DIR_HOME.'/laporan/inc.hak.php');
	...
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				...
				$rs__=$db->Execute($sql__);			  
				 ...
				 if ($kode_perwakilan !='') {
					 ...
					 $sql="select distinct bulan, tahun , (select count(*) from tbl_hak_tki inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_hak_tki.kode_form_pengaduan inner join  tbl_wni on tbl_wni.kode_wni = tbl_kasus_pengaduan.kode_wni where LEFT(tgl_penyerahan,4)='$tahun'  and SUBSTRING(tgl_penyerahan,6,2)=v_hak_tki.bulan and tbl_kasus_pengaduan.kode_perwakilan = v_hak_tki.kode_perwakilan )  as jml_meninggal
					  , (select sum(ekuivalent_rp) from tbl_hak_tki inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_hak_tki.kode_form_pengaduan inner join  tbl_wni on tbl_wni.kode_wni = tbl_kasus_pengaduan.kode_wni where LEFT(tgl_penyerahan,4)='$tahun'  and SUBSTRING(tgl_penyerahan,6,2)=v_hak_tki.bulan and tbl_kasus_pengaduan.kode_perwakilan = v_hak_tki.kode_perwakilan     )  as jml_rp
					  from v_hak_tki where tahun='$tahun' and v_hak_tki.kode_perwakilan='$kode_perwakilan'   ";
			
				 } else {
					
					 $sql="select distinct bulan, tahun , (select count(*) from tbl_hak_tki inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_hak_tki.kode_form_pengaduan where LEFT(tgl_penyerahan,4)='$tahun'  and SUBSTRING(tgl_penyerahan,6,2)=v_hak_tki.bulan )  as jml_meninggal
					  , (select sum(ekuivalent_rp) from tbl_hak_tki inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_hak_tki.kode_form_pengaduan inner join  tbl_wni on tbl_wni.kode_wni = tbl_kasus_pengaduan.kode_wni where LEFT(tgl_penyerahan,4)='$tahun'  and SUBSTRING(tgl_penyerahan,6,2)=v_hak_tki.bulan     )  as jml_rp
					  from v_hak_tki where tahun='$tahun'    ";
				...
				$sql_detail = "select  tbl_wni.nama , tbl_wni.kode_wni  , jk , nm_perwakilan  ,nama_kabupaten,   SUBSTRING(tgl_penyerahan,6,2)  as bulan , LEFT(tgl_penyerahan,4) as tahun , RIGHT(tgl_penyerahan,2) as tanggal  , nama_kasus   , 
 tgl_penyerahan,
 jenis_hak,
 valas,
 jumlah,
 ekuivalent_rp,
 tbl_hak_tki.kode_form_pengaduan ,
 no_bast ,

tbl_hak_tki.keterangan 

				from tbl_hak_tki
				inner join tbl_kasus_pengaduan on tbl_kasus_pengaduan.kode_form_pengaduan = tbl_hak_tki.kode_form_pengaduan
				...
				left join tbl_mast_kasus on tbl_mast_kasus.kode_kasus = tbl_kasus_pengaduan.kode_kasus
				 ...
				 where LEFT(tgl_penyerahan,4) ='$tahun'   ";


				    if ($bulan !='') {
						 $sql_detail.=" and SUBSTRING(tgl_penyerahan,6,2)='$bulan' ";
					}

					if ($kode_perwakilan !='') {
						 $sql_detail.=" and tbl_mast_perwakilan.kode_perwakilan='$kode_perwakilan' ";
 ...
 $sql_detail.=" order by nama asc "; 
				...
				while ($arr2=$recordSet2->FetchRow()) {
					...
					$content_data .= print_content($arr2[nama],$arr2[no_paspor],$arr2[jk],$arr2[nm_perwakilan],$arr2[tgl_penyerahan],$arr2[jenis_hak],
									$arr2[valas],$arr2[jumlah],$arr2[ekuivalent_rp]);
					...
					$tot="TOTAL LAPORAN PEMENUHAN HAK : ".$arr[jml_meninggal];
...
tbl_hak_tki.keterangan 
 ...
 $sql_detail.=" order by nama asc "; 
			   ...
			   $smarty->assign ("DATA_TB2", $data_tb2);
					...
					$sql_detail3="select * from tbl_kasus_pengaduan_detail  inner join tbl_mast_kasus on tbl_mast_kasus.kode_kasus =tbl_kasus_pengaduan_detail.kode_kasus    ";
					$numresults3=$db->Execute($sql_detail3);

					$recordSet3 = $db->Execute($sql_detail3);
					...
					while ($arr3=$recordSet3->FetchRow()) {
						 $count_row = $recordSet3->RecordCount();
						...
						array_push($row_class3, $ROW_CLASSNAME);
						array_push($initSet3, $zz);
						$zz++;
					}

				   $smarty->assign ("DATA_TB3", $data_tb3);

	            $file= $DIR_HOME."/files/laporan_pemenuhan_hak_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_pemenuhan_hak_".$nm_perwakilan."_".$tahun.".xls";

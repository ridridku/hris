...
require_once($DIR_HOME.'/laporan/inc.wni_penampungan.php');
...
$TPL_PATH = base64_decode($_SESSION['THEME']).'/modules/pelaporan/wni_penampungan';
 
#SETTING FILE JS INCLUDE
$JS_MODUL = $DIR_THEME.'/'.(base64_decode($_SESSION['THEME']).'/javascripts/modules/pelaporan');
$FILE_JS  = $JS_MODUL.'/wni_penampungan';
...
if ($_GET['tahun'] != '')
{
	if (Privi($mod_id, $user_id, 'search') != 'no')
	{
				 ...
				 $smarty->assign ("NM_PERWAKILAN", $nm_perwakilan); 
					 ...
					 if ($kode_perwakilan !='') {
					 $sql="select distinct  bulan, tahun , (select count(*) from tbl_wni_penampungan   INNER JOIN tbl_wni ON tbl_wni_penampungan.kode_wni = tbl_wni.kode_wni where YEAR(tgl_masuk)='$tahun'  and MONTH(tgl_masuk)=v_penampungan.bulan and tbl_wni_penampungan.kode_perwakilan = v_penampungan.kode_perwakilan  )  as jml_wni from v_penampungan where tahun='$tahun'  and v_penampungan.kode_perwakilan='$kode_perwakilan' ";
			
				 } 
				 else {
					 $sql="select distinct bulan, tahun , (select count(*) from tbl_wni_penampungan INNER JOIN tbl_wni ON tbl_wni_penampungan.kode_wni = tbl_wni.kode_wni where YEAR(tgl_masuk)='$tahun'  and MONTH(tgl_masuk)=v_penampungan.bulan )  as jml_wni from v_penampungan where tahun='$tahun'    ";
...
tbl_wni_penampungan.kode_wni,
tbl_mast_perwakilan.nm_perwakilan,
tbl_wni_penampungan.tgl_masuk,
tbl_wni_penampungan.tgl_keluar,
MONTH(tgl_masuk) as bulan,
nama_kabupaten,
 tbl_wni.jk,
	            CASE status
	            when '1' THEN 'Masuk' ELSE 'Keluar' end AS statusa , tbl_wni_penampungan.id
	            FROM tbl_wni_penampungan
                INNER JOIN tbl_wni ON tbl_wni_penampungan.kode_wni = tbl_wni.kode_wni
  LEFT JOIN tbl_mast_wil_kabupaten ON tbl_wni.id_kabupaten = tbl_mast_wil_kabupaten.id_kabupaten
                INNER JOIN tbl_mast_perwakilan ON tbl_wni.kode_perwakilan = tbl_mast_perwakilan.kode_perwakilan
				where YEAR(tgl_masuk) ='$tahun'   ";


				    if ($bulan !='') {
						 $sql_detail.=" and MONTH(tgl_masuk)='$bulan' ";
					}

					if ($kode_perwakilan !='') {
						 $sql_detail.=" and tbl_wni_penampungan.kode_perwakilan='$kode_perwakilan' ";
				...
				while ($arr2=$recordSet2->FetchRow()) {
					    ...
					    $content_data .= print_content($arr2[kode_wni],$arr2[nama],$arr2[tgl_masuk],$arr2[tgl_keluar],$arr2[nama_kabupaten],$arr2[jk],
									$arr2[statusa]);
					...
					$tot="<b>TOTAL LAPORAN WNI PENAMPUNGAN : </b>".$arr[jml_wni]."</b>";
	            ...
	            $file= $DIR_HOME."/files/laporan_wni_penampungan_".$nm_perwakilan."_".$tahun.".xls";
				...
				fwrite($fp,$content);
				fclose($fp);
				$file_2= $HREF_HOME."/files/laporan_wni_penampungan_".$nm_perwakilan."_".$tahun.".xls";

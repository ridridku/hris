...
public static function gambar_km_meter(DatabaseInterface $db, $IdRuasJalan, $Waktu, $lbl, $panjang_image){
		...
		while($flag<$panjang_image){				
			if($flag_angka==$awal){
				imagestring($im,2,1,12,$flag_angka." Km",$line_color);
			}else{
				if($tipis==$flag_print){					
					imagestring($im,2,$x,12,$flag_angka,$line_color);
					$tipis=0;
				}else{
		...
		imagestring($im,2,890,12,$sk_length,$line_color);
...
public static function Posisi_Jembatan(
		...
		$im = @imagecreatetruecolor($panjang_image+10, 200)
		 or die("Cannot Initialize new GD image stream"); //create new file image
...
#				imagefilledrectangle($im,$lokasi-1,$tinggi_garis_merah_naik,$lokasi,$tinggi_garis_merah_turun,$color_jemb);
				imagefilledrectangle($im,$lokasi-1,12,$lokasi,188,$color_jemb);
			}
...
public static function gambar_diagram_lebar_ruas_jalan(DatabaseInterface $db, $IdRuasJalan, $Waktu, $lbl, $panjang_image){		
		...
		$im = @imagecreatetruecolor($panjang_image+20, 200)
		 or die("Cannot Initialize new GD image stream"); //create new file image
		...
		if($db->RowCount($rs) == 0){
			...
			$query= "
					  SELECT
						...
						svy_deskripsi.offset AS offset,
						svy_deskripsi.id__mst_lajur AS lajur,
						svy_deskripsi.lebar AS svy_deskripsi_lebar,
...
public static function gambar_diagram_kondisi_tepi(DatabaseInterface $db, $IdRuasJalan, $Waktu, $lbl, $panjang_image, $cek = 0){
		...
		$im = @imagecreatetruecolor($panjang_image, 200)
		 or die("Cannot Initialize new GD image stream"); //create new file image
...
/*				$awal=  (float)$row2['post'] *1000/1000;
				$akhir = (float)$row2['offset'] * 1000/1000;
				$x__y = 0;
				$x1 = ($awal * $per_px_panjang)-$x__y;
				$x2 = ($akhir * $per_px_panjang)-$x__y;
*/				
				if($lajur==LajurConstant::KI1){
					...
					if($bahu==""){	//------------buka if----------
						...
						if($db->RowCount($rs) == 0){
							...
							switch($rowBahu['bahu_jenis']){
								case "Tanah" : $rec = imagecolorallocate($im, 255, 128, 0); break;
								case "Agregat" : $rec = imagecolorallocate($im, 255, 255, 0); break;
								case "Aspal" : $rec = imagecolorallocate($im, 128, 128, 128); break;
							}
						}						
					}	//---------------------tutup if---------
					...
					if($bahu==""){	//------------buka if----------
						...
						if($db->RowCount($rs) == 0){
							...
							switch($rowBahu['bahu_jenis']){
								case "Aspal" : $rec = imagecolorallocate($im, 128, 128, 128); break;
								case "Pasangan Batu" : $rec = imagecolorallocate($im, 0, 0, 255); break;
								case "Beton" : $rec = imagecolorallocate($im, 255, 0, 255); break;
							}
						}						
					}	//---------------------tutup if---------
					...
					if($bahu==""){	//------------buka if----------
						...
						if($db->RowCount($rs) == 0){
							...
							switch($rowBahu['bahu_jenis']){
								...
								case "Pasangan Batu" : $rec = imagecolorallocate($im, 0, 0, 255); break;
								case "Beton" : $rec = imagecolorallocate($im, 255, 0, 255); break;
								case "Pancang Besi" : $rec = imagecolorallocate($im, 255, 0, 0); break;
								case "Bronjong" : $rec = imagecolorallocate($im, 0, 255, 0); break;
							}
						}						
					}	//---------------------tutup if---------
...
public static function gambar_kondisi_jalan_per_10km(DatabaseInterface $db, $IdRuasJalan, $Waktu,$Km_Ke ,$lbl ,$panjang_image){
		...
		if($db->RowCount($rs) == 0){
			...
			//-----------------gambar daerah jalan-----------------------
			while($row2 = $db->FetchArray($rs2) ){ #$row2=mysql_fetch_array($rs2)
#				$lajur=$row2['lajur'];
#				$lebar=$row2['lebar'];
#				$awal=(float)$row2['kmpost'] + $row2['offsetfr']/1000;
#				$akhir = (float)$row2['kmpostto'] + $row2['offsetto']/1000;
#				$kondisi=$row2['kondisi'];
